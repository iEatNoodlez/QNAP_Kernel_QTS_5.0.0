...
#define USB_MAX_RETRIES		10 /* # of retries before error is reported */
...
static struct gpio max3421_gpios[] = {
	{ 4, GPIOF_OUT_INIT_HIGH, "GPIO4_LEVEL_SWITCH_OE" },
	{ 26, GPIOF_IN, "GPIO26_SPIUSB_INT" },
	{ 27, GPIOF_IN, "GPIO27_SPIUSB_GPX" },
};
#endif

#if defined(CONFIG_USB_MAX3421_HCD_LS_LOCK)
extern void max3421_oe_level_shift_lock(void);
extern void max3421_oe_level_shift_unlock(void);
#endif
...
spi_rd8(struct usb_hcd *hcd, unsigned int reg)
{
...
#if defined(CONFIG_USB_MAX3421_HCD_LS_LOCK)
	max3421_oe_level_shift_lock();
#endif

#if defined(CONFIG_USB_MAX3421_HCD_OE)
#if defined(GPIO_DEBUG)
	gpio_set_value(max3421_gpios[0].gpio, 0);
#else
	gpio_set_value(pdata->gpio_oe_pin, 0);
#endif
	udelay(2);
#endif
	...
	udelay(2);
#if defined(GPIO_DEBUG)
	gpio_set_value(max3421_gpios[0].gpio, 1);
#else
	gpio_set_value(pdata->gpio_oe_pin, 1);
#endif
	udelay(2);
#endif

#if defined(CONFIG_USB_MAX3421_HCD_LS_LOCK)
	max3421_oe_level_shift_unlock();
#endif
...
#if defined(CONFIG_USB_MAX3421_HCD_VBUS)
/*
...
#if defined(CONFIG_USB_MAX3421_HCD_OE)
static int max3421_of_oe_pin(struct device *dev, struct max3421_hcd_platform_data *pdata)
{
	int retval;
	uint32_t value[2];
	...
	retval = of_property_read_u32_array(dev->of_node, "pl061,gpio-oe-pin", value, 1);
	if (retval) {
		dev_err(dev, "device tree node property 'pl061,gpio-oe-pin' is missing\n");
		return retval;
	}

	dev_info(dev, "property 'pl061,gpio-oe-pin' value is <%d>\n", value[0]);

	pdata->gpio_oe_pin = value[0];
...
static int max3421_of_intx_pin(struct device *dev, struct max3421_hcd_platform_data *pdata)
{
	int retval;
	uint32_t value[2];
	...
	retval = of_property_read_u32_array(dev->of_node, "pl061,gpio-gpx-pin", value, 1);
	if (retval) {
		dev_err(dev, "device tree node property 'pl061,gpio-gpx-pin' is missing\n");
		return retval;
	}

	dev_info(dev, "property 'pl061,gpio-gpx-pin' value is <%d>\n", value[0]);

	pdata->gpio_gpx_pin = value[0];
...
static int max3421_of_intx_pin(struct device *dev, struct max3421_hcd_platform_data *pdata)
{
	...
	retval = of_property_read_u32_array(dev->of_node, "pl061,gpio-intx-pin", value, 1);
	if (retval) {
		dev_err(dev, "device tree node property 'pl061,gpio-intx-pin' is missing\n");
		return retval;
	}

	dev_info(dev, "property 'pl061,gpio-intx-pin' value is <%d>\n", value[0]);

	pdata->gpio_intx_pin = value[0];
...
max3421_probe(struct spi_device *spi)
{
...
#if defined(GPIO_DEBUG)
	ret = gpio_request_array(max3421_gpios, ARRAY_SIZE(max3421_gpios));
	if(ret < 0) {
		printk(KERN_ERR "%s: gpio_request failed for gpios\n", __func__);
		goto error;
	}

	/* init level oe to high */
	gpio_set_value(max3421_gpios[0].gpio, 1);

	spi->irq = gpio_to_irq(max3421_gpios[1].gpio);
	if(spi->irq < 0) {
...
#if defined(CONFIG_USB_MAX3421_HCD_OE) && !defined(GPIO_DEBUG)
		retval = max3421_of_oe_pin(dev, pdata);
		if (retval)
			goto error;
		else {
			ret = gpio_request_one(pdata->gpio_oe_pin, GPIOF_OUT_INIT_HIGH, "GPIO_LEVEL_SWITCH_OE");
			if(ret) {
				dev_err(dev, "Failed to GPIO OE pin");
				goto error;
			} else {
				gpio_set_value(pdata->gpio_oe_pin, 1);
			}
...
#if defined(CONFIG_USB_MAX3421_HCD_INTX) && !defined(GPIO_DEBUG)
		retval = max3421_of_intx_pin(dev, pdata);
		if (retval)
			goto error;
		else {
			ret = gpio_request_one(pdata->gpio_intx_pin, GPIOF_IN, "GPIO_SPIUSB_INTX");
			if(ret) {
				dev_err(dev, "Failed to GPIO INTX pin");
				goto error;
			}
			spi->irq = gpio_to_irq(pdata->gpio_intx_pin);
			if(spi->irq < 0) {
				printk(KERN_ERR "Unable to request IRQ: %d\n", ret);
				return -EFAULT;
			}
	...
	retval = request_irq(spi->irq, max3421_irq_handler,
			     IRQF_TRIGGER_LOW, "max3421-spi", hcd);
	if (retval < 0) {
...
error:
#if defined(GPIO_DEBUG)
	gpio_free_array(max3421_gpios, ARRAY_SIZE(max3421_gpios));
#endif

	if (IS_ENABLED(CONFIG_OF) && dev->of_node && pdata) {
#if !defined(GPIO_DEBUG)
#if defined(CONFIG_USB_MAX3421_HCD_OE)
		gpio_free(pdata->gpio_oe_pin);
#endif
#if defined(CONFIG_USB_MAX3421_HCD_INTX)
		gpio_free(pdata->gpio_intx_pin);
#endif
#endif
