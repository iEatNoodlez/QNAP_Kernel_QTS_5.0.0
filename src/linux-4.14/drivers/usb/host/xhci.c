...
int xhci_gen_setup(struct usb_hcd *hcd, xhci_get_quirks_t get_quirks)
{
	...
	if (usb_hcd_is_primary_hcd(hcd)) {
		...
		if (xhci->sbrn == 0x31 || xhci->usb3_rhub.min_rev >= 1) {
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QND_ZX_PATCH)
			/*usb3.1 has gen1 and gen2 speed,Some zx's xHCI controller that follow usb3.1 spec but only support gen1*/
			if (pdev->vendor == PCI_VENDOR_ID_ZX) {
				printk("With Zhaoxin xHCI root hub speed patchV1.0.0.\n");
				ssp_support = 0;
			...
			xhci_info(xhci, "Host supports USB 3.1 %s SuperSpeed\n",
