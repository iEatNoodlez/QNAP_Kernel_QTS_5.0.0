				...
				dm_block_t *begin, dm_block_t *end)
{
	...
	DMDEBUG("%s:%d, bio bi_sector(%lu) bi_size(%u)", __func__, __LINE__, 
...
static ssize_t dm_attr_discard_passdown_show(struct tier *tier, char *buf)
{
	...
	sprintf(buf, "%u\n", tier_ctx->discard_passdown);

	return strlen(buf);
}

static ssize_t dm_attr_discard_passdown_store(struct tier *tier, 
	const char *buf, size_t count)
{
	int r;
	uint8_t temp;
	...
	r = kstrtou8(buf, 10, &temp);
	if (r)
		return r;

	tier_ctx->discard_passdown = temp;
...
static DM_ATTR_WR(discard_passdown);
...
int maybe_resize_tier_data_dev(struct dm_target *ti, struct tier *tier, bool *need_commit)
{
	...
	*need_commit = false;
	...
	for (i = 0; i < tc->tier_num; i++) {
		...
		DMDEBUG("%s:%d, tier_data_dev[%d] extends to %llu blocks for block_size = %u", 
