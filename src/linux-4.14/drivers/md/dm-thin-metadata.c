...
static int __write_initial_superblock(struct dm_pool_metadata *pmd)
{
	...
	// FIXME: should we shift SECTOR_SHIFT?
...
int dm_pool_metadata_close(struct dm_pool_metadata *pmd)
{
	...
	if (!pmd->fail_io && !IS_ERR_OR_NULL(pmd->bm) && !dm_bm_is_read_only(pmd->bm)) {
			 ...
			 dm_thin_id dev)
{
	...
	r = dm_btree_lookup(&pmd->details_info, pmd->details_root, &key,
						old_meta(pmd->version) ?
						(void *)&old_details_le : (void *)&details_le);
	if (!r)
		return -EEXIST;
			 ...
			 dm_thin_id dev, dm_thin_id origin, bool is_clone)
{
	...
	r = dm_btree_lookup(&pmd->details_info, pmd->details_root, &dev_key,
						old_meta(pmd->version) ?
						(void *)&old_details_le : (void *)&details_le);
	if (!r)
		return -EEXIST;
				 				 ...
				 				 struct dm_thin_lookup_result *result)
{
	...
	unpack_block_time(block_time, &exception_block, &exception_time, &zeroed);
...
static int __find_next_mapped_block(struct dm_thin_device *td, 
									dm_block_t block,
					  				dm_block_t *vblock,
					  				struct dm_thin_lookup_result *result)
{
...
int dm_thin_find_next_mapped_block(struct dm_thin_device *td, 
								   dm_block_t block,
                                   dm_block_t *vblock,
                                   struct dm_thin_lookup_result *result)
{
...
static int __remove_range(struct dm_thin_device *td, dm_block_t begin, dm_block_t end)
{
	...
	if (td->preallocate < td->mapped_blocks + td->on_the_fly)
		__free_pool_block(pmd, min((dm_block_t)total_count,
