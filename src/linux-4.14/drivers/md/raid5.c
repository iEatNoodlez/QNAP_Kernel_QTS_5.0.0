			      ...
			      struct list_head *temp_inactive_list)
{
	...
	if (test_bit(STRIPE_HANDLE, &sh->state)) {
		...
		if (test_and_clear_bit(STRIPE_PREREAD_ACTIVE, &sh->state)) {
			atomic64_inc(&sh->preread_dec);
			atomic64_inc(&conf->preread_dec);
			if (atomic_dec_return(&conf->preread_active_stripes)
			    < IO_THRESHOLD)
				md_wakeup_thread(conf->mddev->thread);
		}
		atomic_set(&sh->active, 0);
...
retry:
	...
	sh->state = 0;
	atomic_set(&sh->active, 0);
	atomic64_set(&sh->handle_cnt, 0);
	atomic64_set(&sh->preread_inc, 0);
	atomic64_set(&sh->preread_dec, 0);
	...
	struct stripe_head *sh, int no_skipcopy)
{
	...
	bio_for_each_segment(bvl, bio, iter) {
        ...
        int dma = bio->bi_ext_flags & (1 << BIO_EXT_DMA_ADDR);
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(TRXX)
                    if (dma)
                        tx = async_memcpy_phys(page_to_phys(*page), (phys_addr_t)bio_page, page_offset,
...
#if defined(CONFIG_MACH_QNAPTS) && defined(TRXX)
                if (dma)
                    tx = async_memcpy_phys((phys_addr_t)bio_page, page_to_phys(*page), b_offset,
			  ...
			  int forwrite, int previous)
{
	...
	*bip = bi;
	...
	if (conf->mddev->bitmap && firstwrite) {
		...
		atomic_inc(&conf->bitmap_wait_cnt);
		spin_unlock_irq(&sh->stripe_lock);
		bitmap_startwrite(conf->mddev->bitmap, sh->sector,
				  STRIPE_SECTORS, 0);
		spin_lock_irq(&sh->stripe_lock);
		atomic_dec(&conf->bitmap_wait_cnt);
	...
	for (i = disks; i--; ) {
		...
		if (test_bit(R5_ReadError, &sh->dev[i].flags)) {
			...
			if (rdev && is_rdev_in_sync(conf->mddev, rdev) &&
			    !is_rdev_faulty(conf->mddev, rdev))
				atomic_inc(&rdev->nr_pending);
			else
				rdev = NULL;
			rcu_read_unlock();
...
static void handle_stripe(struct stripe_head *sh)
{
	...
	if (atomic_read(&conf->show_log))
		pr_err("handling stripe %llu, state=%#lx cnt=%d, "
			"pd_idx=%d, qd_idx=%d\n, check:%d, reconstruct:%d\n",
		       (unsigned long long)sh->sector, sh->state,
		       atomic_read(&sh->count), sh->pd_idx, sh->qd_idx,
		       sh->check_state, sh->reconstruct_state);
	...
	if (atomic_read(&conf->show_log))
		pr_err("locked=%d uptodate=%d to_read=%d"
		       " to_write=%d failed=%d failed_num=%d,%d\n",
		       s.locked, s.uptodate, s.to_read, s.to_write, s.failed,
		       s.failed_num[0], s.failed_num[1]);
	...
	if (sh->reconstruct_state == reconstruct_state_result) {
		...
		if (sh_src && test_bit(STRIPE_EXPAND_SOURCE, &sh_src->state)) {
			...
			if (!test_and_set_bit(STRIPE_PREREAD_ACTIVE,
				...
				atomic64_inc(&sh_src->preread_inc);
				atomic64_inc(&conf->preread_inc);
			}
...
{
	int bmp = atomic_read(&conf->bitmap_wait_cnt);

	if (atomic_read(&conf->preread_active_stripes) < IO_THRESHOLD + bmp) {
...
static void raid5d(struct md_thread *thread)
{
	...
	while (1) {
		...
		if (set_ro) {
			...
			pr_info("md/raid5:%s: %s set raid5 QNAP readonly\n", mdname(mddev), __func__);
			spin_lock_irq(&conf->device_lock);
		}
...
static ssize_t
stripe_dump_store(struct mddev *mddev, const char *page, size_t len)
{
	int hash = 0, d = 0;
	unsigned long state;
	struct r5conf *conf = mddev->private;
	...
	if (kstrtoul(page, 10, &state))
		return -EINVAL;

	pr_err("%s: show stripes with state %lx\n", __func__, state);
	pr_err("%s: preread_active_stripes %d\n", __func__, atomic_read(&conf->preread_active_stripes));
	pr_err("%s: preread_inc: %lu\n", __func__, atomic64_read(&conf->preread_inc));
	pr_err("%s: preread_dec: %lu\n", __func__, atomic64_read(&conf->preread_dec));
	for (hash = 0; hash < NR_HASH; hash++) {
		struct stripe_head *sh;
		struct stripe_head_state st;
		int lock = (hash & STRIPE_HASH_LOCKS_MASK);

		lock_device_hash_lock(conf, lock);
		hlist_for_each_entry(sh, &conf->stripe_hashtbl[hash], hash) {

			spin_lock_irq(&sh->stripe_lock);
			if (!atomic_read(&sh->active)) {
				spin_unlock_irq(&sh->stripe_lock);
				continue;
			}

			if ((sh->state & state) != state) {
				spin_unlock_irq(&sh->stripe_lock);
				continue;
			}

			active_cnt++;
			analyse_stripe(sh, &st);

			pr_err("=== STRIPE [%llu] ===\n", (unsigned long long)sh->sector);
			pr_err("state: %lx\n", sh->state);
			pr_err("count: %u\n", atomic_read(&sh->count));
			pr_err("handle_cnt: %lu\n", atomic64_read(&sh->handle_cnt));
			pr_err("preread_inc: %lu\n", atomic64_read(&sh->preread_inc));
			pr_err("preread_dec: %lu\n", atomic64_read(&sh->preread_dec));
			pr_err("pd_idx: %d, qd_idx: %d\n", sh->pd_idx, sh->qd_idx);
			pr_err("check_state: %d\n", sh->check_state);
			pr_err("reconstruct_state: %d\n", sh->reconstruct_state);
			pr_err("batch head is%s NULL\n", sh->batch_head ? " not" : "");
			pr_err("stripe_head state: \n");
			pr_err("	syncing: %d, expanding: %d, expanded: %d, replacing: %d\n", st.syncing, st.expanding, st.expanded, st.replacing);
			pr_err("	locked: %d, uptodate: %d, to_read: %d, to_write: %d, failed: %d, written: %d\n", st.locked, st.uptodate, st.to_read, st.to_write, st.failed, st.written);
			pr_err("	to_fill: %d, compute: %d, req_compute: %d, non_overwrite: %d\n", st.to_fill, st.compute, st.req_compute, st.non_overwrite);
			pr_err("	failed_num[0]: %d, failed_num[1]: %d, p_failed: %d, q_failed: %d\n", st.failed_num[0], st.failed_num[1], st.p_failed, st.q_failed);
			pr_err("	ops_request: %lu\n", st.ops_request);
			pr_err("device flags: \n");
			for (d = 0; d < sh->disks; d++)
				pr_err("	device[%2d]-flags: %lx, toread: %d, read: %d, towrite: %d, written: %d\n",
					d, sh->dev[d].flags, !!sh->dev[d].toread, !!sh->dev[d].read, !!sh->dev[d].towrite, !!sh->dev[d].written);
			spin_unlock_irq(&sh->stripe_lock);

		}
		unlock_device_hash_lock(conf, lock);

	}
	pr_err("%s: finished, got %d active stripes\n", __func__, active_cnt);
	return len;
}

static struct md_sysfs_entry
raid5_stripe_dump = __ATTR_WO(stripe_dump);

static ssize_t
show_log_store(struct mddev *mddev, const char *page, size_t len)
{
	int in;
	struct r5conf *conf = mddev->private;
	...
	if (kstrtoint(page, 10, &in))
		return -EINVAL;

	atomic_set(&conf->show_log, in);

	return len;
}

static struct md_sysfs_entry
raid5_show_log = __ATTR_WO(show_log);
...
static struct r5conf *setup_conf(struct mddev *mddev)
{
	...
	atomic_set(&conf->show_log, 0);
	atomic64_set(&conf->preread_inc, 0);
	atomic64_set(&conf->preread_dec, 0);
	atomic_set(&conf->bitmap_wait_cnt, 0);
