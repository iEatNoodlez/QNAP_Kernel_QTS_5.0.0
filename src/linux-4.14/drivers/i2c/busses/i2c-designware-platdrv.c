...
static int dw_i2c_plat_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		if (!device_property_read_u32(&pdev->dev, "i2c-ss-scl-hcnt-raw", &val))
			dev->ss_hcnt = val;
		if (!device_property_read_u32(&pdev->dev, "i2c-ss-scl-lcnt-raw", &val))
			dev->ss_lcnt = val;
		if (!device_property_read_u32(&pdev->dev, "i2c-fs-scl-hcnt-raw", &val))
			dev->fs_hcnt = val;
		if (!device_property_read_u32(&pdev->dev, "i2c-fs-scl-lcnt-raw", &val))
			dev->fs_lcnt = val;
