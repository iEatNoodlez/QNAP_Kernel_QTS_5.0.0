...
#if defined(CONFIG_RTC_NVMEM) && defined(CONFIG_MACH_QNAPTS)
...
static struct nvmem_device *devm_nvmem_register(struct device *dev,
...
static void rtc_nvmem_unregister(struct rtc_device *rtc)
{
...
static int rtc_nvmem_register(struct rtc_device *rtc,
...
#ifdef CONFIG_MACH_QNAPTS
    case rx_8130:
		dev_dbg(dev, "== QC: bf set wday = 0x%x from tm_wday = %d\n", regs[DS1307_REG_WDAY], t->tm_wday);
		regs[DS1307_REG_WDAY] = 0x1 << (t->tm_wday%7);
