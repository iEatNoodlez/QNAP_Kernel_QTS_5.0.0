...
qla2x00_set_isp_flags(struct qla_hw_data *ha)
{
	...
	case PCI_DEVICE_ID_QNAP_QNA32G2FC:
	case PCI_DEVICE_ID_QNAP_QNA16G2FC:
		ha->isp_type |= DT_ISP2261;
		ha->device_type |= DT_ZIO_SUPPORTED;
		ha->device_type |= DT_FWI2;
		ha->device_type |= DT_IIDMA;
...
qla2x00_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (pdev->device == PCI_DEVICE_ID_QLOGIC_ISP2422 ||
	    ...
	    pdev->device == PCI_DEVICE_ID_QNAP_QNA32G2FC ||
	    pdev->device == PCI_DEVICE_ID_QNAP_QNA16G2FC) {
...
#if defined(CONFIG_MACH_QNAPTS)
	{ PCI_DEVICE(PCI_VENDOR_ID_QLOGIC, PCI_DEVICE_ID_QNAP_QNA32G2FC) },
	{ PCI_DEVICE(PCI_VENDOR_ID_QLOGIC, PCI_DEVICE_ID_QNAP_QNA16G2FC) },
#endif
	{ 0 },
};
