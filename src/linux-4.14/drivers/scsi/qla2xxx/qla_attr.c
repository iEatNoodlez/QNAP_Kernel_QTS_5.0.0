...
#if defined(CONFIG_MACH_QNAPTS)
	if (ha->pdev->device == PCI_DEVICE_ID_QNAP_QNA32G2FC || ha->pdev->device == PCI_DEVICE_ID_QNAP_QNA16G2FC)
		strncpy(fc_host_manufacturer(vha->host), "QNAP", strlen("QNAP"));
	else
...
#if defined(CONFIG_MACH_QNAPTS)
		if (ha->pdev->device == PCI_DEVICE_ID_QNAP_QNA16G2FC)
			speed = FC_PORTSPEED_16GBIT | FC_PORTSPEED_8GBIT;
		else
#endif
