		       ...
		       int disposition)
{
	...
	off += scnprintf(logbuf + off, logbuf_len - off,
				 "result=%x sbuf[0,1,2,3,7,12,13]=%x_%x_%x_%x_%x_%x_%x blk_rq_bytes=%x passthrough=%x cmd_flags=%x jiffies_alloc=%x allowed=%x timeout=%x jiffies=%x",
				cmd->result, cmd->sense_buffer[0], cmd->sense_buffer[1], cmd->sense_buffer[2], cmd->sense_buffer[3],
