			...
			unsigned char *cmnd, int cmnd_size, unsigned sense_bytes)
{
	...
	if (sense_bytes) {
		...
		if (status_byte(ses->result) == CHECK_CONDITION && SCSI_SENSE_VALID(scmd)) {
			memcpy(ses->sense_buffer, scmd->sense_buffer, SCSI_SENSE_BUFFERSIZE);
		}
#endif
	}
...
#ifdef CONFIG_MACH_QNAPTS
	if (status_byte(ses->result) == CHECK_CONDITION && SCSI_SENSE_VALID(ses)) {
		memcpy(scmd->sense_buffer, ses->sense_buffer, SCSI_SENSE_BUFFERSIZE);
	}
#endif
}
