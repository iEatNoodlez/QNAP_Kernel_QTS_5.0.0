...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE)
	int rc = ksocket_send(sock, NULL, buf, strlen(buf)+1);
#elif defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_MVEBU)
	int rc = ksocket_send(sock, (struct sockaddr_in *)&addr_send, buf, strlen(buf)+1);
#else
