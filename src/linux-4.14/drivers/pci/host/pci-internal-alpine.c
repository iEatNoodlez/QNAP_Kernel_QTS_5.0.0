...
MODULE_DEVICE_TABLE(of, al_pcie_of_match);
...
static int al_pcie_probe(struct platform_device *pdev)
{
	...
	err = devm_of_pci_get_host_bridge_resources(dev, 0, 0xff, &pci_resources, NULL);
	if (err)
		return err;
...
static int al_pcie_remove(struct platform_device *pdev)
{
	return 0;
}
