...
#ifdef CONFIG_MACH_QNAPTS
static void __qnap_sbc_execute_unmap(
	struct work_struct *work
	)
{
	int ret;
	struct qnap_se_cmd_dr *dr = container_of(work, struct qnap_se_cmd_dr, unmap_work);
...
static sense_reason_t qnap_sbc_execute_unmap(
	struct se_cmd *cmd
	)
{
	...
	{
		if (se_dev->dev_dr.unmap_wq) {
			INIT_WORK(&cmd->cmd_dr.unmap_work, __qnap_sbc_execute_unmap);
			queue_work(se_dev->dev_dr.unmap_wq,  &cmd->cmd_dr.unmap_work);
			return TCM_NO_SENSE;
		}
	}
...
sbc_execute_rw(struct se_cmd *cmd)
{
	...
	&& scsi_write_6_10_12_16(cmd->t_task_cdb)
	)
	{
		struct target_iscsi_zmc_data *p = &cmd->zmc_data.u.izmc;
		if (atomic_read(&p->write_sg_inuse) == ZMC_W_SG_INUSE_VALID) {
			/* we prepared wsgl in __target_execute_cmd()already */
			return ops->execute_rw(cmd, p->wsgl, 
				atomic_read(&p->wsgl_nents), cmd->data_direction);
		} 
		else if (atomic_read(&p->write_sg_inuse) == ZMC_W_SG_INUSE_FREED) 
			return TCM_OUT_OF_RESOURCES;

		/* code shall never come here */
		BUG_ON(true);
	}
#endif
						 ...
						 int *post_ret)
{
	...
	if (qnap_transport_check_iscsi_fabric(cmd)) {
		struct target_iscsi_zmc_data *p = &cmd->zmc_data.u.izmc;
		return p->scsi_ops->ats_callback(cmd, success, post_ret);
	}
#endif		
...
sbc_parse_cdb(struct se_cmd *cmd, struct sbc_ops *ops)
{
	...
	if (qnap_target_perf_enabled())
		qnap_target_perf_set_run_complete_ok_flag(cmd);
#endif
#ifdef QNAP_TARGET_ZMC_FEATURE	
	qnap_target_zmc_prep_backend_op(cmd);
#endif
#endif
	switch (cdb[0]) {
	case READ_6:
	...
	case SERVICE_ACTION_IN_16:
		...
		case SAI_GET_LBA_STATUS:
			if (ops->execute_qnap_get_lba_status)
				cmd->execute_cmd = ops->execute_qnap_get_lba_status;
			break;
#endif
#endif
		default:
	...
	case UNMAP:
		...
		if (cmd->se_dev->dev_dr.unmap_wq)
			cmd->execute_cmd = qnap_sbc_execute_unmap;
		else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		if (ops->execute_qnap_unmap)
			ret = ops->execute_qnap_unmap(cmd, lba, range);
		else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		/* check cmd was aborted or dropped */
		if (test_bit(QNAP_CMD_T_RELEASE_FROM_CONN, &cmd->cmd_dr.cmd_t_state)
		|| atomic_read(&cmd->cmd_dr.tmr_code)
		)
			goto err;			
#endif
	       ...
	       unsigned int ei_lba, struct scatterlist *psg, int psg_off)
{
	...
	cmd->b_se_state |= BS_SE_CMD_SBC_DIF_VERIFY;
	cmd->b_se_state_last = BS_SE_CMD_SBC_DIF_VERIFY;
