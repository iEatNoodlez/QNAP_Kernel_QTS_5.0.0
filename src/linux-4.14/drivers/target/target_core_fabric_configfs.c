...
/* Start of tfc_tpg_redirect_base_cit */

static void target_fabric_redirect_base_release(struct config_item *item)
{
	struct se_tpg_redirect *se_tpg_redirect = container_of(to_config_group(item),
			struct se_tpg_redirect, tpg_redirect_group);
	struct se_portal_group *se_tpg = se_tpg_redirect->tpg_redirect_parent;
	struct target_fabric_configfs *tf = se_tpg->se_tpg_wwn->wwn_tf;

	tf->tf_ops->fabric_drop_redirect(se_tpg_redirect, item->ci_namebuf);
}

static struct configfs_item_operations target_fabric_redirect_base_item_ops = {
	.release	= target_fabric_redirect_base_release,
};

TF_CIT_SETUP_DRV(tpg_redirect_base, &target_fabric_redirect_base_item_ops, NULL);

/* End of tfc_tpg_redirect_base_cit */

/* Start of tfc_tpg_redirect_cit */

static struct config_group *target_fabric_make_redirect(
	struct config_group *group,
	const char *name)
{
	struct se_portal_group *se_tpg = container_of(group,
			struct se_portal_group, tpg_redirect_group);
	struct target_fabric_configfs *tf = se_tpg->se_tpg_wwn->wwn_tf;
	struct se_tpg_redirect *se_tpg_redirect;

	if (!tf->tf_ops->fabric_make_redirect) {
		pr_err("tf->tf_ops.fabric_make_redirect is NULL\n");
		return ERR_PTR(-ENOSYS);
	}

	se_tpg_redirect = tf->tf_ops->fabric_make_redirect(se_tpg, group, name);
	if (!se_tpg_redirect || IS_ERR(se_tpg_redirect))
		return ERR_PTR(-EINVAL);

	se_tpg_redirect->tpg_redirect_parent = se_tpg;
	config_group_init_type_name(&se_tpg_redirect->tpg_redirect_group, name,
			&tf->tf_tpg_redirect_base_cit);

	return &se_tpg_redirect->tpg_redirect_group;
}

static void target_fabric_drop_redirect(
	struct config_group *group,
	struct config_item *item)
{
	/*
	 * struct se_tpg_redirect is released via target_fabric_redirect_base_release()
	 */
	config_item_put(item);
}

#define MAX_REDIRECT_ALGO_LEN    16

static ssize_t target_fabric_redirect_algo_show(
	struct se_portal_group *se_portal_group,
	char *page)
{
	char *get_algo = NULL;
	ssize_t rb = 0;
	struct target_fabric_configfs *tf = se_portal_group->se_tpg_wwn->wwn_tf;

	get_algo = kzalloc(MAX_REDIRECT_ALGO_LEN, GFP_KERNEL);
	if (!get_algo)
		return -ENOMEM;
	if (!tf->tf_ops->fabric_get_redirect_algo) {
		pr_err("tf->tf_ops.fabric_set_redirect_algo is NULL\n");
		return -ENOSYS;
	}

	tf->tf_ops->fabric_get_redirect_algo(se_portal_group, &get_algo);
	rb = snprintf(page, PAGE_SIZE, "iSCSI Login Redirect Algorithm: %s\n", get_algo);
	kfree(get_algo);

	return rb;
}

static ssize_t target_fabric_redirect_algo_store(
	struct se_portal_group *se_portal_group,
	const char *page,
	size_t count)
{
	...
	ret = kstrtou8(page, 0, &set_algo);
	if (ret) {
		pr_err("kstrto8() ret:%d, page:%s set_algo:%d\n",
				ret, page, set_algo);
		return ret;
	}
	if (!tf->tf_ops->fabric_set_redirect_algo) {
		pr_err("tf->tf_ops.fabric_set_redirect_algo is NULL\n");
		return -ENOSYS;
	}

	ret = tf->tf_ops->fabric_set_redirect_algo(se_portal_group, set_algo);
	if (ret) {
		return ret;
	}

	pr_debug("%s_ConfigFS: Changed iSCSI Login Redirect Algorithm: %u\n",
			se_portal_group->se_tpg_tfo->get_fabric_name(),
			set_algo);

	return count;
}

CONFIGFS_ATTR(target_fabric_redirect_, algo);

static struct configfs_attribute *target_fabric_redirect_attrs[] = {
	&target_fabric_redirect_attr_algo,
	NULL,
};

static struct configfs_group_operations target_fabric_redirect_group_ops = {
	.make_group = &target_fabric_make_redirect,
	.drop_item  = &target_fabric_drop_redirect,
};

static struct configfs_item_operations target_fabric_redirect_item_ops = {
};

TF_CIT_SETUP(tpg_redirect,
		&target_fabric_redirect_item_ops,
		&target_fabric_redirect_group_ops,
		target_fabric_redirect_attrs);

/* End of tfc_tpg_redirect_cit */
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(ISCSI_LOGIN_REDIRECT)
	config_group_init_type_name(&se_tpg->tpg_redirect_group, "redirect",
			&tf->tf_tpg_redirect_cit);
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(ISCSI_LOGIN_REDIRECT)
	target_fabric_setup_tpg_redirect_cit(tf);
	target_fabric_setup_tpg_redirect_base_cit(tf);
#endif
