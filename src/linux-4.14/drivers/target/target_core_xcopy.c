...
#include "qnap/target_core_qfbc.h"
#endif
#endif
...
static int target_xcopy_gen_naa_ieee(struct se_device *dev, unsigned char *buf)
{
	...
	qlib_get_naa_6h_code((void *)dev, &dr->dev_naa[0], buf, 
				...
				unsigned short tdll, sense_reason_t *sense_ret)
{
	...
	*sense_ret = TCM_INVALID_PARAMETER_LIST;
	...
	if (xop->src_dev->transport->get_device_type(xop->src_dev) != sdev_type) {
		pr_warn("%s: dev type of src dev is not same as Copy Source "
				"Descriptor data\n", __func__);
		*sense_ret =  TCM_INCORRECT_COPY_TARGET_DEV_TYPE;
		goto out;
	}

	if (xop->dst_dev->transport->get_device_type(xop->dst_dev) != ddev_type) {
		pr_warn("%s: dev type of dst dev is not same as Copy "
...
#ifdef SUPPORT_FAST_BLOCK_CLONE
	rc = qnap_fbc_do_fast_block_clone(xop);
	if (rc != -1) {
		...
		if (rc == 0) {
			...
			ec_cmd->sense_reason = rc;
			goto out;
		}
	} else
		pr_debug("XCOPY: do not fast block clone\n");
#endif
	src_dev = xop->src_dev;
	dst_dev = xop->dst_dev;
	src_lba = xop->src_lba;
