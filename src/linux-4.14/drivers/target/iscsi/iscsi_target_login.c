...
static struct iscsi_login *iscsi_login_init_conn(struct iscsi_conn *conn)
{
	...
	/* only setup ops if trnasport type is ISCSI_TCP */	
	if ((conn->conn_transport->transport_type != ISCSI_TCP)
	|| qnap_iscsit_tcp_zmc_check_ops()
	)
		goto skip_setup_zmc;

	if (qnap_iscsit_zmc_create_obj2cmd_cache())
		goto skip_setup_zmc;

	if (qnap_iscsit_tcp_zmc_alloc_priv(conn)) {
		qnap_iscsit_zmc_destory_obj2cmd_cache();
		goto skip_setup_zmc;
	}

	qnap_iscsit_tcp_zmc_setup_conn_ops_on_conn(conn);
...
#ifdef CONFIG_MACH_QNAPTS
	atomic_set(&sess->sess_now_release, 0);
#endif
	...
	u8 zero_tsih)
{
	...
	if (!zero_tsih) {
		...
		if (conn->zmc_conn_ops)
			conn->zmc_conn_ops->set_sock_callbacks(conn);
#endif
#endif
		/*
...
new_sess_out:
	...
	if (conn->sess) {
		char login_ip[IPV6_ADDRESS_SPACE] = "";
		snprintf(login_ip, sizeof(login_ip), "%pISc", &conn->login_sockaddr);
		qnap_nl_send_post_log(LOGIN_FAIL, LOG_ERROR, conn->sess, login_ip);
	}
#endif
	new_sess = true;
old_sess_out:
