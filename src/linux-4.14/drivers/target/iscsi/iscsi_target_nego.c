...
get_target:
	...
	if (!tiqn) {
		pr_err("Unable to locate Target IQN: %s in"
			" Storage Node from %pISc\n", t_buf, &conn->login_sockaddr);
		iscsit_tx_login_rsp(conn, ISCSI_STATUS_CLS_TARGET_ERR,
				ISCSI_LOGIN_STATUS_SVC_UNAVAILABLE);
		ret = -1;
		goto out;
...
/* Jonathan Ho, 20140416, one target can be logged in from only one initiator IQN */
	...
	/* iSCSI login redirection check */
	if (sessiontype &&
			!strncmp(t_buf, tiqn->tiqn, ISCSI_IQN_LEN) &&
			conn->tpg->tpg_redirect.algo != ISCSI_LOGIN_REDIRECT_ALGO_NONE &&
			!list_empty_careful(&conn->tpg->tpg_redirect.tpg_redirect_list)) {
		kref_put(&tpg_np->tpg_np_kref, iscsit_login_kref_put);
		iscsit_put_tiqn_for_login(tiqn);
		/* We will assign status_detail in iscsit_tx_login_rsp_redirect() */
		iscsit_tx_login_rsp(conn, ISCSI_STATUS_CLS_REDIRECT, 0);
		conn->tpg = NULL;
		ret = -1;
		goto out;
	}
