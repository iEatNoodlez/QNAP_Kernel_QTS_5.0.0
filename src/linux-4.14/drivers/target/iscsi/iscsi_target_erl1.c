...
int iscsit_execute_cmd(struct iscsi_cmd *cmd, int ooo)
{
	...
	if(cmd){
		cmd->b_state |= BS_EXEC_ISCSI_CMD;
		cmd->b_state_last = BS_EXEC_ISCSI_CMD;
	}
		...
		if (cmd->immediate_data) {
			if (cmd->cmd_flags & ICF_GOT_LAST_DATAOUT) {
				...
				if (qnap_target_perf_enabled())
					__ret = qnap_transport_perf_exec_wt_cmd(&cmd->se_cmd);
				else
#endif
					__ret = qnap_transport_exec_wt_cmd(&cmd->se_cmd);
...
static void iscsit_handle_dataout_timeout(unsigned long data)
{
	...
	if ((atomic_read(&se_cmd->cmd_dr.tmr_code) == QNAP_TMR_CODE_LUN_RESET)
	|| (atomic_read(&se_cmd->cmd_dr.tmr_code) == QNAP_TMR_CODE_ABORT_TASK)
	)
	{
		pr_debug("Dataout timeout timer triggered but cmd "
			"was aborted already, skip to disconnect iscsi conn\n"); 
		spin_unlock_bh(&cmd->dataout_timeout_lock);
		iscsit_dec_conn_usage_count(conn);
		return;
	}
