...
#include "qnap/iscsi_target_qtransport.h"
#include "../qnap/target_core_qtransport.h"
#endif

#if defined(CONFIG_MACH_QNAPTS) && defined(ISCSI_LOGIN_REDIRECT)
/* Start items for lio_target_redirect_base_cit */

static ssize_t lio_target_redirect_enable_show(
	struct se_tpg_redirect *se_tpg_redirect,
	char *page)
{
	struct iscsi_portal_group *tpg = NULL;
	struct iscsi_tpg_redirect_group *tpg_redirect_group = container_of(
			se_tpg_redirect, struct iscsi_tpg_redirect_group, se_tpg_redirect);
	...
	rb = sepstrtoipport(
			&se_tpg_redirect->tpg_redirect_group.cg_item.ci_namebuf[0],
			&ip[0], &port);
	if (rb < 0) {
		pr_err("Can not separate str into ip and port\n");
		return rb;
	}

	tpg = tpg_redirect_group->tpg;
	if (iscsit_get_tpg(tpg) < 0)
		return -EINVAL;

	list_for_each_entry(
			redirect, &tpg->tpg_redirect.tpg_redirect_list, redirect_list) {
...
static ssize_t lio_target_redirect_enable_store(
	struct se_tpg_redirect *se_tpg_redirect,
	const char *page,
	size_t count)
{
	...
	rc = strtobool(page, &op);
	if (rc < 0) {
		pr_err("Illegal value for redirect enable: %s\n", page);
		return -EINVAL;
	}
...
CONFIGFS_ATTR(lio_target_redirect_, enable);

static ssize_t lio_target_redirect_type_show(
	struct se_tpg_redirect *se_tpg_redirect,
	char *page)
{
	struct iscsi_portal_group *tpg = NULL;
	...
	list_for_each_entry(
		...
		if (!strncmp(redirect->ip, ip, IPV6_ADDRESS_SPACE) &&
				redirect->port == port) {
			if (redirect->type == ISCSI_LOGIN_STATUS_TGT_MOVED_TEMP)
				rb = sprintf(page, "temp\n");
			else if (redirect->type == ISCSI_LOGIN_STATUS_TGT_MOVED_PERM)
				rb = sprintf(page, "perm\n");
			else
				rb = sprintf(page, "type does not be set\n");
		}
	}
...
static ssize_t lio_target_redirect_type_store(
	struct se_tpg_redirect *se_tpg_redirect,
	const char *page,
	size_t count)
{
	...
	if (!strncmp(page, "temp", 4)) {
		op = ISCSI_LOGIN_STATUS_TGT_MOVED_TEMP;
	} else if (!strncmp(page, "perm", 4)) {
		op = ISCSI_LOGIN_STATUS_TGT_MOVED_PERM;
	} else {
		pr_err("Illegal value for redirect type: %s\n", page);
		return -EINVAL;
	}
...
CONFIGFS_ATTR(lio_target_redirect_, type);

static struct configfs_attribute *lio_target_redirect_base_attrs[] = {
	&lio_target_redirect_attr_enable,
	&lio_target_redirect_attr_type,
	NULL,
};

/* Stop items for lio_target_redirect_base_cit */

/* Start items for lio_target_redirect_cit */

static struct se_tpg_redirect *lio_target_call_addredirecttotpg(
	struct se_portal_group *se_tpg,
	struct config_group *group,
	const char *name)
{
	struct iscsi_portal_group *tpg = NULL;
	struct iscsi_tpg_redirect_group *tpg_redirect_group = NULL;
	struct iscsi_redirect *redirect = NULL;
	char ip[IPV6_ADDRESS_SPACE];
	u16 port = 0;
	int ret = 0;

	if (strlen(name) > MAX_REDIRECT_IPPORT_LEN) {
		pr_err("strlen(name): %d exceeds MAX_REDIRECT_IPPORT_LEN: %d\n",
				(int)strlen(name), MAX_REDIRECT_IPPORT_LEN);
		ret = -EOVERFLOW;
		goto out;
	}
	memset(ip, 0, IPV6_ADDRESS_SPACE);
	ret = sepstrtoipport(name, &ip[0], &port);
	if (ret < 0) {
		pr_err("Can not separate str into ip and port\n");
		goto out;
	}
	...
	/* create a configfs group */
	tpg_redirect_group = kzalloc(
			sizeof(struct iscsi_tpg_redirect_group), GFP_KERNEL);
	if (!tpg_redirect_group) {
		pr_err("Unable to allocate memory for "
				"struct iscsi_tpg_redirect_group\n");
		ret = -ENOMEM;
		goto out_tpg;
	}
	tpg_redirect_group->tpg = tpg;

	/* Create a struct iscsi_redirect and add it to the linked list */
	redirect = kzalloc(sizeof(struct iscsi_redirect), GFP_KERNEL);
	if (!redirect) {
		pr_err("Unable to allocate memory for struct iscsi_redirect\n");
		ret = -ENOMEM;
		goto out_tpg_redirect_group;
	}
	redirect->enable = false;
	snprintf(redirect->ip, sizeof(redirect->ip), "%s", ip);
	redirect->port = port;
	redirect->type = 0;
	redirect->cnt  = 0;
	if (list_empty_careful(&tpg->tpg_redirect.tpg_redirect_list)) {
		redirect->algo_rr_picked = true;
	} else {
		redirect->algo_rr_picked = false;
	}
	list_add_tail(
			&redirect->redirect_list, &tpg->tpg_redirect.tpg_redirect_list);

	iscsit_put_tpg(tpg);
	return &tpg_redirect_group->se_tpg_redirect;
out_tpg_redirect_group:
	kfree(tpg_redirect_group);
out_tpg:
	iscsit_put_tpg(tpg);
out:
	return ERR_CAST(ERR_PTR(ret));
}

static void lio_target_call_delredirectfromtpg(
	struct se_tpg_redirect *se_tpg_redirect,
	const char *name)
{
	struct iscsi_portal_group *tpg = NULL;
	struct iscsi_tpg_redirect_group *tpg_redirect_group = NULL;
	struct iscsi_redirect *redirect = NULL, *redirect_tmp = NULL;
	...
	tpg_redirect_group = container_of(
			se_tpg_redirect, struct iscsi_tpg_redirect_group, se_tpg_redirect);
	tpg = tpg_redirect_group->tpg;
	...
	list_for_each_entry_safe(
			redirect, redirect_tmp,
			&tpg->tpg_redirect.tpg_redirect_list, redirect_list) {
		if (!strncmp(redirect->ip, ip, IPV6_ADDRESS_SPACE) &&
				redirect->port == port) {
			list_del(&redirect->redirect_list);
			kfree(redirect);
		}
	}

	iscsit_put_tpg(tpg);
	tpg_redirect_group->tpg = NULL;
	kfree(tpg_redirect_group);
}

/* End items for lio_target_redirect_cit */

/* Start attributes for tpg_redirect_group */

static int lio_target_call_setredirectalgototpg(
	struct se_portal_group *se_tpg,
	const u8 set_algo)
{
	struct iscsi_portal_group *tpg = NULL;
	...
	if (set_algo == ISCSI_LOGIN_REDIRECT_ALGO_RR)
		tpg->tpg_redirect.algo = ISCSI_LOGIN_REDIRECT_ALGO_RR;
	else
		tpg->tpg_redirect.algo = ISCSI_LOGIN_REDIRECT_ALGO_NONE;

	iscsit_put_tpg(tpg);
	return ret;
}

#define MAX_REDIRECT_ALGO_LEN    256

static void lio_target_call_getredirectalgofromtpg(
	struct se_portal_group *se_tpg,
	char **get_algo)
{
	struct iscsi_portal_group *tpg = NULL;
	...
	if (tpg->tpg_redirect.algo == ISCSI_LOGIN_REDIRECT_ALGO_RR)
		snprintf(*get_algo, MAX_REDIRECT_ALGO_LEN, "%s", "RR");
	else
		snprintf(*get_algo,MAX_REDIRECT_ALGO_LEN,  "%s", "NONE");

	iscsit_put_tpg(tpg);
}

/* End attributes for tpg_redirect_group */
#endif
...
#define TMP_TPG_CMD_DUMP_FILE_PATH	"/tmp/iscsi_cmd_dump_%s"
static ssize_t lio_dump_iscsi_target_cmd (struct se_portal_group *se_tpg, char *page)
{
	uint16_t max_page_size = 4096; /* page size */
	ssize_t len = 0;

	struct iscsi_portal_group* iscsi_portal = NULL;
	char *endpoint_wwn = NULL;
	...
	iscsi_portal = container_of(se_tpg, struct iscsi_portal_group, tpg_se_tpg);
	endpoint_wwn = iscsi_portal->tpg_tiqn->tiqn;

	snprintf(file_path, sizeof(file_path), TMP_TPG_CMD_DUMP_FILE_PATH, endpoint_wwn);

	data_buf = vmalloc(PAGE_SIZE * 10);
	if (!data_buf) {
		pr_err("%s: data buf allocated failed\n", __func__);
		len += snprintf(page + len, max_page_size - len, "data buf allocated failed");
		return len + 1;
	}
	...
	list_for_each_entry(se_sess, &se_tpg->tpg_sess_list, sess_list) {
		...
		data_size += sprintf(data_buf + data_size, "initiator:%s\n", se_sess->se_node_acl->initiatorname);

		sess = (struct iscsi_session *)se_sess->fabric_sess_ptr;
		data_size += sprintf(data_buf + data_size, "   session:%p\n", sess);

		spin_lock_bh(&sess->conn_lock);
		list_for_each_entry(conn, &sess->sess_conn_list, conn_list) {
			
			cmd_cnt = 0;
			data_size += sprintf(data_buf + data_size, "   connection:%p, stat_sn:0x%x, exp_statsn:0x%x\n", 
				conn, conn->stat_sn, conn->exp_statsn);

			spin_lock_bh(&conn->cmd_lock);
			list_for_each_entry_safe(cmd, cmd_p, &conn->conn_cmd_list, i_conn_node) {
				
				cmd_cnt++;
				if(cmd->complete_jiffies)
					tmp_jiffies = cmd->complete_jiffies;
				else
					tmp_jiffies = jiffies;
				data_size += sprintf(data_buf + data_size, 
					"      iscsi(%d):%p, op:0x%x, cmd_sn:0x%x, stat_sn:0x%x, exp_stat_sn:0x%x, "
					"r2t_sn:0x%x, b_state:0x%x,0x%x, time:%ld (%ld-%ld)\n", 
					cmd_cnt, cmd, cmd->iscsi_opcode, cmd->cmd_sn, cmd->stat_sn, cmd->exp_stat_sn, 
					cmd->r2t_sn, cmd->b_state, cmd->b_state_last,
					(tmp_jiffies - cmd->start_jiffies), cmd->start_jiffies, cmd->complete_jiffies);

				se_cmd = &cmd->se_cmd;
				if(se_cmd) {
					if(se_cmd->complete_jiffies)
						tmp_jiffies = se_cmd->complete_jiffies;
					else
						tmp_jiffies = jiffies;
					data_size += sprintf(data_buf + data_size, 
						"	 se_cmd:%p, ITT:0x%08x, i_s:%d, t_s:%d, transport_s:0x%x, "
						"op:0x%x, b_se_state:0x%llx,0x%llx, time:%ld (%ld-%ld)\n", 
						se_cmd, be32_to_cpu(se_cmd->se_tfo->get_task_tag(se_cmd)), se_cmd->se_tfo->get_cmd_state(se_cmd), 
						se_cmd->t_state, se_cmd->transport_state, se_cmd->t_task_cdb[0],
						se_cmd->b_se_state, se_cmd->b_se_state_last,
						(tmp_jiffies - se_cmd->start_jiffies), se_cmd->start_jiffies, se_cmd->complete_jiffies);
				}
			}
	...
	/* open file */
	fd = filp_open(file_path, O_RDWR | O_CREAT | O_DSYNC, S_IRWXU);
	if (IS_ERR(fd)) {
		pr_err("%s: file path error:%s\n", __func__, file_path);
		len += snprintf(page + len, max_page_size - len, "Failed to create file path: %s", file_path);
		return len + 1;
	}
	...
	if(ret < 0) {
		...
		len += snprintf(page + len, max_page_size - len, "%s: vfs_iter_write() error. ret:%d\n", __func__, ret);
		return len + 1;
	}
	...
	len += snprintf(page + len, max_page_size - len, "Dump complete, please check file: %s, len:%ld\n", file_path, data_size);
	return len + 1;
}


static ssize_t lio_target_tpg_cmd_dump_show(struct config_item *item,
	char *page)
{
	return lio_dump_iscsi_target_cmd(to_tpg(item), page);
}
...
const struct target_core_fabric_ops iscsi_ops = {
	...
	.get_task_tag			= qanp_iscsi_get_task_tag,
	.get_login_ip			= qnap_iscsi_get_login_ip,
	.get_cmdsn			= qnap_iscsi_get_cmdsn,	
	.set_delay_remove		= qnap_iscsi_set_delay_remove,	
#ifdef ISCSI_D4_INITIATOR
	.copy_node_attributes		= qnap_lio_copy_node_attributes,
#endif
#ifdef ISCSI_LOGIN_REDIRECT
	.fabric_set_redirect_algo	= lio_target_call_setredirectalgototpg,
	.fabric_get_redirect_algo	= lio_target_call_getredirectalgofromtpg,
	.fabric_make_redirect		= lio_target_call_addredirecttotpg,
	.fabric_drop_redirect		= lio_target_call_delredirectfromtpg,

	.tfc_tpg_redirect_base_attrs	= lio_target_redirect_base_attrs,
#endif
#endif
};
