...
#include "qnap/iscsi_target_qlog.h"
...
unsigned int iscsit_cpumask=0xffff;
			   ...
			   u32 buf_len)
{
	if (dr) {
		...
		if (cmd->zmc_cmd_ops) {
			...
			if (scsi_read_6_10_12_16(se_cmd->t_task_cdb)) {
				return cmd->zmc_cmd_ops->xmit_datain_pdu(conn, 
						cmd, buf);
			}
		}
#endif	
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
	/* we created obj2cmd cache in iscsi_login_init_conn() */
	qnap_iscsit_zmc_destory_obj2cmd_cache();
#endif
	...
	unsigned char *buf)
{
	...
	if(cmd){
		cmd->b_state |= BS_ADD_ISCSI_REJECT;
		cmd->b_state_last = BS_ADD_ISCSI_REJECT;
	} 
...
static int iscsit_allocate_iovecs(struct iscsi_cmd *cmd)
{
	...
	if(cmd){
		cmd->b_state |= BS_ALLOCATE_ISCSI_IOVECS;
		cmd->b_state_last = BS_ALLOCATE_ISCSI_IOVECS;
	} 
			  ...
			  unsigned char *buf)
{
	...
	int sam_task_attr;
	if(cmd){
		cmd->b_state |= BS_SETUP_ISCSI_CMD;
		cmd->b_state_last = BS_SETUP_ISCSI_CMD;
	} 
...
attach_cmd:
	spin_lock_bh(&conn->cmd_lock);
	if(cmd){
		cmd->b_state |= BS_ADD_ISCSI_CMD_TO_LIST;
		cmd->b_state_last = BS_ADD_ISCSI_CMD_TO_LIST;
	}
			    ...
			    struct iscsi_scsi_req *hdr)
{
	 ...
	 */
	if(cmd){
		cmd->b_state |= BS_PROCESS_SCSI_CMD;
		cmd->b_state_last = BS_PROCESS_SCSI_CMD;
	}
			  ...
			  bool dump_payload)
{
	...
	{
		if (check_ret == CMDSN_LOWER_THAN_EXP) {
			pr_debug("OOOOOOHHHH !! we got received cmdsn: 0x%08x, "
				"itt:0x%08x, is less than expcmdsn: 0x%08x, "
				"to ignore it\n", be32_to_cpu(hdr->cmdsn), 
				be32_to_cpu(cmd->init_task_tag),
				conn->sess->exp_cmd_sn);
		}
		
		/* if cmdsn is not we want, NOT get data from socket */
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
	if (cmd->zmc_cmd_ops) {
		immed_ret = cmd->zmc_cmd_ops->handle_immediate_data(cmd, hdr, 
					cmd->first_burst_len);
	} else
#endif
#endif
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
			if (cmd->zmc_cmd_ops) {
				rc = cmd->zmc_cmd_ops->dump_data_payload(cmd->conn,
 						cmd, cmd->first_burst_len, 1);
			} 
			else
#endif
#endif
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
		if (cmd->zmc_cmd_ops) {
			return cmd->zmc_cmd_ops->dump_data_payload(conn, cmd,
					payload_length, 1);
		}
#endif
#endif
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
			if (cmd->zmc_cmd_ops) {
				return cmd->zmc_cmd_ops->dump_data_payload(
						conn, cmd, payload_length, 1);
			}
#endif
#endif
			return iscsit_dump_data_payload(conn, payload_length, 1);
		   ...
		   struct iscsi_data *hdr)
{
	...
	cmd->se_cmd.cmd_dr.digest_zero_copy_skip = true;
	if (qnap_iscsit_zc_splice_work_on_scsi_op(&cmd->se_cmd))
		cmd->se_cmd.cmd_dr.digest_zero_copy_skip = false;
#endif
	if (conn->conn_ops->DataDigest) {
			     ...
			     bool data_crc_failed)
{
	...
	if ((rc == DATAOUT_NORMAL) || (rc == DATAOUT_WITHIN_COMMAND_RECOVERY))
		...
		if (qnap_target_perf_enabled())
			rc = qnap_transport_perf_exec_wt_cmd(&cmd->se_cmd);
		else
#endif
		rc = qnap_transport_exec_wt_cmd(&cmd->se_cmd);
		if (!rc)
			return 0;
#endif
...
static int iscsit_handle_data_out(struct iscsi_conn *conn, unsigned char *buf)
{
	...
	if(cmd){
		cmd->b_state |= BS_HANDLE_ISCSI_DATA_OUT;
		cmd->b_state_last = BS_HANDLE_ISCSI_DATA_OUT;
	}

#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
	if (cmd->zmc_cmd_ops)
		rc = cmd->zmc_cmd_ops->get_dataout(conn, cmd, hdr);
	else
#endif
#endif
			 ...
			 struct iscsi_nopout *hdr)
{
	...
	if(cmd){
		cmd->b_state |= BS_SETUP_ISCSI_NOP_OUT;
		cmd->b_state_last = BS_SETUP_ISCSI_NOP_OUT;
	} 
			   ...
			   unsigned char *buf)
{
	...
	if(cmd){
		cmd->b_state |= BS_HANDLE_ISCSI_TMC;
		cmd->b_state_last = BS_HANDLE_ISCSI_TMC;
	} 
		      ...
		      struct iscsi_text *hdr)
{
	...
	if(cmd){
		cmd->b_state |= BS_SETUP_ISCSI_TEXT_CMD;
		cmd->b_state_last = BS_SETUP_ISCSI_TEXT_CMD;
	}
			...
			struct iscsi_text *hdr)
{
	...
	if(cmd){
		cmd->b_state |= BS_PROC_ISCSI_TEXT_CMD;
		cmd->b_state_last = BS_PROC_ISCSI_TEXT_CMD;
	} 
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
		if (cmd->zmc_cmd_ops) {
			rx_got = qnap_iscsit_tcp_zmc_rx_data(conn, cmd, &iov[0], 
					niov, rx_size);
		} else
#endif
#endif
	...
	u32 length)
{
	...
	if(cmd){
		cmd->b_state |= BS_HANDLE_ISCSI_IMMED_DATA;
		cmd->b_state_last = BS_HANDLE_ISCSI_IMMED_DATA;
	} 
...
static int iscsit_send_datain(struct iscsi_cmd *cmd, struct iscsi_conn *conn)
{
	...
	/* workaround for TMR request */
	if (atomic_read(&se_cmd->cmd_dr.tmr_code)) {
		qnap_transport_tmr_handle_send_datain(cmd, conn, dr, &eodr, 
			atomic_read(&se_cmd->cmd_dr.tmr_resp_tas));

		/* SHALL return 2 if tmr_resp_tas is 1 */
		return eodr;		
	}
#endif
...
static int iscsit_send_response(struct iscsi_cmd *cmd, struct iscsi_conn *conn)
{
	...
	if (atomic_read(&se_cmd->cmd_dr.tmr_code)) {
		 ...
		 */
		resp_tas = qnap_transport_tmr_handle_send_response(cmd, conn, &ret);

		/* 1. We don't transmit any data for resp_tas is 0. Take care this 
		 * 2. The return value MUST depend on caller who calls iscsit_send_response()
		 */
		if (!resp_tas)
			return 0;
...
bool iscsit_check_inaddr_any(struct iscsi_np *np)
{
	bool ret = false;
				  ...
				  int skip_bytes, bool *completed)
{
	...
	if (!payload) {
		...
		payload = vzalloc(buffer_len);
		if (!payload)
			pr_warn("memory allocation failure from vzalloc() "
					"when build sendtargets buffer\n");
	}
#endif
	if (!payload)
	...
	list_for_each_entry(tiqn, &g_tiqn_list, tiqn_list) {
		...
		if (qnap_iscsit_get_matched_initiator_count(
				tiqn, conn->sess->sess_ops->InitiatorName) == 0)
			continue;
#endif
#endif
		...
		list_for_each_entry(tpg, &tiqn->tiqn_tpg_list, tpg_list) {
...
#ifdef CONFIG_MACH_QNAPTS
			if(qnap_iscsit_check_allow_np(tpg, conn, network_transport) == 0) {
				pr_info("IP:%pISpc is not in the allow potal, do not report the Target:%s for discovery.\n", &conn->local_sockaddr, tiqn->tiqn);
				continue;
			}
#endif
			...
			list_for_each_entry(tpg_np, &tpg->tpg_gnp_list,
				...
				if (np->np_sockaddr.ss_family !=
						conn->login_sockaddr.ss_family)
					continue;
...
void iscsit_thread_get_cpumask(struct iscsi_conn *conn)
{
	...
	if (iscsit_cpumask != 0xffff) {
		pr_debug("iscsit_cpumask %x\n",iscsit_cpumask);
		while (tmp_bitmaps !=0) {
...
iscsit_immediate_queue(struct iscsi_conn *conn, struct iscsi_cmd *cmd, int state)
{
	...
	if(cmd){
		cmd->b_state |= BS_HANLE_ISCSI_IMMED_QUEUE;
		cmd->b_state_last = BS_HANLE_ISCSI_IMMED_QUEUE;
...
iscsit_response_queue(struct iscsi_conn *conn, struct iscsi_cmd *cmd, int state)
{
	...
	if(cmd){
		cmd->b_state |= BS_HANLE_ISCSI_RESP_QUEUE;
		cmd->b_state_last = BS_HANLE_ISCSI_RESP_QUEUE;
		cmd->complete_jiffies = jiffies;
	}
...
#ifdef QNAP_TARGET_PERF_FEATURE
		if (qnap_target_perf_enabled())
			/* do nothing we don't want bind rx/tx pairs on same cpu */
			do {} while(0);
		else
#endif
#endif
...
int iscsi_target_rx_thread(void *arg)
{
	...
	if (conn->zmc_conn_ops)
		conn->zmc_conn_ops->get_rx_pdu(conn);
	else
#endif
#endif
	...
	struct iscsi_conn *conn)
{
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef CONFIG_QISCSI_LOG
	char login_ip[IPV6_ADDRESS_SPACE] = "";
	snprintf(login_ip, sizeof(login_ip), "%pISc", &conn->login_sockaddr);
#endif
#ifdef SUPPORT_TPC_CMD
	struct iscsi_portal_group *tpg = sess->tpg;
	struct se_portal_group *se_tpg = &tpg->tpg_se_tpg;
	u8 isid_buf[PR_REG_ISID_LEN];
	char __login_ip[IPV6_ADDRESS_SPACE];

	snprintf(isid_buf, PR_REG_ISID_LEN, "%6phN", sess->isid);
	snprintf(__login_ip, sizeof(__login_ip), "%pISc", &conn->login_sockaddr);	

	if (atomic_read(&sess->nconn) == 1) {
		/* to discard all tokens if this is last connection if 
		 * connection was closed or session reinstatment case 
		 */
		qnap_it_nexus_odx_discard_all_tokens(sess->sess_ops->InitiatorName, 
			isid_buf, __login_ip, se_tpg);
	}
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	/* here to delete conn from sess list head after nobody use this 
	 * connection via iscsit_check_conn_usage_count(). To avoid the
	 * conn pointer will be checked by race condition in iscsit_free_session()
	 * or iscsit_stop_session()
	 */
	spin_lock_bh(&sess->conn_lock);
	list_del(&conn->conn_list);
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
	if (conn->zmc_conn_ops)
		conn->zmc_conn_ops->restore_sock_callbacks(conn);	
#endif
#endif
	if (conn->sock)
	...
	if (atomic_read(&sess->nconn)) {
		...
		if (atomic_read(&sess->sess_now_release)) {
			/* we want one by one. Actially, we may use session_stop_active ?? ... */
			spin_unlock_bh(&sess->conn_lock);
			return 0;
		}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	atomic_set(&sess->sess_now_release, 1);
#endif
...
/* === wrapper codes used by tcp zmc functions === */

int qnap_iscsit_tcp_zmc_add_reject(
	struct iscsi_conn *conn,
	u8 reason,
	unsigned char *buf
	)
{
	return iscsit_add_reject(conn, reason, buf);
}

int qnap_iscsit_tcp_zmc_target_rx_opcode(
	struct iscsi_conn * conn, 
	unsigned char * buf
	)
{
	return iscsi_target_rx_opcode(conn, buf);
}

void qnap_iscsit_tcp_zmc_do_crypto_hash_buf(
	struct ahash_request *hash,
	const void *buf,
	u32 payload_length,
	u32 padding,
	...
	u8 *data_crc)
{
	iscsit_do_crypto_hash_buf(hash, buf, payload_length, padding, 
			pad_bytes, data_crc);
}

u32 qnap_iscsit_tcp_zmc_do_crypto_hash_sg(
	struct ahash_request * hash, 
	struct iscsi_cmd * cmd, 
	u32 data_offset, 
	u32 data_length, 
...
{
	return iscsit_do_crypto_hash_sg(hash, cmd, data_offset, 
			data_length, padding, pad_bytes);
}
