...
struct iscsi_cmd *iscsit_allocate_cmd(struct iscsi_conn *conn, int state)
{
	...
	qnap_iscsit_tcp_zmc_setup_cmd_ops_on_iscsi_cmd(conn, cmd);
#endif	
#endif

	if(cmd){
		cmd->b_state |= BS_ALLOCATE_ISCSI_CMD;
		cmd->b_state_last = BS_ALLOCATE_ISCSI_CMD;
			...
			unsigned char *buf, __be32 cmdsn)
{
	...
	if(cmd){
		cmd->b_state |= BS_SEQ_ISCSI_CMD;
		cmd->b_state_last = BS_SEQ_ISCSI_CMD;
	} 
	...
	u8 state)
{
	...
	if(cmd){
		cmd->b_state |= BS_ADD_ISCSI_CMD_TO_RSP_Q;
		cmd->b_state_last = BS_ADD_ISCSI_CMD_TO_RSP_Q;
	} 
...
#ifdef CONFIG_MACH_QNAPTS
	if (is_vmalloc_addr(cmd->buf_ptr))
		vfree(cmd->buf_ptr);
	else
#endif
	kfree(cmd->buf_ptr);
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
	if (cmd->conn && cmd->conn->zmc_dr)
		qnap_iscsit_tcp_zmc_release_cmd(cmd);
#endif
#endif
...
int iscsit_tx_login_rsp(struct iscsi_conn *conn, u8 status_class, u8 status_detail)
{
	...
	if (status_class == ISCSI_STATUS_CLS_REDIRECT)
		return iscsit_tx_login_rsp_redirect(conn, status_class, status_detail);
#endif
