...
/* Copied from include/linux/configfs.h */
...
QNAP_CONFIGFS_ATTRIB_SHOW(emulate_v_sup);
...
static ssize_t qnap_target_dev_wt_show(
	struct config_item *item,
	char *page
	)
{
...
static ssize_t qnap_target_dev_wt_store(
	struct config_item *item,
	const char *page,
	size_t count
	)
...
static ssize_t qnap_target_dev_zc_show(
	struct config_item *item,
	char *page
	)
{
...
static ssize_t qnap_target_dev_zc_store(
	struct config_item *item,
	const char *page,
	size_t count
	)
 ...
 * (1,1) Invalid option
 */
QNAP_CONFIGFS_ATTR(target_dev_, wt);

/* switch to enable / disable the zc (zero copy) for lun by manual */
QNAP_CONFIGFS_ATTR(target_dev_, zc);
