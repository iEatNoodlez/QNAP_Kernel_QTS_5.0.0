	...
	)
{
	...
	if (!qlib_is_ib_fbdisk_dev(&se_dev->dev_dr)) {
		...
		/* here is for iblock + fbdisk, it doesn't support fast block cloning */
		...
		pr_info("FD: LIO(File I/O) + Block Backend: "
			"%s support fast clone\n", 
			((se_dev->dev_dr.fast_blk_clone) ? "" : "not"));

	} else
		pr_warn("unknown configuration: not support fast clone\n");
