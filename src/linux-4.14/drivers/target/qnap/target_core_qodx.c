...
/**/
static int ____qnap_odx_create_initiator_id(
	u8 *initiator_name,
	u8 *isid,
	u64 *id_hi,
	u64 *id_lo
	)
{
	...
	if (ret < 0) {
		pr_err("crypto_shash_update() failed for initiator name\n");
		crypto_free_shash(tfm);
		goto out;
	}
	...
	if (ret < 0) {
		pr_err("crypto_shash_update() failed for isid\n");
		crypto_free_shash(tfm);
		goto out;
	}
	...
	if (ret < 0) {
		pr_err("crypto_shash_final() failed for md5 digest for "
			"initiator name + isid\n");
		crypto_free_shash(tfm);
		goto out;
	}
	...
	)
{
	...
	return ____qnap_odx_create_initiator_id(&initiator_name[0], 
			&isid[0], id_hi, id_lo);
	...
	)
{
	...
	*id_lo = 0;
	...
	if (ret < 0) {
		pr_err("crypto_shash_update() failed for target name\n");
		crypto_free_shash(tfm);
		goto out;
	}
	...
	*(u16 *)&data[0] = pg_tag;
	...
	if (ret < 0) {
		pr_err("crypto_shash_update() failed for pg tag id\n");
		crypto_free_shash(tfm);
		goto out;
	}
	...
	)
{
	...
	*(u32 *)&data[0] = list_id;
	...
	if (ret < 0) {
		pr_err("crypto_shash_update() failed for list id\n");
		crypto_free_shash(tfm);
		goto out;
	}
...
int qnap_it_nexus_odx_discard_all_tokens(
	char *initiator_name,
	u8 *isid_buf,
	char *login_ip,
	struct se_portal_group *se_tpg
	)
{
	...
	ret = ____qnap_odx_create_initiator_id(initiator_name, isid_buf,
	...
	snprintf(msg_str, sizeof(msg_str), "[iSCSI][RELEASE CONN] ip: %s", login_ip);

	odx_rb_discard_all_tokens(tpg_p, &reg_data, DISCARD_TOKEN_I_T_NEXUS_LOSS, msg_str);

	odx_rb_tpg_put(tpg_p);
	return 0;

}
EXPORT_SYMBOL(qnap_it_nexus_odx_discard_all_tokens);
