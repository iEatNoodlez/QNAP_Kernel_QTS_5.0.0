...
#include "qnap/target_core_zmc.h"
#endif
...
static void target_complete_failure_work(struct work_struct *work)
{
...
#ifdef CONFIG_MACH_QNAPTS
	/* we need to take care iblock case since it uses callback ... */
	int exit = 0;
	...
	if (cmd->cmd_dr.work_io_err != TCM_NO_SENSE) {
		...
		case TCM_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED:
			transport_generic_request_failure(cmd, cmd->cmd_dr.work_io_err);
			exit = 1;
			break;
#endif
		default:
...
#endif
{
...
#ifdef CONFIG_MACH_QNAPTS
	bool hit_cond = qnap_transport_hit_read_deletable(&cmd->se_dev->dev_dr,
			cmd->t_task_cdb);
#endif
	...
	if (cmd->transport_state & CMD_T_ABORTED ||
		...
		/* TODO - hmmmm ... for QNAP way, CMD_T_ABORTED won't be setup ... */
		WARN_ON((cmd->transport_state & CMD_T_ABORTED));

		int tmr_code = atomic_read(&cmd->cmd_dr.tmr_code);
	...
	unsigned char *sense_buffer)
{
	...
	qnap_target_se_cmd_zmc_init(cmd);
#endif

#ifdef SUPPORT_ISCSI_ZERO_COPY
	/* default to skip zero copy */
	cmd->cmd_dr.digest_zero_copy_skip = true;
#endif
#ifdef SUPPORT_TPC_CMD
...
void __target_execute_cmd(struct se_cmd *cmd, bool do_checks)
{
	...
	if (qnap_transport_check_iscsi_fabric(cmd)) {
		/* prep final sgl from all node and be freed when to free iscsi cmd */
		struct target_iscsi_zmc_data *p = &cmd->zmc_data.u.izmc;
		if (atomic_read(&p->write_sg_inuse) != ZMC_W_SG_INUSE_VALID) 
			goto not_run_iscsi_wzmc;

		/* try to prepare wsgl from all wsgl node again, 
		 * wsgl shall be NULL now, and free all nodes if create wsgl
		 * successfully since we will issue command here
		 */
		WARN_ON(p->wsgl);
		p->wsgl = p->ops->prep_wsgl(p, p->wsgl_node, 
					atomic_read(&p->wsgl_nents));
		p->ops->free_all_wsgl_node(p, &cmd->se_dev->dev_dr);
		if (!p->wsgl) {
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
	if (qnap_transport_check_iscsi_fabric(cmd) && cmd->t_task_cdb) {
		/* wsgl only used for cdb cmd */		
		struct target_iscsi_zmc_data *p = &cmd->zmc_data.u.izmc;
		p->ops->free_wsgl_mem(p);
	}
#endif
...
void *transport_kmap_data_sg(struct se_cmd *cmd)
{
	...
	if (qnap_transport_check_iscsi_fabric(cmd)) {
		struct target_iscsi_zmc_data *p = &cmd->zmc_data.u.izmc;
		if (!p->ops->kmap_wsgl(p, cmd->data_length))
			return p->data_vmap;
	}
#endif
...
#ifdef QNAP_ISCSI_TCP_ZMC_FEATURE
		if (qnap_transport_check_iscsi_fabric(cmd)) {
			/* Take care this path may be called before to get
			 * immediate data
			 */			
			ret = qnap_target_iscsi_zmc_alloc_sgl(
					cmd->data_direction, &zmc_alloc_done);
			if (zmc_alloc_done && (ret < 0))
				return TCM_LOGICAL_UNIT_COMMUNICATION_FAILURE;
		}
#endif

no_zmc:
		if ((cmd->data_direction == DMA_TO_DEVICE) && zmc_alloc_done)
			do {} while (0);
		else
#endif
#endif
...
#ifdef QNAP_TARGET_PERF_FEATURE
			if (qnap_target_perf_enabled())
				ret = qnap_transport_perf_exec_wt_cmd(cmd);
			else
#endif
				ret = qnap_transport_exec_rt_cmd(cmd);
...
int transport_generic_free_cmd(struct se_cmd *cmd, int wait_for_tasks)
{
	...
	if (!(cmd->se_cmd_flags & SCF_SE_LUN_CMD)) {
		...
		if (wait_for_tasks){
			cmd->b_se_state |= BS_SE_CMD_FREE_2;
			cmd->b_se_state_last = BS_SE_CMD_FREE_2;
...
#ifdef CONFIG_MACH_QNAPTS
	case TRANSPORT_WORK_THREAD_PROCESS: return "WORK_THREAD_PROCESS";
#endif
	}
...
#ifdef CONFIG_MACH_QNAPTS
	/* we don't use target_show_cmd() due to it will also be called in 
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef SUPPORT_TPC_CMD
	/* report transfer count to INFORMATION filed if ODX command */
	u8 *__cdb = cmd->t_task_cdb;

	if (si->add_sector_info 
	&& ((__cdb[0] == EXTENDED_COPY) 
		&& (IS_TPC_SCSI_PT_OP(__cdb[1]) || IS_TPC_SCSI_WUT_OP(__cdb[1])))
	)
	{
		pr_debug("report transfer_counts:0x%llx to Information Field\n",
			(unsigned long long)cmd->odx_dr.transfer_counts);
