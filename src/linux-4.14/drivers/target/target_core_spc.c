...
#ifdef CONFIG_MACH_QNAPTS
	if(!strcmp(cmd->se_tfo->get_fabric_name(), "iSCSI") && (dev->t10_wwn.model_is_set == 0))
		memcpy(&buf[16], "iSCSI Storage", strlen("iSCSI Storage"));
	else
#endif
...
static int spc_modesense_control(struct se_cmd *cmd, u8 pc, u8 *p)
{
	...
	if (target_lun_is_rdonly(cmd))
		...
		if ((cmd->se_lun) && cmd->se_lun->lun_access_ro)
			p[4] |= 0x08;
	}
#endif
	/*
