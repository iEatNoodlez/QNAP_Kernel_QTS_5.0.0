...
static int rbd_tcm_start_reset(struct se_device *se_dev, u32 timeout)
{
	...
	rc = rbd_acknowledge_notify_scsi_event(rbd_tcm_dev->rbd_dev,
...
static void rbd_tcm_reset_event_workfn(struct work_struct *work)
{
	...
	rbd_acknowledge_notify(rbd_dev, evt->notify_id, evt->cookie);
}

void rbd_tcm_reset_notify_handle(void *data, u64 notify_id, u64 cookie)
{
	...
	rbd_tcm_dev->reset_evt.cookie = cookie;
	schedule_work(&rbd_tcm_dev->reset_evt.work);
}
EXPORT_SYMBOL(rbd_tcm_reset_notify_handle);
