...
static void rbd_osd_req_format_rw(struct rbd_obj_request *obj_request)
{
	...
	osd_req->r_snapc  = obj_request->img_request?obj_request->img_request->snapc:NULL;
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QTS_CEPH)
EXPORT_SYMBOL(rbd_acknowledge_notify);
#endif

#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QTS_CEPH)
void rbd_acknowledge_notify_result(struct rbd_device *rbd_dev,
					  u64 notify_id, u64 cookie, s32 result)
#else
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QTS_CEPH)
EXPORT_SYMBOL(rbd_acknowledge_notify_result);
					  ...
					  u64 notify_op, u32 notify_timeout)
{
	...
	int buf_size = sizeof(notify_buf)+CEPH_ENCODING_START_BLK_LEN;
	char buf[buf_size];
	void *p = buf;
	...
	ceph_start_encoding(&p, 1, 1,
	    buf_size - CEPH_ENCODING_START_BLK_LEN);
	ceph_encode_copy(&p, &notify_buf, sizeof(notify_buf));

	ret = ceph_osdc_notify_ack(osdc, &rbd_dev->header_oid,
				   &rbd_dev->header_oloc, rbd_dev->watch_cookie, rbd_dev->watch_cookie,
				   buf, buf_size);
	if (ret)
		rbd_warn(rbd_dev, "acknowledge_notify_scsi_event failed: %d", ret);

	return ret;
}
EXPORT_SYMBOL(rbd_acknowledge_notify_scsi_event);
#endif
			 ...
			 u64 notifier_id, void *data, size_t data_len)
{
	...
	case RBD_NOTIFY_OP_SCSI_LUN_RESET:
		...
		rbd_tcm_reset_notify_handle(rbd_dev->tcm_device, notify_id, cookie);
		break;
#endif
	default:
	...
	return;
decode_fail:
	rbd_warn(rbd_dev, "Invalid notify_op %u", notify_op);
#endif
}
...
static ssize_t rbd_name_store(struct device *dev,
			      ...
			      size_t count)
{
	...
	new_image_name = kmalloc(strlen(buf), GFP_KERNEL);
	if (!new_image_name)
		return -ENOMEM;

	memcpy(new_image_name, buf, strlen(buf)-1);
	new_image_name[strlen(buf)-1] = '\0';
	kfree(spec->image_name);
	spec->image_name = new_image_name;
...
static DEVICE_ATTR(name, S_IRUGO|S_IWUSR, rbd_name_show, rbd_name_store);
