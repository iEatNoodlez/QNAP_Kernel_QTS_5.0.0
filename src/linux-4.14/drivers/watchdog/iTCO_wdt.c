		...
		struct itco_wdt_platform_data *pdata)
{
	...
	if (p->iTCO_version >= 6 && p->iTCO_version != 1000)
	{
		p->update_no_reboot_bit = update_no_reboot_bit_def;
	}
	else if (p->iTCO_version >= 2 && p->iTCO_version != 1000)
#else
	if (p->iTCO_version >= 2)
#endif
...
static int iTCO_wdt_probe(struct platform_device *pdev)
{
	 ...
	 */
#if defined(CONFIG_MACH_QNAPTS)
	if (p->iTCO_version >= 2 && p->iTCO_version < 6 && p->iTCO_version != 1000 &&!pdata->update_no_reboot_bit) {
#else
