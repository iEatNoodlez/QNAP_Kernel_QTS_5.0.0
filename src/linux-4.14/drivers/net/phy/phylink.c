			       ...
			       const struct phylink_mac_ops *ops)
{
    ...
    if (pl->link_interface == PHY_INTERFACE_MODE_2500BASET)
    {
        phylink_set(pl->supported, 2500baseT_Full);
		linkmode_copy(pl->link_config.advertising, pl->supported);
		dev_dbg(&ndev->dev, "pl->supported = %*pb, pl->link_config.advertising = %*pb",
				  ...
				  struct ethtool_link_ksettings *kset)
{
	...
	case MLO_AN_INBAND:
		...
		{
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_MVEBU)
			if (pl->phydev->interface == PHY_INTERFACE_MODE_2500BASET)
			{
				phylink_set(pl->supported, 2500baseT_Full);
				linkmode_copy(kset->link_modes.supported, pl->supported);
				linkmode_copy(kset->link_modes.advertising, pl->supported);
			}
#endif
			break;
        }
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_MVEBU)
	if (pl->phydev && (pl->phydev->interface == PHY_INTERFACE_MODE_2500BASET))
	{
		phylink_set(pl->supported, 2500baseT_Full);
		if (pl->netdev)
			dev_dbg(&pl->netdev->dev, "pl->supported = %*pb\n", __ETHTOOL_LINK_MODE_MASK_NBITS, pl->supported);
