...
static int al_eth_phy_init(struct al_eth_adapter *adapter)
{
	...
	phydev = phy_connect(adapter->netdev, phydev_name(phydev), al_eth_adjust_link, PHY_INTERFACE_MODE_RGMII);
	...
	netdev_info(adapter->netdev, "phy[%d]: driver %s\n",
		adapter->phy_addr, phydev->drv ? phydev->drv->name : "unknown");
	...
	netdev_info(adapter->netdev, "phy[%d]:supported %x adv %x\n",
		adapter->phy_addr, phydev->supported, phydev->advertising);
...
static int al_eth_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	...
	al_err("ioctl: phy id 0x%x, reg 0x%x, val_in 0x%x\n",
...
static int al_eth_close(struct net_device *netdev)
{
	...
	netdev_dbg(adapter->netdev, "%s\n", __func__);
        ...
        netdev_info(adapter->netdev, "phy[%d]: \n", adapter->phy_addr);
	if(adapter->phy_addr == 0x00)
...
#if 0
static int
al_eth_get_ksettings(struct net_device *netdev, struct ethtool_link_ksettings *ecmd)
{
	struct al_eth_adapter *adapter = netdev_priv(netdev);
...
static int
al_eth_set_ksettings(struct net_device *netdev, const struct ethtool_link_ksettings *ecmd)
{
	struct al_eth_adapter *adapter = netdev_priv(netdev);
	int rc = 0;
...
static const struct ethtool_ops al_eth_ethtool_ops = {
	.get_link_ksettings	= al_eth_get_ksettings,
	.set_link_ksettings	= al_eth_set_ksettings,
