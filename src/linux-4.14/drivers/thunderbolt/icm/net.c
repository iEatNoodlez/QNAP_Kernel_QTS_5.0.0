...
struct tbt_port {
	...
	uuid_t interdomain_local_uuid;
	uuid_t interdomain_remote_uuid;
	u32 command_id;
...
static int tbt_net_open(struct net_device *net_dev)
{
	...
	if (port->nhi_ctxt->msix_entries) {
...
#ifdef CONFIG_MACH_QNAPTS
		scnprintf(port->irq_rx_name, sizeof(port->irq_rx_name), "tbt-net-rx-%02u", port->num);
		res = devm_request_irq(&port->nhi_ctxt->pdev->dev,
			port->nhi_ctxt->msix_entries[3+(port->num*2)].vector,
			tbt_net_rx_msix, 0, port->irq_rx_name, port);
#else
		...
		name[8] = 't';
#ifdef CONFIG_MACH_QNAPTS
		scnprintf(port->irq_tx_name, sizeof(port->irq_tx_name), "tbt-net-tx-%02u", port->num);
		res = devm_request_irq(&port->nhi_ctxt->pdev->dev,
			port->nhi_ctxt->msix_entries[2+(port->num*2)].vector,
			tbt_net_tx_msix, 0, port->irq_tx_name, port);
#else
		res = devm_request_irq(&port->nhi_ctxt->pdev->dev,
			port->nhi_ctxt->msix_entries[2+(port->num*2)].vector,
			...
			enum medium_status medium_sts)
{
	...
	case MEDIUM_DISCONNECTED:
		...
		if (port->negotiation_status &(BIT(MEDIUM_CONNECTED) | BIT(MEDIUM_READY_FOR_CONNECTION)))
		{
				      ...
				      u8 port_num, struct genl_info *info)
{
	...
	scnprintf(net_dev->name, sizeof(net_dev->name), "tbtnet%%dp%hhu",
		  CHANNELS_PER_PORT_NUM - 1 - port->num);

    if(nhi_ctxt->pdev->device == PCI_DEVICE_ID_INTEL_FALCON_RIDGE_C_4C_NHI)
        scnprintf(net_dev->name, sizeof(net_dev->name), "tbtnet%%dp%hhu",
		    port->num);
#else
    scnprintf(net_dev->name, sizeof(net_dev->name), "tbtnet%%dp%hhu",
