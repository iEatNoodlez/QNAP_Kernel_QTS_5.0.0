...
// Fix Bug 215015 , need export symbo of reboot_type in kernel/reboot.c
...
// Fix Bug 215015 
// register an reboot callback and change reboot type to 'P'
// if there is any thunderbolt connection
atomic_t tbt_ports = ATOMIC_INIT(0);
EXPORT_SYMBOL(tbt_ports);

static int tbtnotifier_fn(struct notifier_block *nb, unsigned long action, void *data)
{
    int ret = 0;
    if (ret = atomic_read(&tbt_ports) > 0)
    {
        reboot_type = BOOT_CF9_FORCE;
        printk("tbt_ports = %d, set reboot_type = %d\n", ret, reboot_type);
    }
    return NOTIFY_OK;
}
 
struct notifier_block tbtnotifier = {
    .notifier_call = tbtnotifier_fn,
};
EXPORT_SYMBOL(tbtnotifier);
#endif // Patch for thunderbolt driver
					...
					struct thunderbolt_ip_header *hdr)
{
	...
	const uuid_t proto_uuid = APPLE_THUNDERBOLT_IP_PROTOCOL_UUID;

	if (!uuid_equal(&proto_uuid, &hdr->apple_tbt_ip_proto_uuid))
		return true;
...
static const struct nla_policy nhi_genl_policy[NHI_ATTR_MAX + 1] = {
	...
	[NHI_ATTR_LOCAL_UUID]		= { .len = sizeof(uuid_t) },
	[NHI_ATTR_REMOTE_UUID]		= { .len = sizeof(uuid_t) },
...
// Fix Bug 215015 
#ifdef CONFIG_MACH_QNAPTS
    printk("register tbt reboot notifier\n");
    register_reboot_notifier(&tbtnotifier);
#endif
	return 0;
...
// Fix Bug 215015 
#ifdef CONFIG_MACH_QNAPTS
    printk("unregister tbt reboot notifier\n");
    unregister_reboot_notifier(&tbtnotifier);
#endif
	genl_unregister_family(&nhi_genl_family);
