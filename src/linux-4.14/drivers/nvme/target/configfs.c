...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
//QNAP-Add
	case NVMF_TRTYPE_PCIE:
		return sprintf(page, "pcie\n");
#endif
	default:
		return sprintf(page, "\n");
	}
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
static ssize_t nvmet_serial_show(struct config_item *item,
		char *page)
{
	struct nvmet_port *port = to_nvmet_port(item);
	...
	if (!ctrl)
		printk("%s: Port %d controller is null\n", __func__, port->disc_addr.portid);

	return sprintf(page, "%s\n", ctrl->id_ctrl.sn);
}

static ssize_t nvmet_mn_show(struct config_item *item,
		char *page)
{
	struct nvmet_port *port = to_nvmet_port(item);
	struct al_nvme_ctrl *ctrl = port->priv;
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
//QNAP-Add
static void nvmet_port_init_tsas_pcie(struct nvmet_port *port)
{
	port->disc_addr.trtype = NVMF_TRTYPE_PCIE;
	memset(&port->disc_addr.tsas, 0, NVMF_TSAS_SIZE);
}
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
//QNAP-Add
	} else if (sysfs_streq(page, "pcie")) {
		nvmet_port_init_tsas_pcie(port);
#endif
	} else {
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
static ssize_t nvmet_serial_store(struct config_item *item,
		const char *page, size_t count)
{
	struct nvmet_port *port = to_nvmet_port(item);
	struct al_nvme_ctrl *ctrl = port->priv;
	...
	return snprintf(ctrl->id_ctrl.sn,
			sizeof(ctrl->id_ctrl.sn), "%s", page);
}

CONFIGFS_ATTR(nvmet_, serial);

static ssize_t nvmet_mn_store(struct config_item *item,
		const char *page, size_t count)
{
	struct nvmet_port *port = to_nvmet_port(item);
	struct al_nvme_ctrl *ctrl = port->priv;
	...
	return snprintf(ctrl->id_ctrl.mn,
			sizeof(ctrl->id_ctrl.mn), "%s", page);
}

CONFIGFS_ATTR(nvmet_, mn);
#endif
...
static ssize_t nvmet_ns_size_changed_show(struct config_item *item, char *page)
{
	struct nvmet_ns *ns = to_nvmet_ns(item);

	return sprintf(page, "ns size %ld GB, ns blksize %ld\n", ns->size >> 30, ns->blksize_shift);
}

static ssize_t nvmet_ns_size_changed_store(struct config_item *item,
		const char *page, size_t count)
{
	bool resize;
	...
	if (strtobool(page, &resize))
		return -EINVAL;

	if (ns->enabled && resize)
		nvmet_ns_size_changed(ns);

	return count;
}

CONFIGFS_ATTR(nvmet_ns_, size_changed);
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
/* QNAP-Add */
    if (port->disc_addr.trtype == NVMF_TRTYPE_PCIE) {
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
static ssize_t nvmet_subsys_attr_fw_rev_show(struct config_item *item,
					      char *page)
{
	struct nvmet_subsys *subsys = to_subsys(item);
...
static ssize_t nvmet_subsys_attr_fw_rev_store(struct config_item *item,
					       const char *page, size_t count)
{
	struct nvmet_subsys *subsys = to_subsys(item);

	return snprintf(subsys->fw_rev,
			sizeof(subsys->fw_rev), "%s", page);
}

CONFIGFS_ATTR(nvmet_subsys_, attr_fw_rev);
#endif
