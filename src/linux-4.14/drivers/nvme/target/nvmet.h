...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_AHCI_ALPINE) && defined(CONFIG_PCI_ALPINE_ENDPOINT)
/* QNAP-Add
 * If the target attached by Windows OS host, the PRPs transfer by Wins 
 * are NOT always 4k aligned. We have to send all PRPs (one NVMe command) 
 * to BIO one time, NOT splitting the BIO vector.
 */
#define NVMET_MAX_INLINE_BIOVEC	512
#else
...
void nvmet_ns_size_changed(struct nvmet_ns *ns);
