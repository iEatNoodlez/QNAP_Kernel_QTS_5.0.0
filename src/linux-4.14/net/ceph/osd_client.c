...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QTS_CEPH)
	if (opcode == CEPH_OSD_OP_WRITE || opcode == CEPH_OSD_OP_WRITEFULL || opcode == CEPH_OSD_OP_CMPEXT)
#else
...
static void ceph_osdc_abort_on_full(struct ceph_osd_client *osdc)
{
	...
	for (n = rb_first(&osdc->osds); n; n = rb_next(n)) {
		...
		while (m) {
			...
			if (req->r_abort_on_full &&
			    !(req->r_flags & CEPH_OSD_FLAG_FULL_FORCE)) {
				victims = true;
				break;
			}
		}
...
alloc_watch_request(struct ceph_osd_linger_request *lreq, u8 watch_opcode)
{
	...
	/*
	 * Pass 0 for cookie because we don't know it yet, it will be filled in
	 * by linger_submit().
	 */
