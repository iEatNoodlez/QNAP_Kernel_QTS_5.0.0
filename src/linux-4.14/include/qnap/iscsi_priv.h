 ...
 */
#define BIO_QNAP_ISCSI_IO_DROP_BIT	0

static inline void qnap_iscsi_fbdisk_set_bio_drop(
	struct bio_rec *brec
	) 
{
	set_bit(BIO_QNAP_ISCSI_IO_DROP_BIT, &brec->flags);
}

/* qnap_iscsi_fbdisk_is_bio_dropped() can be called in fbdisk driver */
static inline bool qnap_iscsi_fbdisk_is_bio_dropped(
	void *bi_private
	) 
{
	...
	if (qnap_bi_private_is_brec(bi_private)) {
		...
		ret = test_and_clear_bit(BIO_QNAP_ISCSI_IO_DROP_BIT, &brec->flags);
	}
