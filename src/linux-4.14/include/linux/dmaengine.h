...
#define MAX_DMA_ENGINE 4
#define DMA_CHAN_PER_ENGINE 4
typedef struct dma_chan_table 
{
    struct dma_device *dma_device;
    struct dma_chan *chan_table[DMA_TX_TYPE_END][DMA_CHAN_PER_ENGINE];
    unsigned int rr_index;
    unsigned int chan_count;
    struct proc_dir_entry *dir;
    char name[3];// name of index, max string len determinate by MAX_DMA_ENGINE count;
} DMA_CHAN_TABLE;
...
#ifdef CONFIG_ASYNC_TX_DMA_ROUND_ROBIN
#define async_dma_find_channel_by_index(type, index) dma_find_channel_by_index(DMA_ASYNC_TX, index)
#endif
...
#ifdef CONFIG_ASYNC_TX_DMA_ROUND_ROBIN
#define async_dma_find_channel_by_index(type, index) dma_find_channel_by_index(type, index)
#endif
