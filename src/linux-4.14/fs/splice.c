...
ssize_t (*qnap_splice_actor)(struct file *, struct socket *,
		loff_t __user *, size_t) = qnap_do_splice_from_socket;
EXPORT_SYMBOL(qnap_splice_actor);
#endif
...
#ifdef CONFIG_QND_QTWEAK
			error = qnap_splice_actor(out.file, sock, off_out, len);
#else
			error = qnap_do_splice_from_socket(out.file, sock, off_out, len);
#endif
