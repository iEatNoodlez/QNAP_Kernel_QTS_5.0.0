...
{
		struct nfsd3_vstorageargs *args = rqstp->rq_argp;
		...
		if(args->flags==0x03){
			args->dstlen=ntohl(*p++);
			args->dstname=kmalloc(args->dstlen+1, GFP_KERNEL);
			memset(args->dstname,0,args->dstlen+1);
			memcpy(args->dstname, p, args->dstlen);
			p=p+XDR_QUADLEN(args->dstlen);
			args->srclen=ntohl(*p++);
			args->srcname=kmalloc(args->srclen+1, GFP_KERNEL);
			memset(args->srcname,0,args->srclen+1);
			memcpy(args->srcname, p, args->srclen);
			p=p+XDR_QUADLEN(args->srclen);
		}else{
			args->srclen=0;
			args->srcname=NULL;
			args->dstlen=0;
...
#ifdef CONFIG_MACH_QNAPTS
		dchild = lookup_one_len_unlocked_without_acl(name, dparent, namlen);
#else
...
/*
 * QNAP: bug#37047 workaround, nfs(v3) readdir cause duplicate filename.
...
{
		struct nfsd3_vstorageres *resp = rqstp->rq_resp;
