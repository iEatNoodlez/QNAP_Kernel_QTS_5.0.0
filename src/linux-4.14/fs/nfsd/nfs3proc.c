...
static __be32
nfsd3_proc_vstorage(struct svc_rqst * rqstp)
{
		struct nfsd3_vstorageargs *argp = rqstp->rq_argp;
...
#endif
    			nfserr = nfsd_clonefile(rqstp, &argp->fh, &argp->dst, argp->offset, &resp->count, argp->flags,argp->srclen,argp->srcname,argp->dstlen,argp->dstname);
				if(argp->srcname)
					kfree(argp->srcname);
				if(argp->dstname)
					kfree(argp->dstname);
...
#ifdef CONFIG_MACH_QNAPTS
#if defined(NFS_VAAI)	// 2012/12/04 Cindy Jen add for NFS VAAI
static const struct svc_procedure		nfsd_procedures3[23] = {
#else
...
#if defined(NFS_VAAI)	// 2012/12/04 Cindy Jen add for NFS VAAI
	[NFS3PROC_VSTORAGE] = {
		.pc_func = nfsd3_proc_vstorage,
		.pc_decode = nfs3svc_decode_vstorageargs,
		.pc_encode = nfs3svc_encode_vstorageres,
