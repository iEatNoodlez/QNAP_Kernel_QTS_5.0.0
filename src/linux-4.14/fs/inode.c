...
int inode_init_always(struct super_block *sb, struct inode *inode)
{
	...
	if (IS_RICHACL(inode)) {
		...
		inode->i_dummy_richacl = ACL_NOT_CACHED;
	} else {
...
void __destroy_inode(struct inode *inode)
{
	...
	if (IS_RICHACL(inode)) {
		if (inode->i_richacl && !is_uncached_richacl(inode->i_richacl))
			richacl_put(inode->i_richacl);
		if (inode->i_dummy_richacl &&
		    !is_uncached_richacl(inode->i_dummy_richacl))
			richacl_put(inode->i_dummy_richacl);
	} else {
# ifdef CONFIG_FS_POSIX_ACL
