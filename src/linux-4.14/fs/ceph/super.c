	...
	if (!is_remount) {
		...
		if (dev_name_end) {
			if (strlen(dev_name_end) > 1) {
				fsopt->server_path = kstrdup(dev_name_end,
							     GFP_KERNEL);
...
static void free_options(struct ceph_mount_options *fsopt,
...
static int ceph_remount(struct super_block *sb, int *flags, char *data)
{
	...
	pr_info("%s: %d\n", __func__, new_fsopt->sb_flags & MS_POSIXACL);
		       ...
		       int flags, const char *dev_name, void *data)
{
	...
	flags |= MS_POSIXACL;
#endif
	err = parse_mount_options(&fsopt, &opt, flags, data, dev_name, false);
	if (err < 0) {
		res = ERR_PTR(err);
		goto out_final;
	}
