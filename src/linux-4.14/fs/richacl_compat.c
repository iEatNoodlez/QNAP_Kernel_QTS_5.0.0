...
richacl_from_mode(mode_t mode)
{
	...
	acl->a_owner_mask = richacl_mode_to_mask(mode >> 6, S_ISDIR(mode));
	acl->a_group_mask = richacl_mode_to_mask(mode >> 3, S_ISDIR(mode));
	acl->a_other_mask = richacl_mode_to_mask(mode, S_ISDIR(mode));

	ace = acl->a_entries;
	ace->e_type = ACE4_ACCESS_ALLOWED_ACE_TYPE;
	ace->e_flags = ACE4_SPECIAL_WHO;
	ace->e_mask = ACE4_NTFS_FULL_CONTROL;
			...
			mode_t mode, struct richacl_alloc *x)
{
	...
	FOREACH_ACL_ENTRY(pa, pacl, pe) {
		...
		ace->e_type = ACE4_ACCESS_ALLOWED_ACE_TYPE;
		ace->e_flags = eflags;
		ace->e_mask = richacl_mode_to_mask(pa->e_perm, S_ISDIR(mode));
	...
	/* set acl mask values */
	x->acl->a_owner_mask = richacl_mode_to_mask(mode >> 6, S_ISDIR(mode));
	x->acl->a_group_mask = richacl_mode_to_mask(mode >> 3, S_ISDIR(mode));
	x->acl->a_other_mask = richacl_mode_to_mask(mode, S_ISDIR(mode));
	/*
		...
		richacl_for_each_entry(ace1, x.acl) {
			richacl_for_each_entry(ace2, x.acl) {
				if (richace_is_same_identifier(ace1, ace2) &&
				    ...
				    richace_is_inherit_only(ace1) !=
					richace_is_inherit_only(ace2) &&
				    richace_is_inherited(ace1) ==
