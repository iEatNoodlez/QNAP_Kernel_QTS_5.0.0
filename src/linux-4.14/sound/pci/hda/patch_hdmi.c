...
static int generic_hdmi_build_pcms(struct hda_codec *codec)
{
	 ...
	 */
	if (audio_id_reverse){
		int reverse_idx = spec->num_nids - 1;
			...
			if ( reverse_idx >= idx )
				spec->pcm_rec[reverse_idx - idx].pcm = info;
			else
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QND_ZX_PATCH)
static int zx_hdmi_build_pcms(struct hda_codec *codec)
{
	struct hdmi_spec *spec = codec->spec;
	int idx;
	int codec_idx;
	...
	*/
	...
	//ZX driver with two codec,and num_pins is 1,so two codec all name with "HDMI 0"
	//here ,patch to distinguish it

	if(codec->core.vendor_id == 0x1d179f8a)
	{
		codec_idx = 1;
	}
	else if(codec->core.vendor_id == 0x1d179f8b)
	{
		codec_idx = 2;
	}
	else
	...
	for (idx = 0; idx < spec->num_nids + spec->dev_num - 1; idx++) {
		...
		info = snd_hda_codec_pcm_new(codec, "HDMI %d", codec_idx);
		if (!info)
			return -ENOMEM;
...
static int patch_zx_hdmi(struct hda_codec *codec)
{
	...
	codec->patch_ops.build_pcms = zx_hdmi_build_pcms;
