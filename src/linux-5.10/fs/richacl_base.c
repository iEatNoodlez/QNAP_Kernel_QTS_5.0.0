...
/*
 * Init a fresh richacl
 */
void
richacl_init(struct richacl *acl, int count)
{
	atomic_set(&acl->a_refcount, 1);
	acl->a_count = count;
}
EXPORT_SYMBOL(richacl_init);
...
/**
 * richacl_clone
 * create a copy of a richacl
 */
static struct richacl *
richacl_clone(const struct richacl *acl, gfp_t flags)
{
	int count = acl->a_count;
	size_t size = sizeof(struct richacl) + count * sizeof(struct richace);
	struct richacl *dup = kmalloc(size, flags);
...
/**
 * richacl_mask_to_mode
 * compute the file permission bits which correspond to @mask
 ...
 */
static int
richacl_mask_to_mode(unsigned int mask, int is_dir)
{
	int mode = 0;

	if (is_dir) {
		if ((mask & ACE4_NTFS_FULL_CONTROL) == ACE4_NTFS_FULL_CONTROL)
			return S_IRWXO;
	} else {
		unsigned int file_perms;

		file_perms = ACE4_NTFS_FULL_CONTROL &
			     ~(ACE4_DELETE | ACE4_DELETE_CHILD);

		if ((mask & file_perms) == file_perms)
			return S_IRWXO;
	}

	/* Ignore allways allowed permissions */
 ...
 * compute the file permission bits from the file masks
 *
 ...
 */
int
richacl_masks_to_mode(const struct richacl *acl, int is_dir)
{
	return richacl_mask_to_mode(acl->a_owner_mask, is_dir) << 6 |
	       richacl_mask_to_mode(acl->a_group_mask, is_dir) << 3 |
	       richacl_mask_to_mode(acl->a_other_mask, is_dir);
}
...
richacl_mode_to_mask(mode_t mode, int is_dir)
{
	...
	if ((mode & S_IRWXO) == S_IRWXO) {
		if (is_dir)
			return ACE4_NTFS_FULL_CONTROL;
		else
			return (ACE4_NTFS_FULL_CONTROL & ~ACE4_DELETE);
	}
...
richacl_chmod(struct richacl *acl, mode_t mode)
{
	...
	owner_mask = richacl_mode_to_mask(mode >> 6, S_ISDIR(mode));
	group_mask = richacl_mode_to_mask(mode >> 3, S_ISDIR(mode));
	other_mask = richacl_mode_to_mask(mode, S_ISDIR(mode));
...
richacl_equiv_mode(const struct richacl *acl, mode_t *mode_p)
{
	...
	mode = richacl_masks_to_mode(acl, is_dir);
	if ((acl->a_group_mask & x) !=
	    (richacl_mode_to_mask(mode >> 3, is_dir) & x) ||
	    (acl->a_other_mask & x) !=
	    (richacl_mode_to_mask(mode, is_dir) & x))
		return -1;
	...
	if ((acl->a_owner_mask & x) !=
	    (richacl_mode_to_mask(mode >> 6, is_dir) & x))
		return -1;
...
int check_richacl(struct inode *inode, int want)
{
	...
	if (want & MAY_NOT_BLOCK) {
		...
		if (acl == ACL_NOT_CACHED)
			return -ECHILD;
		return richacl_permission(inode, acl,
			want & ~MAY_NOT_BLOCK);
	} else {
		...
		if (acl) {
			int error = richacl_permission(inode, acl, want);
...
int richacl_check_acl(struct inode *inode, int want)
{
#ifdef CONFIG_FS_RICHACL
	struct richacl *acl;
