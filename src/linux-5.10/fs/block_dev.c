			     ...
			     loff_t len)
{
	...
	error = truncate_bdev_range(bdev,
	                            mode == FALLOC_FL_QNAP_MAP ? file->f_mode | FMODE_EXCL : file->f_mode,
	                            start, end);
	if (error)
		return error;
	...
	case FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE | FALLOC_FL_NO_HIDE_STALE:
		...
		break;
	/*
	 * Issue REQ_OP_QNAP_MAP to dm-thin device.
	 */
	case FALLOC_FL_QNAP_MAP:
		error = blkdev_issue_preallocate(bdev, start >> 9, len >> 9,
						 GFP_KERNEL);
		break;
	default:
		return -EOPNOTSUPP;
