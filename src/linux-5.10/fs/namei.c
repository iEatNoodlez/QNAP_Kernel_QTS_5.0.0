...
#ifdef CONFIG_QNAP_SEARCH_FILENAME_CASE_INSENSITIVE
#include <linux/nls.h>
#endif
...
int qnap_lookup_one_len_common(const char *name, struct dentry *base,
...
static int lookup_one_len_common(const char *name, struct dentry *base,
			   int len, struct qstr *this)
{
	return qnap_lookup_one_len_common(name, base, len, this,
					  QNAP_GENERIC_LINUX);
}
/**
...
/**
 * lookup_one_len_common - filesystem helper to lookup single pathname component
 * @name:	pathname component to lookup
 * @base:	base directory to lookup from
 * @len:	maximum length @len should be interpreted to
 *
				   ...
				   int len, int qnap_type)
{
	...
	err = qnap_lookup_one_len_common(name, base, len, &this, qnap_type);
	if (err)
		return ERR_PTR(err);
...
struct dentry *qnap_lookup_one_len_unlocked(const char *name,
					    struct dentry *base, int len,
					    int qnap_type)
{
	struct qstr this;
...
struct dentry *lookup_one_len_unlocked(const char *name,
				       struct dentry *base, int len)
{
	return qnap_lookup_one_len_unlocked(name, base, len,
		QNAP_GENERIC_LINUX);
}
EXPORT_SYMBOL(lookup_one_len_unlocked);

#ifdef CONFIG_MACH_QNAPTS
struct dentry *lookup_one_len_unlocked_without_acl(const char *name,
						   struct dentry *base,
						   int len)
{
	return qnap_lookup_one_len_unlocked(name, base, len,
...
EXPORT_SYMBOL(lookup_one_len_unlocked_without_acl);
#endif

#ifdef CONFIG_FS_RICHACL
struct dentry *lookup_one_len_unlocked_nfsv4_racl(const char *name,
					 struct dentry *base, int len)
{
	return qnap_lookup_one_len_unlocked(name, base, len,
		QNAP_NFSV4_RICHACL);
}
EXPORT_SYMBOL(lookup_one_len_unlocked_nfsv4_racl);
#endif
			   ...
			   bool isdir, bool replace, int qnap_type)
{
...
#ifdef CONFIG_MACH_QNAPTS
	case QNAP_WITHOUT_ACL:
		error = inode_permission_without_acl(dir, mask | MAY_WRITE);
		break;
#endif
#ifdef CONFIG_FS_RICHACL
	case QNAP_NFSV4_RICHACL:
		error = inode_permission_nfsv4_racl(dir, mask | MAY_WRITE);
		break;
#endif
	default:
		error = inode_permission(dir, mask | MAY_WRITE);
	}
	...
	case QNAP_WITHOUT_ACL:
		if (error && IS_RICHACL(inode)) {
			if (!inode_permission_without_acl(inode,
				MAY_DELETE_SELF) &&
	...
	case QNAP_NFSV4_RICHACL:
		if (error && IS_RICHACL(inode)) {
			if (!inode_permission_nfsv4_racl(inode,
				MAY_DELETE_SELF) &&
			    !inode_permission(dir, mask))
				error = 0;
		}
	...
	default:
		if (error && IS_RICHACL(inode)) {
			/*
			 * Deleting is also permitted with MAY_EXEC on
			 * the directory and MAY_DELETE_SELF on the inode.
			 */
...
#ifdef CONFIG_MACH_QNAPTS
static inline int may_create_without_acl(struct inode *dir,
...
int qnap_vfs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode,
		   int qnap_type)
{
	int error = 0;
...
#ifdef CONFIG_FS_RICHACL
int vfs_mkdir_nfsv4_racl(struct inode *dir, struct dentry *dentry,
			 umode_t mode)
{
...
#ifdef CONFIG_MACH_QNAPTS
int vfs_mkdir_without_acl(struct inode *dir, struct dentry *dentry,
			  umode_t mode)
{
	       ...
	       int qnap_type)
{
...
#ifdef CONFIG_MACH_QNAPTS
	case QNAP_WITHOUT_ACL:
		error = may_delete_without_acl(old_dir, old_dentry, is_dir, 0);
		if (error)
			return error;
		...
		if (!target) {
			...
			if (!(flags & RENAME_EXCHANGE))
				error = may_delete_without_acl(new_dir,
					new_dentry, is_dir, 1);
			else
				error = may_delete_without_acl(new_dir,
					new_dentry, new_is_dir, 1);
		}
		break;
#endif
#ifdef CONFIG_FS_RICHACL
	case QNAP_NFSV4_RICHACL:
		...
		if (!target) {
			error = may_create_nfsv4_racl(new_dir, new_dentry,
				is_dir);
		} else {
			new_is_dir = d_is_dir(new_dentry);

			if (!(flags & RENAME_EXCHANGE))
				error = may_delete_nfsv4_racl(new_dir,
					new_dentry, is_dir, 1);
			else
				error = may_delete_nfsv4_racl(new_dir,
					new_dentry, new_is_dir, 1);
	...
	default:
		...
		if (!target) {
			...
			if (!(flags & RENAME_EXCHANGE))
				error = may_delete(new_dir, new_dentry,
					is_dir, 1);
			else
				error = may_delete(new_dir, new_dentry,
					new_is_dir, 1);
...
#ifdef CONFIG_FS_RICHACL
			case QNAP_NFSV4_RICHACL:
				error = inode_permission_nfsv4_racl(source,
					MAY_WRITE);
				break;
#endif
#ifdef CONFIG_MACH_QNAPTS
			case QNAP_WITHOUT_ACL:
				error = inode_permission_without_acl(source,
					MAY_WRITE);
				break;
#endif
			default:
...
#ifdef CONFIG_FS_RICHACL
			case QNAP_NFSV4_RICHACL:
				error = inode_permission_nfsv4_racl(target,
					MAY_WRITE);
				break;
#endif
#ifdef CONFIG_MACH_QNAPTS
			case QNAP_WITHOUT_ACL:
				error = inode_permission_without_acl(target,
					MAY_WRITE);
				break;
#endif
			default:
                                  ...
                                  const char __user *file)
{
	...
	error = filename_lookup(AT_FDCWD, getname_kernel(path_name->name),
	                        LOOKUP_DIRECTORY | LOOKUP_CASE_INSENSITIVE | LOOKUP_FOLLOW,
	                        &nd.path, NULL);
