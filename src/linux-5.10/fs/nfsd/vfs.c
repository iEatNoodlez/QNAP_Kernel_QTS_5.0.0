...
#ifdef NFS_VAAI_SUPPORT_ZFS
#define PAGE_SIZE_FOR_ZFS_LAZY_CLONE (PAGE_SIZE<<5)
long my_vfs_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
        int error = -ENOTTY;
			...
			struct iattr *iap)
{
	 ...
	 */
#ifdef QNAP_NFS_FORCE_UMASK
	if(nfsd_force_umask==0)
		iap->ia_valid &= ~ATTR_MODE;
#else
	iap->ia_valid &= ~ATTR_MODE;
#endif
		...
		int type, dev_t rdev, struct svc_fh *resfhp)
{
	...
	err = nfsd_permission(rqstp, fhp->fh_export, dentry, mask);
	if (err)
		goto out;
				...
				struct svc_fh *resfhp)
{
	...
	mask |= NFSD_MAY_CREATE_FILE;
#endif
	err = fh_verify(rqstp, fhp, S_IFDIR, mask);
	if (err)
		goto out;
...
nfsd_init_clonefile_slab(void)
{
	...
	clonefile_slab = kmem_cache_create("nfsd_clonefile",
			PAGE_SIZE_FOR_ZFS_LAZY_CLONE, 0, 0, NULL);
#else
	clonefile_slab = kmem_cache_create("nfsd_clonefile",
			PAGE_SIZE, 0, 0, NULL);
	...
	if(job_to_do>0 || time_after(jiffies, t->inquiry_time  + (1*HZ) )){
		...
		list_for_each_entry_safe(t, n,cmd_list_head, cmd_list_entry){
			if(t->status==VAAI_CMD_STATUS_INIT){
				...
				if(t->operation == NFS3_VSTORAGEOP_RESERVESPACE || t->operation == NFS4_VSTORAGEOP_RESERVESPACE)
					reserve_thread(t);
				else if(t->operation == NFS3_VSTORAGEOP_CLONEFILE || t->operation == NFS4_VSTORAGEOP_CLONEFILE)
					clonefile_thread(t);
				return err;
			}else{
	...
	param->status = VAAI_CMD_STATUS_RUNNING;
        if (!fp->f_op->fallocate) {
                ...
                if (!buf) {
                        ...
                        //oldfs = get_fs();
                        //set_fs(get_ds());
                        ret = kernel_write(fp, buf, len, &offset);
                        //set_fs(oldfs);
                        if ((ssize_t)ret < 0) {
...
nfsd_reservespace(struct svc_rqst *rqstp, struct svc_fh *fhp, loff_t offset, u64 *count)
{
	...
	if(vaai_cmd_list_init())
		...
		if (rqstp->rq_vers < 4)
		   ...
		   param->operation = NFS4_VSTORAGEOP_RESERVESPACE;
...
nfsd_extendedstat(struct svc_rqst *rqstp, struct svc_fh *fhp, u64 *total, u64 *alloc, u64 *unique)
{
	...
	inode = fp->f_path.dentry->d_inode;
#ifdef NFS_VAAI_SUPPORT_ZFS
	struct ioctl_data_e *iodata=NULL;
	int ret=-1;
	iodata=kmalloc(sizeof(struct ioctl_data_e),GFP_KERNEL);
	if(iodata==NULL){
		DBG_PRINT("nfsd: ERROR allcate iodata  failed\n");
		err = nfserr_jukebox;
		goto done;
	}

	memset(iodata,0,sizeof(struct ioctl_data_e));
	iodata->src_inode=(u64)inode;
	iodata->total=0;
	iodata->alloc=0;
	iodata->unique=0;

	ret=my_vfs_ioctl(fp,0xF0000001,(unsigned long)iodata);
	if(ret){
		*total = 0;
		*alloc = 0;
		*unique = 0;
	}else{
		*total=(unsigned long long)iodata->total;
                *alloc=(unsigned long long)iodata->alloc;
                *unique=(unsigned long long)iodata->unique;
	}
	if(iodata != NULL)
		kfree(iodata);
...
int clonefile_thread(void *a)
{
	...
	len = param->count;
#ifdef NFS_VAAI_SUPPORT_ZFS
	struct ioctl_data *iodata=NULL;
	struct inode *src_inode = NULL;
	struct inode *dst_inode = NULL;

	if(flags==0x03){
		iodata=kmalloc(sizeof(struct ioctl_data),GFP_KERNEL);
		if(iodata==NULL){
			DBG_PRINT("nfsd: ERROR allcate iodata  failed\n");
			err = nfserr_jukebox;
			goto clonefile_done;
		}
		memset(iodata,0,sizeof(struct ioctl_data));
		
		src_inode = file_inode(src_fp);
	       	dst_inode = file_inode(dst_fp);//dst_fp->f_path.dentry->d_inode;
		...
		struct iattr size_attr = {
			.ia_valid       = ATTR_SIZE | ATTR_UID | ATTR_GID | ATTR_SIZE ,
			.ia_mode	= src_inode->i_mode,
			.ia_uid		= src_inode->i_uid,
			.ia_gid		= src_inode->i_gid,
			.ia_size	= src_inode->i_size,
		};
		struct dentry *d=dst_fp->f_path.dentry;
		inode_lock(d->d_inode);
	
		err = notify_change(d, &size_attr, NULL);
		...
		iodata->src_inode=(u64)src_inode;
	        iodata->dst_inode=(u64)dst_inode;
/*
		iodata->src_inode=(u64)src_fp;
		iodata->dst_inode=(u64)dst_fp;
*/	        
		iodata->offset=0;
	        iodata->clone_length=len;
		iodata->return_done=0;
	}else
#endif
	{
	...
	param->status = VAAI_CMD_STATUS_RUNNING;
		...
		pos = progress;
#ifdef NFS_VAAI_SUPPORT_ZFS
		size = MIN(PAGE_SIZE_FOR_ZFS_LAZY_CLONE, len - progress);
		if(flags==0x03){
/*
			iodata->src_inode=(u64)src_fp;
			iodata->dst_inode=(u64)dst_fp;
*/
			...
			iodata->dst_inode=(u64)dst_inode;
			iodata->offset=pos;
			iodata->clone_length=size;
			//iodata->return_done=0;
			ret=my_vfs_ioctl(src_fp,0xF0000000,(unsigned long)iodata);
			if (ret!=0){
				DBG_PRINT("nfsd: ERROR vfs_ioctl failed\n");
				err = nfserr_io;
				goto clonefile_done;
			}
		}else
#endif
		{
	               	//oldfs = get_fs();
	               	//set_fs(get_ds());
	               	ret = kernel_read(src_fp, buf, size, &pos);
	               	//set_fs(oldfs);
	               	if (ret < 0) {
				...
				if ((flags & 0x10) == 0x10 && my_is_empty(buf, size) == 0) {
					...
					//oldfs = get_fs();
					//set_fs(get_ds());
					ret = kernel_write(dst_fp, buf, size, &pos);
					//set_fs(oldfs);
					if (ret < 0) {
	...
	list_for_each_entry(t, cmd_list_head, cmd_list_entry){
		if(!memcmp( &(t->tag), &(dst_fhp->fh_handle) ,sizeof(struct knfsd_fh))){
			...
			*status=t->status;
			...
			}else if(*status == VAAI_CMD_STATUS_RUNNING && (t->operation == NFS3_VSTORAGEOP_RESERVESPACE || t->operation == NFS4_VSTORAGEOP_RESERVESPACE)){
	                    update_progress(t);
	                    *count=t->progress;
			}
			spin_unlock(&cmd_list_lock);
...
__be32
nfsd_clonefile(struct svc_rqst *rqstp, struct svc_fh *src_fhp, struct svc_fh *dst_fhp, loff_t offset, u64 *count, u32 flags,int srclen,char *srcname,int dstlen,char *dstname )
{
	struct param_vaai *param=NULL;
	...
	if (rqstp->rq_vers < 4)
	   ...
	   param->operation = NFS4_VSTORAGEOP_CLONEFILE;
	...
	param->abort = 0;
	if(flags==0x03){
		struct svc_fh realsrc;

		if (rqstp->rq_vers < 4)
		   fh_init(&realsrc, NFS3_FHSIZE);
		else
		   fh_init(&realsrc, NFS4_FHSIZE);
		
		nfserr = nfsd_lookup(rqstp, src_fhp,srcname,srclen,&realsrc);
		if(nfserr==0){
			if(nfsd_open(rqstp, &realsrc, S_IFREG, NFSD_MAY_READ, &param->src_fp)){
		...
		fh_put(&realsrc);
	}else if(nfsd_open(rqstp, src_fhp, S_IFREG, NFSD_MAY_READ, &param->src_fp)){
		DBG_PRINT("nfsd: ERROR can not open SRC file\n");
		kfree(param);
		goto out;
	}

	if(src_fhp->fh_locked)
		fh_unlock(src_fhp);
	...
	param->progress = 0;
	if(flags==0x03){
		struct svc_fh realdst;
		if (rqstp->rq_vers < 4)
		   fh_init(&realdst, NFS3_FHSIZE);
		else
		   fh_init(&realdst, NFS4_FHSIZE);
		
		nfserr = nfsd_lookup(rqstp, dst_fhp,dstname,dstlen,&realdst);
		if(nfserr==0){
			memset(&(param->tag),0,sizeof(struct knfsd_fh));
			memcpy(&(param->tag),&(realdst.fh_handle),sizeof(struct knfsd_fh));
			if(nfsd_open(rqstp, &realdst, S_IFREG, NFSD_MAY_WRITE, &param->dst_fp)){
				DBG_PRINT("nfsd: ERROR can not open SRC file\n");
				kfree(param);
				fh_put(&realdst);
				goto out;
		...
		fh_put(&realdst);
	}else if(nfsd_open(rqstp, dst_fhp, S_IFREG, NFSD_MAY_WRITE, &param->dst_fp)){
		DBG_PRINT("nfsd: ERROR can not open DST file\n");
		kfree(param);
		goto out;
	}
	if(dst_fhp->fh_locked)
		fh_unlock(dst_fhp);
