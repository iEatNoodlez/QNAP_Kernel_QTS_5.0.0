...
static int qnap_ext4_run_trim_request(struct ext4_trim_request *etr)
{
	...
	if (etr->tr_next_group == etr->tr_first_group)
		pr_info("EXT4-fs (device %s): ext4trim start "
...
static int qnap_ext4_run_trim_thread(struct super_block *sb)
{
	...
	if (!sbi->s_trim_task) {
		sbi->s_trim_task = kthread_run(qnap_ext4_trim_thread,
			       sbi->s_trim_request, "ext4trim/%s", sb->s_id);
		if (IS_ERR(sbi->s_trim_task)) {
			int err = PTR_ERR(sbi->s_trim_task);
			pr_crit("EXT4-fs: create trim thread failed, %d.\n",
				err);
			return err;
		}
	}
