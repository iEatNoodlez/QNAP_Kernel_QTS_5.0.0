				  ...
				  int flags)
{
	...
	int ret2 = 0, ret3 = 0;
	int retries = 0;
	bool thin_clone = !(flags & EXT4_GET_BLOCKS_UNWRIT_EXT);
	int depth = 0;
	struct ext4_map_blocks map;
...
retry:
	while (ret >= 0 && len) {
		...
		if (!ret2 && q && blk_queue_pool_block_size(q)) {
			if ((map.m_flags & EXT4_MAP_NEW) && !thin_clone &&
			    !blk_queue_thick(q) && /* Only thin device should care about this */
			    blkdev_issue_preallocate(
				    inode->i_sb->s_bdev,
