...
bool qnap_ext4_fake_readonly_check(struct super_block *sb)
{
	...
	return (ext4_test_mount_flag(sb, EXT4_MF_FAKE_READONLY))? true : false;
}
...
static void enable_acl(struct super_block *sb)
{
	...
	ext4_msg(sb, KERN_ERR, "POSIX ACL options not supported");
	goto out;
#endif
...
static void disable_acl(struct super_block *sb)
{
	...
	goto out;
#endif
	sb->s_flags &= ~(SB_POSIXACL | SB_RICHACL);
	goto out;
out:
	return;
}
			    ...
			    unsigned int *journal_ioprio, int is_remount)
{
	...
	case Opt_inlinecrypt:
		...
		return 1;
	case Opt_priv:
		ext4_set_mount_flag(sb, EXT4_MF_FAKE_READONLY);
		return 1;
	case Opt_nopriv:
		ext4_clear_mount_flag(sb, EXT4_MF_FAKE_READONLY);
		return 1;
	}
...
static int ext4_run_li_request(struct ext4_li_request *elr)
{
	...
	int ret = 0;
	struct ext4_sb_info *sbi = EXT4_SB(elr->lr_super);
	int status;
...
#endif
                pr_info("EXT4-fs (device %s): ext4lazyinit start"
                        " (start from %u, total %u)\n", sb->s_id,
                        elr->lr_first_group, ngroups - elr->lr_first_group);
        }
...
static void ext4_remove_li_request(struct ext4_li_request *elr)
{
	...
	EXT4_SB(elr->lr_super)->s_lazyinit_status = 0;
	kfree(elr);
}
...
static void ext4_unregister_li_request(struct super_block *sb)
{
	...
	pr_info("EXT4-fs (device %s): ext4lazyinit exit. (unregister path)\n"
		, sb->s_id);
...
static void ext4_clear_request_list(void)
{
	...
	list_for_each_safe(pos, n, &ext4_li_info->li_request_list) {
		elr = list_entry(pos, struct ext4_li_request,
				 lr_request);
		pr_info("EXT4-fs (device %s): ext4lazyinit exit. (clear path)\n",
			elr->lr_super->s_id);
		ext4_remove_li_request(elr);
	}
	mutex_unlock(&ext4_li_info->li_list_mtx);
...
static int ext4_remount(struct super_block *sb, int *flags, char *data)
{
	...
	if (ext4_test_mount_flag(sb, EXT4_MF_FAKE_READONLY))
