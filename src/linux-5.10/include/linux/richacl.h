 ...
 *
 * Based one the mapping table of redmine#26076
 * ACE4_DELETE is special permission and does not belong to POSIX "write"
 */
				      ...
				      struct richacl *acl)
{
	...
	spin_lock(&inode->i_lock);
	old = inode->i_richacl;
	rcu_assign_pointer(inode->i_richacl, richacl_get(acl));
	spin_unlock(&inode->i_lock);
...
extern void richacl_init(struct richacl *, int);
...
extern int richacl_masks_to_mode(const struct richacl *, int is_dir);
extern unsigned int richacl_mode_to_mask(mode_t, int is_dir);
