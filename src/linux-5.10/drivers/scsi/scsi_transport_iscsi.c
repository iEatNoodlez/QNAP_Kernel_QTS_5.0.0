			...
			uint16_t queue_depth)
{
...
#ifdef __QNAP__
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_VIRTUAL_JBOD)
	 ...
	 * the cmds_max will be power of 2 so don't worry about bit 0
	 */
	bool vjbod_sess = ((cmds_max & 0x0001) ? true : false);
	if (vjbod_sess)
		cmds_max &= ~0x0001;
#endif
#endif
			       ...
			       struct iscsi_uevent *ev, int msg_type)
{
	...
	if (non_blocking & CONNECT_VJBOD_ISER_BIND_SRC_ADDR) {
		...
		if (!transport->ep_connect_bind_src) {
			pr_warn("[VJBOD ISER] %s, not found ep_connect_bind_src "
				"function during to bind src, skip to bind\n", __func__);
		} else {

			pr_info("[VJBOD ISER] %s, got CONNECT_VJBOD_ISER_BIND_SRC_ADDR flag, "
