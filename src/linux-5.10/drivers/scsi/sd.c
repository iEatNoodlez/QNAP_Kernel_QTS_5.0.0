...
/* 1: is iscsi disk , 0: NOT iscsi disk */
int is_iscsi_disk(struct scsi_device *sdp)
{
...
/* 0: is QNAP iSCSI disk, 1: failure case */
...
static void sd_rescan(struct device *dev)
{
	...
	bool it_is_iscsi_disk = ((sdp) ? is_iscsi_disk(sdp) : false);
#endif
#endif
...
static int sd_probe(struct device *dev)
{
...
#if defined(CONFIG_MACH_QNAPTS)
#ifdef CONFIG_VIRTUAL_JBOD
	error = qnap_get_iscsi_vjbod_idx(sdp, sdp->probe_sd_type, &index);
	if (error != -ENODEV)
		goto end_get_idx;
#endif
	...
	if (error != -ENODEV) {
#ifdef CONFIG_VIRTUAL_DISK_EX
		its_iscsi_device = true; // Jay Wei,20150122, Task #11550.
#endif
		goto end_get_idx;
	}
#endif
...
static void scsi_disk_release(struct device *dev)
{
	...
	int ret;
#ifdef CONFIG_VIRTUAL_JBOD
	ret = qnap_clear_iscsi_vjbod_idx(sdkp->device, 
			sdkp->device->probe_sd_type, sdkp->index);
	if (ret != -ENODEV) 
		goto end_free_idx;
#endif
#ifdef CONFIG_VIRTUAL_DISK
	ret = QNAP_clear_iscsi_index(sdkp->device, sdkp->index);
	if (ret != -ENODEV) 
		goto end_free_idx;
#endif
...
#ifdef CONFIG_VIRTUAL_DISK
#ifdef CONFIG_VIRTUAL_DISK_EX
	qnap_init_virtual_disk_ex_vars();
#endif
