...
static int NotifyQEvent(int event_type, const char *target_name)
{
	...
	memcpy((char *) &addr_send.sun_path + 1, QNAP_NOTIFY_NAMESPACE, 
						strlen(QNAP_NOTIFY_NAMESPACE));

	if (!sock->ops || !sock->ops->connect) {
		printk("sock ops or connect is NULL\n");
		goto out_release;
	}
        if ((retval = sock->ops->connect(sock, (struct sockaddr *)&addr_send, 
				sizeof(addr_send), 0)) < 0) 
	{
	...
	rc = ksocket_send(sock, NULL, buf, strlen(buf)+1);
#else
	rc = ksocket_send(sock, (struct sockaddr_in *)&addr_send, buf, strlen(buf)+1);
#endif
...
{
	spin_lock_init(&qnap_event_lock);
...
int qnap_iscsi_has_ping_timed_out(struct iscsi_conn *conn)
{
	return time_before_eq(conn->last_ping + (conn->ping_timeout * HZ), jiffies);
}
EXPORT_SYMBOL_GPL(qnap_iscsi_has_ping_timed_out);
