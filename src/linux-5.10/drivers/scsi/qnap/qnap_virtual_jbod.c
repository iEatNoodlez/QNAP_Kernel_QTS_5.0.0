...
/**/
static DEFINE_SPINLOCK(qnap_vjbod_sd_index_lock);
...
static int ____qnap_get_iscsi_vjbod_idx(
...
static void ____qnap_clear_iscsi_vjbod_idx(
	struct scsi_device *sdev,
	u32 idx
	)
{
	...
	)
{
	...
	switch(type) {
	case SD_VJBOD:
		ret = ____qnap_get_iscsi_vjbod_idx(sdev, idx);
		break;
	default:
		ret = -ENODEV;
		break;
...
/* -ENODEV: NOT QNAP VJBOD disk
 * 0: good to get vjbod idx
 * others: fail to get vjbod idx
 */
	...
	)
{
	...
	if (!qnap_is_iscsi_disk(sdev) && (sdev->probe_sd_type == SD_VJBOD)) {
		spin_lock(&qnap_vjbod_sd_index_lock);		
		ret = __qnap_get_iscsi_vjbod_idx(sdev, type, idx);
		spin_unlock(&qnap_vjbod_sd_index_lock);
	...
	)
{
	switch(type) {
	case SD_VJBOD:
		____qnap_clear_iscsi_vjbod_idx(sdev, idx);
		break;
	default:
		break;
	}
...
/* -ENODEV: not QNAP VJBOD disk
 * 0: good to clear vjbod idx
 */
int qnap_clear_iscsi_vjbod_idx(
	struct scsi_device *sdev,
	int type,
	u32 idx
	)
{
	...
	if (!qnap_is_iscsi_disk(sdev) && (sdev->probe_sd_type == SD_VJBOD)) {
		spin_lock(&qnap_vjbod_sd_index_lock);
		__qnap_clear_iscsi_vjbod_idx(sdev, type, idx);
		spin_unlock(&qnap_vjbod_sd_index_lock);
		ret = 0;
	}
