		       ...
		       int disposition)
{
	...
	off += scnprintf(logbuf + off, logbuf_len - off,
				 "cmd_age=%lus result=%x sbuf[0,1,2,3,7,12,13]=%x_%x_%x_%x_%x_%x_%x blk_rq_bytes=%x passthrough=%x cmd_flags=%x jiffies_alloc=%x allowed=%x timeout=%x jiffies=%x",
				cmd_age, cmd->result, cmd->sense_buffer[0], cmd->sense_buffer[1], cmd->sense_buffer[2], cmd->sense_buffer[3],
				cmd->sense_buffer[7], cmd->sense_buffer[12], cmd->sense_buffer[13], blk_rq_bytes(cmd->request), 
				blk_rq_is_passthrough(cmd->request), cmd->request->cmd_flags, cmd->jiffies_at_alloc, cmd->allowed, cmd->request->timeout, jiffies);
out_printk:
	dev_printk(KERN_INFO, &cmd->device->sdev_gendev, "%s", logbuf);
	scsi_log_release_buffer(logbuf);
}
