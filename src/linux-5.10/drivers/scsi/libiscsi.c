...
void iscsi_conn_failure(struct iscsi_conn *conn, enum iscsi_err err)
{
	...
	iscsi_conn_error_event(conn->cls_conn, err);
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_VIRTUAL_JBOD_DEBUG)
#if 0 // just for debug purpose
	if (qnap_vjbod_is_vjbod_session(conn->session) == 0) {
		iscsi_conn_printk(KERN_ERR, conn, 
			"[VJBOD] %s: err: %d, try dump stack\n", __func__, err);
		dump_stack();
	}
#endif
...
/* take care this code !! it was referred by iscsi_check_transport_timeouts() */
static bool __qnap_check_transport_timeouts(
	struct iscsi_conn *conn,
	int (*has_ping_timeout_func)(struct iscsi_conn *),
	char *msg_prefix
	)
{
	struct iscsi_session *session = conn->session;
	unsigned long next_timeout = 0, last_recv;

	last_recv = conn->last_recv;

	if (has_ping_timeout_func(conn)) {	
		iscsi_conn_printk(KERN_ERR, conn, "%s ping timeout "
			"of %d secs expired, last rx %lu, "
			"last ping %lu, now %lu, retry %d, Hz %lu\n", 
			msg_prefix, conn->ping_timeout, last_recv, 
			conn->last_ping, jiffies, conn->qnap_ping_retry, HZ);
		
		if (++ conn->qnap_ping_retry > MAX_PING_RETRY) {
			spin_unlock(&session->frwd_lock);

			iscsi_conn_printk(KERN_ERR, conn, "%s exceeds ping timeout "
				"retry counter\n", msg_prefix);			
			iscsi_conn_failure(conn, ISCSI_ERR_CONN_FAILED);
			return true;
		}
		...
		iscsi_conn_printk(KERN_ERR, conn, "%s Setting next tmo %lu\n", 
			msg_prefix, next_timeout);
...
/* refer from original iscsi_check_transport_timeouts() but add some patch code */
static void iscsi_check_transport_timeouts(struct timer_list *t)
{
	...
	bool vjbod_sess = ((!qnap_vjbod_is_vjbod_session(session)) ? true: false);
#endif
...
#ifdef CONFIG_VIRTUAL_JBOD
		if (vjbod_sess) {
			if (__qnap_check_transport_timeouts(conn,
					qnap_vjbod_iscsi_has_ping_timed_out, "[VJBOD]"))
				return;	
		}
		else 
#endif
			if (__qnap_check_transport_timeouts(conn,
					qnap_iscsi_has_ping_timed_out, ""));
				return;
	}
...
void iscsi_conn_stop(struct iscsi_cls_conn *cls_conn, int flag)
{
	...
	if (flag == STOP_CONN_RECOVER) {
		...
		if (session->state == ISCSI_STATE_IN_RECOVERY &&
			 ...
			 */
			if (session && session->targetname && (!conn->qnap_ping_retry))
			{
				struct Scsi_Host *shost;
				shost = session->host;
