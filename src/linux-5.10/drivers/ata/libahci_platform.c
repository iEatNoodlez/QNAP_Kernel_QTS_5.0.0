...
#if defined(CONFIG_ARCH_ROCKCHIP)
ssize_t rk3568_ahci_transmit_led_message(struct ata_port *ap, u32 state,
		ssize_t size);
struct ata_port_operations ahci_platform_ops = {
	.inherits	= &ahci_ops,
	.host_stop	= ahci_host_stop,
	.transmit_led_message	= rk3568_ahci_transmit_led_message,
};
#else
struct ata_port_operations ahci_platform_ops = {
	.inherits	= &ahci_ops,
			    ...
			    struct scsi_host_template *sht)
{
	...
	rc = of_get_named_gpio(dev->of_node, "gpios", 0);
	if (gpio_is_valid(rc)) {
		hpriv->led_gpio[0] = rc;
		rc = gpio_request(hpriv->led_gpio[0], "sata led gpio");
		if (rc) {
			dev_err(&pdev->dev, "rk3568 ahci gpio_request %d failed: %d\n",
				hpriv->led_gpio[0], rc);
		} else {
			gpio_direction_output(hpriv->led_gpio[0], 1);
