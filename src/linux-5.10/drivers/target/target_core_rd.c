...
#if defined(Athens)
	ssize_t bl = sprintf(b, "Cisco RamDisk ID: %u  RamDisk Makeup: rd_mcp\n",
			rd_dev->rd_dev_id);
#elif defined(IS_G)
	ssize_t bl = sprintf(b, "RamDisk ID: %u  RamDisk Makeup: rd_mcp\n",
			rd_dev->rd_dev_id);
#else
	ssize_t bl = sprintf(b, "QNAP RamDisk ID: %u  RamDisk Makeup: rd_mcp\n",
			rd_dev->rd_dev_id);
#endif
#else
