...
#ifdef CONFIG_MACH_QNAPTS
	qlib_get_naa_6h_code((void *)dev, &dev->dev_dr.dev_naa[0], buf, 
...
static int target_xcopy_locate_se_dev_e4_iter_old(struct se_device *se_dev,
					      void *data)
{
...
#ifdef CONFIG_MACH_QNAPTS
	/* Original concept is correct since the each copy opeartion (3rd-copy)
	 * is unique (identified by List ID) for each session. But, for vmware 
	 * xcopy feature, it requres SNLID (Support No List ID) bit is 1 (reported
	 * from target_rcr_operating_parameters() function), it shall be implicit
	 * to search all global devices
	 *
	 * 1.original concept
	 * - https://www.spinics.net/lists/kernel/msg3798835.html
	 *
	 * 2. orignal xcopy porting
	 * - https://www.spinics.net/lists/target-devel/msg05127.html
	 */
	if (SNLID_BIT == 1) {
		...
		ret = target_for_each_device(
				target_xcopy_locate_se_dev_e4_iter_old, &info);
		if (ret == 1) {
			*_found_dev = info.found_dev;
			*_found_lun_ref = NULL;
			return 0;
		} else
			goto err_out;
...
#ifdef CONFIG_MACH_QNAPTS
	/* the read/write operation will use the cmd->execute_cmd, actually, 
	 * it will go to backend's execute_rw. we did everything (handle no-spc, 
	 * hit-threshold) already, but still need to print clean message to know
	 * what happens
	 */
	if (qlib_thin_lun(&dst_dev->dev_dr)) {
		if (se_cmd->cmd_dr.work_io_err == TCM_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED)
			ec_cmd->cmd_dr.work_io_err = se_cmd->cmd_dr.work_io_err;

		if (se_cmd->cmd_dr.work_io_err == TCM_SPACE_ALLOCATION_FAILED_WRITE_PROTECT) {
			ec_cmd->cmd_dr.work_io_err = se_cmd->cmd_dr.work_io_err;
...
#ifdef CONFIG_MACH_QNAPTS
	if (ec_cmd->cmd_dr.io_ops.fbc_xcopy) {
		/* TODO, shall check dc bit ? */
		u64 copy_bytes = ((u64)xop->nolb * xop->src_dev->dev_attrib.block_size);
		rc = ec_cmd->cmd_dr.io_ops.fbc_xcopy(xop->src_dev, xop->dst_dev,
				xop->src_lba, xop->dst_lba, &copy_bytes);
		if (!rc) {
			/* fast block clone succeed */
			if (!qnap_transport_check_cmd_hit_thin_threshold(ec_cmd)) {
				rc = TCM_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED;
				goto fbc_err;
			}		
		...
		if (rc != -ENOTSUPP) {
			if (rc == -ENOSPC)
				rc = TCM_SPACE_ALLOCATION_FAILED_WRITE_PROTECT;
			else
				rc = TCM_3RD_PARTY_DEVICE_FAILURE;
fbc_err:
			/* fast block clone fail */
			ec_cmd->cmd_dr.work_io_err = rc;
			goto out;
		}
		/* fall-through */
	}
...
static sense_reason_t target_rcr_operating_parameters(struct se_cmd *se_cmd)
{
	 ...
	 */
#ifdef CONFIG_MACH_QNAPTS
	/* tricky method to use definition here */
	p[4] = SNLID_BIT;
#else
	p[4] = 0x1;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	/* even we don't use them but still need to do this ... */
	p[37] = ilog2(se_cmd->se_dev->dev_attrib.block_size);
	p[38] = ilog2(se_cmd->se_dev->dev_attrib.block_size);
	p[39] = ilog2(se_cmd->se_dev->dev_attrib.block_size);
#else
	/*
