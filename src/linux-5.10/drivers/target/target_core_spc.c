...
#include "qnap/target_core_qspc.h"
#include "qnap/target_core_qsbc.h"
#include "qnap/target_core_qodx.h"
#include "qnap/target_core_qodx_scsi.h"
#endif

#ifdef CONFIG_MACH_QNAPTS
static int do_qnap_sbc_modesense_lbp(
	struct se_cmd *cmd, 
	u8 pc, 
	unsigned char *p
	)
{
	if (cmd->cmd_dr.spc_ops.spc_modesense_lbp)
		return cmd->cmd_dr.spc_ops.spc_modesense_lbp(cmd, pc, p);
	return 0;
}

static int do_qnap_spc_modesense_caching(
	struct se_cmd *cmd, 
	u8 pc, 
	unsigned char *p
	)
{
	if (cmd->cmd_dr.spc_ops.spc_modesense_caching)
		return cmd->cmd_dr.spc_ops.spc_modesense_caching(cmd, pc, p);
	return 0;
}

static sense_reason_t do_qnap_sbc_spc_emulate_evpd_b2(
	struct se_cmd *cmd, 
	unsigned char *buf
	)
{
	if (cmd->cmd_dr.spc_ops.spc_evpd_b2)
		return cmd->cmd_dr.spc_ops.spc_evpd_b2(cmd, buf);
	return 0;
}

static sense_reason_t _qnap_spc_odx_emulate_evpd_8f(
	struct se_cmd *cmd, 
	unsigned char *buf
	)
{
	if (cmd->cmd_dr.spc_ops.spc_odx_ops.sbc_evpd_8f)
		return cmd->cmd_dr.spc_ops.spc_odx_ops.sbc_evpd_8f(cmd, buf);
	return 0;
}
...
spc_emulate_inquiry_std(struct se_cmd *cmd, unsigned char *buf)
{
...
#ifdef CONFIG_MACH_QNAPTS
	/*
	 * spc4r37a, p354, VERSION DESCRIPTOR
	 * It is recommened that the first version is
	...
	/* Shall return spaces(0x20) instead of zeros in ASCII field */
	for (i = 8; i < 58; i++)
...
#ifdef CONFIG_MACH_QNAPTS
	if(likely(strcmp(dev->dev_dr.dev_naa, "3.8.1"))) { 
		/* BUG 29894
		 * We have three dev_naa type: 
		 * (1) legacy 
		 * (2) 3.8.1
		 * (3) and, qnap 
		 *
		 * For compatible issue, we shall use old type method
		 * to create naa body when naa hdr is qnap (new style)
		 * or legacy (old style). For others, we go new style to create
		 * naa body
		 */
		qlib_make_naa_6h_body_old_style(p, buf);
		return;
	}
#endif
	/*
...
spc_emulate_evpd_83(struct se_cmd *cmd, unsigned char *buf)
{
	...
	if(!strcmp(dev->dev_dr.dev_naa, "qnap")) {
		qlib_make_naa_6h_hdr_new_style(&buf[off]);
		off += 3;
		/*
...
static struct {
	...
	/* take care this !!
	 * the page code in supported vpd lists shall be reported in ascending
...
#ifdef CONFIG_MACH_QNAPTS
	{ .page = 0x8f, .emulate = _qnap_spc_odx_emulate_evpd_8f },
#endif
	{ .page = 0xb0, .emulate = spc_emulate_evpd_b0 },
	{ .page = 0xb1, .emulate = spc_emulate_evpd_b1 },
#ifdef CONFIG_MACH_QNAPTS
	{ .page = 0xb2, .emulate = do_qnap_sbc_spc_emulate_evpd_b2 },
#else
...
#ifdef CONFIG_MACH_QNAPTS
	{ .page = 0x1c, .subpage = 0x02, .emulate = do_qnap_sbc_modesense_lbp },
	{ .page = 0x08, .subpage = 0x00, .emulate = do_qnap_spc_modesense_caching },
#else
...
spc_parse_cdb(struct se_cmd *cmd, unsigned int *size)
{
	...
	case MODE_SELECT:
		*size = cdb[4];
#ifdef CONFIG_MACH_QNAPTS
		if (cmd->cmd_dr.spc_ops.spc_modeselect) {
			cmd->execute_cmd = cmd->cmd_dr.spc_ops.spc_modeselect;
			break;
		}
#endif
	...
	case LOG_SENSE:
		*size = get_unaligned_be16(&cdb[7]);
#ifdef CONFIG_MACH_QNAPTS
		if ((cdb[0] == LOG_SENSE) && cmd->cmd_dr.spc_ops.spc_logsense)
		{
			cmd->execute_cmd = cmd->cmd_dr.spc_ops.spc_logsense;
			break;
		}
#endif
		break;
	...
	case EXTENDED_COPY:
		*size = get_unaligned_be32(&cdb[10]);
		...
		if (IS_TPC_SCSI_PT_OP(cdb[1])) {
			if (cmd->cmd_dr.sbc_ops.sbc_odx_ops.sbc_odx_pt) {
				cmd->execute_cmd = 
					cmd->cmd_dr.sbc_ops.sbc_odx_ops.sbc_odx_pt;
				break;
			}
			return TCM_UNSUPPORTED_SCSI_OPCODE;
			
		} else if (IS_TPC_SCSI_WUT_OP(cdb[1])) {
			if (cmd->cmd_dr.sbc_ops.sbc_odx_ops.sbc_odx_wut) {
				cmd->execute_cmd = 
					cmd->cmd_dr.sbc_ops.sbc_odx_ops.sbc_odx_wut;
				break;
			}
			return TCM_UNSUPPORTED_SCSI_OPCODE;
		} else
	...
	case RECEIVE_COPY_RESULTS:
		*size = get_unaligned_be32(&cdb[10]);
		...
		if (IS_TPC_SCSI_RRTI_OP(cdb[1])) {
			if (cmd->cmd_dr.sbc_ops.sbc_odx_ops.sbc_odx_rrti) {
				cmd->execute_cmd = 
					cmd->cmd_dr.sbc_ops.sbc_odx_ops.sbc_odx_rrti;
				break;
			}
			return TCM_UNSUPPORTED_SCSI_OPCODE;
		}
