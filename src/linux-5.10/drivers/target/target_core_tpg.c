		...
		const unsigned char *initiatorname)
{
	...
	/* 2009/09/23 Nike Chen add for default initiator */
	if (tpg->default_acl)
		queue_depth = tpg->default_acl->queue_depth;
	else
#endif
#endif
...
void core_tpg_del_initiator_node_acl(struct se_node_acl *acl)
{
	...
	else if (!strcmp(acl->initiatorname, DEFAULT_INITIATOR))
		; // TODO: Should iSCSI do the same thing?
	else if (tpg->default_acl) {
		/*
		 * Do not kill the existing sessions.
		 * Change the node acl to dynamic node acl.
		 */
		bool get_session = false;
		struct se_session *sess;
		unsigned long flags;
