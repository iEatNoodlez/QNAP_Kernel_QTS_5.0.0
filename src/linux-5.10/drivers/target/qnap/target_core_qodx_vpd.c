...
/**/
static int _qnap_tpc_vpd_get_total_blkdev_rod_limits_desc_len(u16 *len);
static int _qnap_tpc_vpd_build_blkdev_rod_limits_desc(
       TPC_DESC_TYPE_CODE desc_code, u8 *desc_ptr, u32 blk_size);

static int _qnap_tpc_vpd_get_total_supported_cmds_len(u16 *len);
static int _qnap_tpc_vpd_build_supported_cmds(
	TPC_DESC_TYPE_CODE desc_code, u8 *desc_ptr, u32 blk_size);

static int _qnap_tpc_vpd_get_total_gen_copy_op_len(u16 *len);
static int _qnap_tpc_vpd_build_gen_copy_op(TPC_DESC_TYPE_CODE desc_code, 
       u8 *desc_ptr, u32 blk_size);

static int _qnap_tpc_vpd_get_total_supported_descs_len(u16 *len);
static int _qnap_tpc_vpd_build_supported_descs(TPC_DESC_TYPE_CODE desc_code, 
       u8 *desc_ptr, u32 blk_size);

/* 0x00: LID1 extended copy
 * 0x10: populate token
 * 0x11: write using token
 */
u8 tpc_op_83h_sac[] = {0x00, 0x10, 0x11, 0xff};

/* 0x03: receive copy operating parameter
 * 0x07: receive rod token information
 */
u8 tpc_op_84h_sac[] = {0x03, 0x7, 0xff};

static struct {
	u8	op;
	u8	*sac;
} gTpcOpTable[2] = {
	{EXTENDED_COPY, tpc_op_83h_sac},
	{RECEIVE_COPY_RESULTS, tpc_op_84h_sac},
};
...
struct build_tpc_desc{
	...
	int (*build_desc)(TPC_DESC_TYPE_CODE, u8 *, u32);
...
struct build_tpc_desc gTpcDescTable[] = {
	...
	_qnap_tpc_vpd_get_total_blkdev_rod_limits_desc_len,
	_qnap_tpc_vpd_build_blkdev_rod_limits_desc 
	},
 
	/* This function is mandatory (SPC4R36, page 769) */
	{	 
	TPC_DESC_SUPPORTED_CMDS , 
	_qnap_tpc_vpd_get_total_supported_cmds_len ,
	_qnap_tpc_vpd_build_supported_cmds 
	},
	...
	_qnap_tpc_vpd_get_total_params_len ,
	_qnap_tpc_vpd_build_params 
	},  
#endif
	/*
	 * This function shall be mandatory if the EXTENDED COPY (LID4) or
	...
	_qnap_tpc_vpd_get_total_supported_descs_len ,
	_qnap_tpc_vpd_build_supported_descs 
	},
 #if 0
 	/*
 	 * This function shall be mandatory if the EXTENDED COPY (LID4) or
	...
	_qnap_tpc_vpd_get_total_supported_cscd_ids_len ,
	_qnap_tpc_vpd_build_supported_cscd_ids 
	},
#endif
	...
	_qnap_tpc_vpd_get_total_rod_token_feature_len ,
	_qnap_tpc_vpd_build_rod_token_feature 
	},
#endif
	...
	_qnap_tpc_vpd_get_total_supported_rod_len ,
	_qnap_tpc_vpd_build_supported_rod 
	},
#endif
	...
	_qnap_tpc_vpd_get_total_gen_copy_op_len ,
	_qnap_tpc_vpd_build_gen_copy_op 
	},
 
#if 0 // mark this cause of no way to verify this
	{
	TPC_DESC_STREAM_COPY_OP ,
	_qnap_tpc_vpd_get_total_stream_copy_op_len ,
	_qnap_tpc_vpd_build_stream_copy_op  
	},
	...
	{	 
	TPC_DESC_HOLD_DATA , 
	_qnap_tpc_vpd_get_total_hold_data_len ,
	_qnap_tpc_vpd_build_hold_data 
	},
#endif
	{   
	MAX_TPC_DESC_TYPE , 
...
static u16 _qnap_odx_vpd_fill_supported_desc_type_codes(
	u8 *p
	)
{
	u16 Count = 0, Index = 0;
...
static int _qnap_tpc_vpd_get_total_blkdev_rod_limits_desc_len(
	u16 *len
	)
{
...
static int _qnap_tpc_vpd_get_total_gen_copy_op_len(
	u16 *len
	)
{
	*len = 0;
...
#if 0
static int _qnap_tpc_vpd_get_total_params_len(
	u16 *len
	)
{
...
static int _qnap_tpc_vpd_get_total_supported_descs_len(
	u16 *len
	)
{
	u16 total_len = 0, pad_len = 0;

	total_len = __qnap_tpc_get_max_supported_cscd_desc_count();
	total_len += __qnap_tpc_get_max_supported_seg_desc_count();
...
#if 0
static int _qnap_tpc_vpd_get_total_supported_cscd_ids_len(
	u16 *len
	)
{
	u16 total_len = 0, pad_len = 0;
...
static int _qnap_tpc_vpd_get_total_supported_rod_len(
	u16 *len
	)
{
	u16 total_len = 0, pad_len = 0;
...
static int _qnap_tpc_vpd_get_total_stream_copy_op_len(
	u16 *len
	)
{
	*len = 0;
...
static int _qnap_tpc_vpd_get_total_hold_data_len(
	u16 *len
	)
{
	*len = 0;
...
static int _qnap_tpc_vpd_get_total_rod_token_feature_len(
	u16 *len
	)
{
	u16 total_len = 0, pad_len = 0;
...
static int _qnap_tpc_vpd_build_rod_token_feature(
	void *work_data,
	TPC_DESC_TYPE_CODE desc_code, 
	u8 *desc_ptr
	)
...
{
	u8 bs_order = ilog2(blk_size);

	put_unaligned_be16(desc_code, &desc_ptr[0]);
	put_unaligned_be16(0x0020, &desc_ptr[2]);
	put_unaligned_be16(__qnap_odx_get_max_supported_blk_dev_range(), &desc_ptr[10]);
...
static int _qnap_tpc_vpd_get_total_supported_cmds_len(
	u16 *len
	)
{
	u16 idx0 = 0, idx1 = 0;
	...
	for (idx0 = 0; idx0 < ARRAY_SIZE(gTpcOpTable); idx0++) {
		sac_table = gTpcOpTable[idx0].sac;
	
		/* to count the u8Index1 value, it is the length for list of
		 * supported service actions */
		for (idx1 = 0;; idx1++) {
			if (sac_table[idx1] == 0xff)
				break;
		}
		idx1 += 2;
		total_len += idx1;
...
{
	u8 *cmd_desc = NULL, *sac = NULL, *sac_table = NULL;
	u8 idx0 = 0, idx1 = 0;
	...
	for (idx0 = 0 ; idx0 <  ARRAY_SIZE(gTpcOpTable) ; idx0++){
		cmd_desc[0] = gTpcOpTable[idx0].op;
		sac_table  = gTpcOpTable[idx0].sac;

		for (idx1 = 0;; idx1++) {
			if (sac_table[idx1] == 0xff)
				break;
			sac[idx1] = sac_table[idx1];
		}
		 ...
		 */
		cmd_desc[1] = idx1;
		u16Len += (u16)(cmd_desc[1] + 2);

		/* (1) Go to next cmd supported desc pos
		 * (2) Go to next service action code list pos
		 */
		cmd_desc += (size_t)(cmd_desc[1] + 2);
		sac = &cmd_desc[2];
	}// end of (u8Index0 = 0; u8Index0 < MAX_TPC_CMD_INDEX; u8Index0++)

	/* fill the total cmd supported desc list length */
	desc_ptr[4] = (u8)u16Len;

	/* The supported commands for third-party descriptor should be
	 * a multiple of four */
	if ((u16Len + 5) & (0x03)){
		/* Caculate the pad length */
		u16PadLen = (((((u16Len+ 5) + 4) >> 2) << 2) - (u16Len+ 5));
		/* u16Len is length for commands supported list plus pad length */
		u16Len	+= u16PadLen;
	}

	/* one byte for command supported list length */
	put_unaligned_be16((u16)(u16Len + 1), &desc_ptr[2]);

	return 0;
}

static int _qnap_tpc_vpd_build_gen_copy_op(
	TPC_DESC_TYPE_CODE desc_code, 
	u8 *desc_ptr,
	u32 blk_size	
	)
	...
	)
{
	...
	CscdDescCount = __qnap_tpc_get_max_supported_cscd_desc_count();
	if (CscdDescCount == 0)
		return 1;

	SegDescCount = __qnap_tpc_get_max_supported_seg_desc_count();
	if (SegDescCount == 0)
		return 1;
	...
	put_unaligned_be32(__qnap_tpc_get_total_supported_desc_len(), &desc_ptr[12]);
	put_unaligned_be32(0x0, &desc_ptr[16]);
	return 0;
}
#endif

static int _qnap_tpc_vpd_build_supported_descs(
	TPC_DESC_TYPE_CODE desc_code,
	u8 *desc_ptr,
	u32 blk_size
	)
{
	...
	put_unaligned_be16((u16)desc_code, &desc_ptr[0]);
	Len = _qnap_odx_vpd_fill_supported_desc_type_codes(&desc_ptr[5]);
	desc_ptr[4] = (u8)Len;
...
#if 0
static int _qnap_tpc_vpd_build_supported_cscd_ids(
	TPC_DESC_TYPE_CODE desc_code, 
	u8 *desc_ptr,
	u32 blk_size
	)
...
static int _qnap_tpc_vpd_build_supported_rod(
	TPC_DESC_TYPE_CODE desc_code, 
	u8 *desc_ptr,
	u32 blk_size
	)
...
static int _qnap_tpc_vpd_build_stream_copy_op(
	void *work_data,
	TPC_DESC_TYPE_CODE desc_code, 
	u8 *desc_ptr
	)
...
static int _qnap_tpc_vpd_build_hold_data(
	TPC_DESC_TYPE_CODE desc_code, 
	u8 *desc_ptr,
	u32 blk_size
	)
...
int qnap_odx_vpd_emulate_evpd_8f_step1(
	u16 alloc_len
	)
{
	u16 total_len = 0, tmp_len, curr_len = 0, idx;
	int ret;

	/* subtract 4 bytes first and remain bytes is for 3rd-party copy 
	 * descriptor lists data 
	 */
	tmp_len = alloc_len - 4;
...
int qnap_odx_vpd_emulate_evpd_8f_step2(
	u8 *buffer, 
	u32 blk_size
	)
{
	...
	for (idx = 0 ;; idx++) {
		...
		ret = gTpcDescTable[idx].build_desc(gTpcDescTable[idx].desc_code, 
				desc_ptr, blk_size);
