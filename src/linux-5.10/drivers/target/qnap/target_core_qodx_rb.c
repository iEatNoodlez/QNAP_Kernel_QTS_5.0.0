...
/**/
static void _qnap_odx_rb_del_tpg_cmd(
	...
	)
{
	...
	while (cmd_n) {
		...
		do {
			...
			pr_warn("%s: tpc cmd (list id:0x%x) refcount not 1, "
				"it is still using by someone, wait next chance\n", 
...
static void _qnap_odx_rb_del_tpg_token(
	struct tpc_tpg_data *p
	)
{
	...
	while (token_n) {
		...
		do {
			...
			pr_warn("%s: tpc token (list id:0x%x) refcount not 1, "
				"it is still using by someone, wait next chance\n", 
				__func__, token_p->reg_data.list_id);
	...
	qnap_odxlib_free_token_lists(&del_list);

}

static void _qnap_odx_rb_del_tpg(
	struct tpc_tpg_data *p
	)
{
//	_qnap_odx_rb_del_tpg_cmd(p);
	_qnap_odx_rb_del_tpg_token(p);
}

static struct tpc_cmd_data *_qnap_odx_rb_cmd_alloc(
...
static struct tpc_tpg_data *_qnap_odx_rb_tpg_alloc(
	u64 tpg_id_hi,
	u64 tpg_id_lo
	)
{
...
static struct tpc_token_data *_qnap_odx_rb_token_find(
	struct tpc_tpg_data *tpg_p,
	u64 cmd_id_hi, 
	u64 cmd_id_lo, 
	u64 initiator_id_hi, 
...
{
	/* token_root_lock of the tpg_p must be locked before this call */
	...
	while (n) {
		...
		|| ((cmd_id_hi == reg_d->cmd_id_hi) 
			...
			)
			{
				...
				ret = __qnap_odxlib_is_token_invalid(data, &rc);
				if (ret != 0) {
					if (!monitor_cp_req)
						return NULL;
					else {
						pr_warn("%s: find token "
						"(id:0x%x, sac:0x%x), req comes "
						"from monitor cp\n", __func__,
						reg_d->list_id, reg_d->sac);
					}
				}

				pr_debug("%s: found token (0x%p), (id:0x%x) by "
					"cmd id(hi):0x%llx, cmd id(lo):0x%llx, "
					"initiator id(hi):0x%llx, "
					"initiator id(lo):0x%llx\n", 
					__func__, data, reg_d->list_id, 
...
static struct tpc_cmd_data *_qnap_odx_rb_cmd_find(
	struct tpc_tpg_data *tpg_p,
	struct __reg_data *reg_data, 
	bool skip_iid, 
	bool skip_sac
...
#if 0
			pr_info("passed type:%d\n", reg_data->cmd_type);
			pr_info("passed cmd_id_hi:0x%llx\n", reg_data->cmd_id_hi);
			pr_info("passed cmd_id_lo:0x%llx\n", reg_data->cmd_id_lo);
			pr_info("checked cmd_id_hi:0x%llx\n", tmp_reg_data->cmd_id_hi);
			pr_info("checked cmd_id_lo:0x%llx\n", tmp_reg_data->cmd_id_lo);
			pr_info("checked type:%d\n", tmp_reg_data->cmd_type);
#endif
			if ((reg_data->cmd_id_hi == tmp_reg_data->cmd_id_hi)
			&& (reg_data->cmd_id_lo == tmp_reg_data->cmd_id_lo)
...
struct tpc_tpg_data *_qnap_odx_rb_tpg_find(
	u64 tpg_id_hi, 
	u64 tpg_id_lo
	)
{
...
static bool _qnap_odx_rb_find_conflict_lba_in_token(
	...
	)
{
	...
	token_p = _qnap_odx_rb_token_find(tpg_p, cmd_id_hi, cmd_id_lo, 
	...
	)
{
	...
	pr_debug("%s: add token(0x%p) (list_id:0x%x), "
		"cmd id (high,low): (0x%x, 0x%x), "
		"initiator_id (high,low): (0x%x, 0x%x\n", __func__, token_p, 
		token_p->reg_data.list_id, data->cmd_id_hi,  data->cmd_id_lo, 
		data->initiator_id_hi, data->initiator_id_lo);
	...
	)
{
	...
	tpg_data = _qnap_odx_rb_tpg_alloc(tpg_id_hi, tpg_id_lo);
	if (!tpg_data) {
...
struct tpc_tpg_data *qnap_odx_rb_tpg_get(
	u64 tpg_id_hi, 
	u64 tpg_id_lo
	)
{
	...
	tpg_p = _qnap_odx_rb_tpg_find(tpg_id_hi, tpg_id_lo);
	if (tpg_p) {
	...
	)
{
	...
	data = _qnap_odx_rb_cmd_alloc(tpg_p, reg_data);
	if (!data)
		return NULL;
	...
	)
{
	...
	p = _qnap_odx_rb_cmd_find(tpg_p, reg_data, skip_iid, skip_sac);
	if (p)
...
int qnap_odx_rb_parse_conflict_token_range(
	struct tpc_tpg_data *tpg_p, 
	...
	)
{
			...
			token_n; token_n = rb_next(token_n)) 
	{
		...
		ret = __qnap_odxlib_is_token_invalid(token_p, &rc);
		if (ret != 0) {
			pr_debug("%s: warning, token (list id:0x%x, sac:0x%x) "
				"was NOT alive during to parse conflict "
				"lba range by cmd (cdb[0]:0x%x, cdb[1]:0x%x, "
				"desc index:%d). status:%d\n", __func__, 
				reg_d->list_id, reg_d->sac, cdb0, cdb1, 
				desc_idx, rc);
			continue;
		}
		...
		invalidate_token = _qnap_odx_rb_find_conflict_lba_in_token(
					token_p, lba, nr_blks);

		if (invalidate_token) {
			pr_warn("%s: WARNING, cmd range (cdb[0]:0x%x, cdb[1]:0x%x, "
				"desc idx:%d, lba:0x%llx, nr_blks:0x%x) "
				"conflicts with token (list id:0x%x, "
				"op_sac:0x%x). try to cancel token\n", __func__, 
...
int qnap_odx_rb_discard_all_tokens(
	struct tpc_tpg_data *tpg_p,
	struct __reg_data *reg_data,
	int discard_type,
	char *type_str
	)
{
	...
	if ((discard_type != DISCARD_TOKEN_LUN_RESET)
		&& (discard_type != DISCARD_TOKEN_I_T_NEXUS_LOSS)
	)
		return -EINVAL;
				...
				token_n; token_n = rb_next(token_n)) 
	{
		...
		if (discard_type == DISCARD_TOKEN_LUN_RESET) {
			/* find the device we want from NAA */
			if (memcmp(&reg_data->dev_info.naa[0], 
						&reg_d->dev_info.naa[0], NAA_LEN))
				continue;
		}

		if (discard_type == DISCARD_TOKEN_I_T_NEXUS_LOSS) {
			/* find i_t nexuss we want */
			if ((reg_data->initiator_id_hi != reg_d->initiator_id_hi)
			|| (reg_data->initiator_id_lo != reg_d->initiator_id_lo)
			|| (reg_data->tpg_id_hi != reg_d->tpg_id_hi)
			|| (reg_data->tpg_id_lo != reg_d->tpg_id_lo)
			)
				continue;			
		}
		...
		pr_warn("warning, %s to cancel token (list id:0x%x, "
			"op_sac:0x%x)\n", (type_str ? type_str : ""), 
			reg_d->list_id, reg_d->sac);
