...
/**/
static bool _qnap_odx_core_wut_check_wuzrt(
	struct rod_token_512b *token512b
	)
{
	u32 type;

	/* check what kind of WUT we want to run ... */
	type = get_unaligned_be32(&token512b->gen_data.type[0]);
	return (type == ROD_TYPE_BLK_DEV_ZERO);
}

static int _qnap_odx_core_rrti_s2_update_by_token(
...
static int _qnap_odx_core_rrti_s1_update_by_tc_data(
...
static int _qnap_odx_core_rrti_s2(
	...
	)
{
	...
	pr_debug("%s: cmd id (high,low): (0x%x, 0x%x), "
		"initiator_id (high,low): (0x%x, 0x%x)\n", __func__, 
		odx_wr->reg_data.cmd_id_hi,  odx_wr->reg_data.cmd_id_lo, 
		odx_wr->reg_data.initiator_id_hi, odx_wr->reg_data.initiator_id_lo);

	token_p = qnap_odx_rb_token_get(odx_wr->tpg_p, odx_wr->reg_data.cmd_id_hi,
	...
	if (!token_p) {	
		pr_warn("%s: RRTI s2: not found token\n", __func__);
	...
	if (alloc_len_in_cdb == 0){
		 ...
		 * field set to zero
		 */
		pr_warn("%s: RRTI s2: warning, discard token (list id:0x%x, "
			"sac:0x%x) when alloc_len_in_cdb is zero ... \n", 
			__func__, token_p->reg_data.list_id, token_p->reg_data.sac);

		/* delete token */
		__qnap_odxlib_cmpxchg_token_status(token_p, O_TOKEN_ALIVE, 
								O_TOKEN_CANCELLED);
		do {
			count = qnap_odx_rb_token_put(token_p);
			if (count == 1)
				break;

			pr_warn("%s: RRTI s2: token refcount not 1, it is still "
				"using by someone, wait next chance\n", __func__);
			cpu_relax();
		} while (1);
	 ...
	 */
	ret = __qnap_odxlib_is_token_invalid(token_p, &odx_wr->rc);
	if (ret == -ENODEV) {
		pr_warn("%s: RRTI s2: warning token (list id:0x%x, sac:0x%x) "
			"was NOT alive, rc:%d\n", __func__, 
	...
	ret = _qnap_odx_core_rrti_s2_update_by_token(token_p, odx_wr->buff, true, 
		       alloc_len_in_cdb);
	...
	qnap_odx_rb_token_put(token_p);
	return ret;
}

static int _qnap_odx_core_rrti_s1(
	struct odx_work_request *odx_wr
	)
{
	 ...
	 * list id)
	 */
	tc_p = qnap_odx_rb_cmd_get(odx_wr->tpg_p, &odx_wr->reg_data, false, true);
	if (!tc_p) {
		/* if can't get command, the cmd may be completed, we will try
		 * to search from token tree again
		 */
		pr_debug("%s: odx (list id:0x%x) not found in RRTI phase 1, " 
	...
	status = __qnap_odx_get_tc_data_status(tc_p);
	...
	if (T_CMD_IS_NOT_START(status)){
		 ...
		 * report error
		 */
		pr_warn("%s: odx (list id:0x%x, sac:0x%x) not "
			"start work yet\n", __func__, tc_p->reg_data.list_id, 
			tc_p->reg_data.sac);
		odx_wr->rc = RC_INVALID_PARAMETER_LIST;
		ret = -EINVAL;
	}
	...
	)
	{
		...
		pr_warn("%s: odx (list id:0x%x, sac:0x%x) is in "
			"process now\n", __func__, tc_p->reg_data.list_id, 
			tc_p->reg_data.sac);

		ret = _qnap_odx_core_rrti_s1_update_by_tc_data(tc_p, 
					odx_wr->buff, status);
		if (ret != 0)
			odx_wr->rc = RC_INVALID_PARAMETER_LIST;
		else
	...
	)
	{
		 ...
		 * go to step 2
		 */
		pr_debug("%s: odx (list id:0x%x, sac:0x%x) was "
		...
		pr_warn("%s: odx (list id:0x%x, sac:0x%x) is unknown status\n", 
...
static int _qnap_odx_core_pt_io(
	struct odx_work_request *odx_wr, 
	...
	)
{
	...
	desc_counts = __qnap_odx_get_desc_counts(get_unaligned_be16(
	...
	for (index = 0; index < desc_counts; index++){
		...
		while (alloc_nr_blks){
			...
			br = qnap_odxlib_alloc_br();
			if (!br) {
				qnap_odxlib_free_br_lists(&br_list);
			...
			__qnap_odxlib_lock_update_tc_cmd_transfer_count(
				odx_wr->tc_p, (sector_t)br->nr_blks);
	...
	)
{
	...
	*go_pt = false;
	...
	if (p->rtv){
		...
		if(qnap_odxlib_check_valid_supported_rod_type(rod_type)
		|| (rod_type == ROD_TYPE_BLK_DEV_ZERO)
		)
		{
			pr_err("%s: unsupported token type:0x%x in PT parameter "
				"data (id:0x%x, sac:0x%x)\n", __func__, rod_type, 
	...
	{
		pr_err("%s: token data length or blk dev range desc "
	...
	if (qnap_odxlib_check_same_lba_in_desc_list(s, desc_counts) != 0){
		pr_err("%s: found same LBA in blk dev range "
	...
	if (qnap_odxlib_check_overlap_lba_in_desc_list(s, desc_counts) != 0){
		pr_err("%s: found overlapped LBA in blk dev range "
			"descriptor (id:0x%x, sac:0x%x)\n", __func__,
			reg_data->list_id, reg_data->sac);
	...
	if (qnap_odxlib_check_max_lba_in_desc_list(s, reg_data->dev_info.dev_max_lba,
			desc_counts) != 0){
		pr_err("%s: found LBA in blk dev range descriptor execeeds "
			"the max LBA of device (id:0x%x, sac:0x%x)\n", 
			__func__, reg_data->list_id, reg_data->sac);
	...
	if (get_unaligned_be32(&p->inactivity_timeout[0]) > MAX_INACTIVITY_TIMEOUT) {
		pr_err("%s: timeout value is larger than max-timeout "
	...
	if (desc_counts > __qnap_odx_get_max_supported_blk_dev_range()){
		pr_err("%s: blk dev range descriptor length exceeds "
	...
	all_nr_blks = __qnap_odx_get_total_nr_blks_by_desc(s, desc_counts);
	...
	if (all_nr_blks > (MAX_TRANSFER_SIZE_IN_BYTES >> bs_order)){
		pr_err("%s: sum of contents in blk dev range descriptor "
	...
	)
{
	 ...
	 *    discard.
	 */
	qnap_odxlib_is_duplicated_token_then_delete(
		odx_wr->tpg_p, 
	...
	p = qnap_odxlib_token_data_alloc(odx_wr, &odx_wr->reg_data);
	if (!p) {
	...
	__qnap_odxlib_update_tc_data_status(odx_wr->tc_p, T_CMD_IS_STARTING_IN_FG);

	/* main code for Pupulate Token */
	ret = _qnap_odx_core_pt_io(odx_wr, p);
	if (ret != 0) {
		qnap_odxlib_token_data_free(p);
		__qnap_odxlib_update_tc_data_status(odx_wr->tc_p, T_CMD_COMPLETED_W_ERR);
		return ret;
	}

	/**/ 
	__qnap_odxlib_update_tc_data_status(odx_wr->tc_p, T_CMD_COMPLETED_WO_ERR);
	p->cp_op_status = OP_COMPLETED_WO_ERR;

	qnap_odxlib_set_obj_completion_status(p);
	qnap_odxlib_set_obj_op_counter(p);

	p->segs_processed = odx_wr->tc_p->segs_processed = 0;
	p->transfer_count = __qnap_odxlib_lock_get_tc_cmd_transfer_count(odx_wr->tc_p);
	p->create_time = get_jiffies_64();

	/* it is tricky to report transfer count even if we don't need do
	 * any I/O for PT
	 */
	odx_wr->transfer_counts = p->transfer_count;

	qnap_odxlib_build_512b_token_data(odx_wr, p);
	__qnap_odxlib_update_token_status(p, O_TOKEN_ALIVE);

	/* add new token into tree */
	qnap_odx_rb_token_add(odx_wr->tpg_p, p);

	/* fire timer */
	p->token_timeout = get_unaligned_be32(&param->inactivity_timeout[0]);
	qnap_odxlib_start_token_timer(p, p->token_timeout);
	return 0;

}

int qnap_odx_core_wut_pre_work(
	struct odx_work_request *odx_wr,
	bool *go_wut
	)
{
	...
	*go_wut = false;
	...
	if (get_unaligned_be32(&cdb[10]) < 552){
		pr_err("%s: allocation len is smaller than 552 bytes "
			"(id:0x%x, sac:0x%x)\n", __func__,  
	...
	if (get_unaligned_be16(&p->token_data_len[0]) < 550){
		pr_err("%s: avaiable data len in param is smaller than "
	...
	if (get_unaligned_be16(&p->blkdev_range_desc_len[0]) < 0x10){
		pr_err("%s: blk dev range desc length is not enougth "
			"(id:0x%x, sac:0x%x)\n", __func__,
			odx_wr->reg_data.list_id, odx_wr->reg_data.sac);
	...
	if (get_unaligned_be16(&token->token_len[0]) != 0x1f8 ){
		pr_err("%s: token length is NOT 0x1f8 bytes (id:0x%x, sac:0x%x)\n", 
			__func__, odx_wr->reg_data.list_id, odx_wr->reg_data.sac);

		odx_wr->rc = RC_INVALID_TOKEN_OP_AND_INVALID_TOKEN_LEN;
		return -EINVAL;
	}

	if (qnap_odxlib_check_valid_supported_rod_type(
		get_unaligned_be32(&token->type[0])) != 0)
	{
		pr_err("%s: unsupported token type (id:0x%x, sac:0x%x)\n", 
			__func__, odx_wr->reg_data.list_id, odx_wr->reg_data.sac);
	...
	if (qnap_odxlib_check_same_lba_in_desc_list(s, desc_counts) != 0){
		pr_err("%s: found same LBA in blk dev range descriptor "
			"(id:0x%x, sac:0x%x)\n", __func__,
			odx_wr->reg_data.list_id, odx_wr->reg_data.sac);
	...
	if (qnap_odxlib_check_max_lba_in_desc_list(s, 
	...
	if (desc_counts > __qnap_odx_get_max_supported_blk_dev_range()){
		pr_err("%s: error !! blk dev range descriptor length exceeds "
			"the max value (id:0x%x, sac:0x%x)\n", 
			__func__, odx_wr->reg_data.list_id, odx_wr->reg_data.sac);
	...
	*/
	...
	if (all_nr_blks > (MAX_TRANSFER_SIZE_IN_BYTES >> bs_order)){
		pr_err("%s: error !! sum of contents in blk dev range descriptor "
			"length exceeds the max value (id:0x%x, sac:0x%x)\n", 
			__func__, odx_wr->reg_data.list_id, odx_wr->reg_data.sac);
	...
	)
{
	...
	run_wuzrt = _qnap_odx_core_wut_check_wuzrt(tmp_s_token512b_p);
	...
	if (run_wuzrt == false) {
		/* if it is NOT Write Using Zero ROD Token, try get token / tpg
		 * data of source from token tree 
		 */
		ret = qnap_odxlib_wut_get_src_tpg_p_and_token_p(tmp_s_token512b_p, 
			&s_tpg_p, &s_token_p, d_token_p, &odx_wr->rc);

		if (ret)
			goto fail_before_wut;

		d_token_p->token_timeout = s_token_p->token_timeout;

		/* Before to do WUT, let's investigate some conditions again */	
		s_nr_blks = qnap_odxlib_lock_get_nr_blks_by_s_token(s_token_p);
		off_to_rod = get_unaligned_be64(&p->off_into_rod[0]);

		ret = qnap_odxlib_wut_check_rod_off_before_wut_io(
				s_token_p, d_token_p, s_nr_blks, d_nr_blks, 
				off_to_rod, &odx_wr->rc);
fail_before_wut:
		if (ret) {
			if (s_token_p)
				qnap_odx_rb_token_put(s_token_p);
			if (s_tpg_p)
				qnap_odx_rb_tpg_put(s_tpg_p);

			qnap_odxlib_token_data_free(d_token_p);
	...
	qnap_odxlib_wut_io(odx_wr, s_token_p, d_token_p, run_wuzrt);

	transfer_blks = __qnap_odxlib_lock_get_tc_cmd_transfer_count(odx_wr->tc_p);
	pr_debug("%s: transfer_blks:0x%llx\n", __func__, (unsigned long long)transfer_blks);
	...
	if (d_nr_blks == transfer_blks) {
	...
	else if (__qnap_odxlib_cmd_was_asked_drop(odx_wr->tc_p)) {
		...
		if (odx_wr->rc == RC_NO_SPACE_WRITE_PROTECT) {
			...
			if (transfer_blks) {
				pr_debug(
				"%s: COPY_ABORT_DATA_UNDERRUN_COPY_TARGET: "
				"list id:0x%x, d_nr_blks:0x%llx, "
				"transfer_blks:0x%llx,\n", 
				__func__, reg_data->list_id,
				...
				pr_debug(
				"%s: ILLEGAL_REQ_DATA_UNDERRUN_COPY_TARGET: "
				"list id:0x%x, d_nr_blks:0x%llx, "
				"transfer_blks:0x%llx,\n", 
				__func__, reg_data->list_id,
				(unsigned long long)d_nr_blks,
	...
	qnap_odxlib_set_obj_completion_status(d_token_p);
	qnap_odxlib_set_obj_op_counter(d_token_p);
	...
	|| (odx_wr->rc == RC_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED)
	)
	{
		if (odx_wr->rc == RC_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED) {
			curr_tc_status = T_CMD_COMPLETED_WO_ERR;
			ret = 0;
		} else {
		...
		qnap_odxlib_build_token_sense_data(d_token_p, odx_wr->rc, 0, 0);
	}

	__qnap_odxlib_update_tc_data_status(odx_wr->tc_p, curr_tc_status);
	__qnap_odxlib_update_token_status(d_token_p, O_TOKEN_ALIVE);	
	...
	/* add new token into tree */
	qnap_odx_rb_token_add(odx_wr->tpg_p, d_token_p);

	/* fire timer */
	qnap_odxlib_start_token_timer(d_token_p, d_token_p->token_timeout);
	return ret;

}

int qnap_odx_core_rrti(
	struct odx_work_request *odx_wr
	)
{
	int ret;

	ret = _qnap_odx_core_rrti_s1(odx_wr);
	if ((ret == -ENODEV) && (odx_wr->rc == RC_GOOD))
		ret = _qnap_odx_core_rrti_s2(odx_wr);
