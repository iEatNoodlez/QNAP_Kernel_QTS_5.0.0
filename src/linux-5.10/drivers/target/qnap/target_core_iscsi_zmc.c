	...
	)
{
	...
	if (data_len < (1 << SECTOR_SHIFT))
		tmp_len = data_len;
	else
		/* must be multipled by 512b */
		tmp_len = ((data_len >> SECTOR_SHIFT) << SECTOR_SHIFT);
	...
	)
{
	...
	for (rec_idx = 0; ;rec_idx++) {
		...
		WARN_ON(total & ((1 << SECTOR_SHIFT) - 1));
		WARN_ON(total != sgl_rec[rec_idx].total_len);

		block_lba += (total >> SECTOR_SHIFT);
