	...
	struct se_session *se_sess)
{
	...
	list_for_each_entry_safe(se_cmd, next, &dev->state_list, state_list) {
		...
		if (!rc) {
			printk("ABORT TASK TMR: non-zero kref_get_unless_zero\n");
			continue;
		}
...
#ifdef CONFIG_MACH_QNAPTS
		if (se_cmd->cmd_dr.io_ops.tmr_abort_task) {
			se_cmd->cmd_dr.io_ops.tmr_abort_task(se_cmd, 
				((se_cmd->se_sess) ? se_cmd->se_sess->se_tpg : NULL));
		}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		qnap_transport_show_cmd("Wait ABORT_TASK: ", se_cmd);
#endif
		target_put_cmd_and_wait(se_cmd);

#ifdef CONFIG_MACH_QNAPTS
		qnap_transport_show_cmd("Done Wait ABORT_TASK: ", se_cmd);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		if (cmd->cmd_dr.io_ops.tmr_lunreset_drain_state) {
			cmd->cmd_dr.io_ops.tmr_lunreset_drain_state(cmd,
					((tmr_sess) ? tmr_sess->se_tpg : NULL));
		}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		qnap_transport_show_cmd("Wait LUN_RESET: ", cmd);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		qnap_transport_show_cmd("Done Wait LUN_RESET: ", cmd);
#endif
	}
}
