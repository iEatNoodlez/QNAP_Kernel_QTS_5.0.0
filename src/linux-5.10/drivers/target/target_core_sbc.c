...
#ifdef CONFIG_MACH_QNAPTS
	/* reset to NULL if it is the last callback */
						 ...
						 int *post_ret)
{
	...
	if (zmc_ops && zmc_ops->sbc_ats_callback) {
		rc = zmc_ops->sbc_ats_callback(cmd, success, post_ret);
		if (!rc || (rc != -ENOTSUPP))
	...
	if (qlib_thin_lun(&dev->dev_dr)) {
		if (cmd->cmd_dr.work_io_err == TCM_SPACE_ALLOCATION_FAILED_WRITE_PROTECT)
...
sbc_parse_cdb(struct se_cmd *cmd, struct sbc_ops *ops)
{
	...
	case SERVICE_ACTION_IN_16:
		...
		case SAI_GET_LBA_STATUS:
			if (cmd->cmd_dr.sbc_ops.sbc_get_lba_status)
				cmd->execute_cmd = cmd->cmd_dr.sbc_ops.sbc_get_lba_status;
			break;
#endif
		default:
...
#ifdef CONFIG_MACH_QNAPTS
			if (cmd->cmd_dr.sbc_ops.sbc_sync_ops.sbc_sync_cache) { 
				cmd->cmd_dr.sbc_ops.sbc_sync_ops.orig_sbc_sync_cache = 
					ops->execute_sync_cache;			
				cmd->execute_cmd = 
					cmd->cmd_dr.sbc_ops.sbc_sync_ops.sbc_sync_cache;
			}
			else			
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		if (cmd->cmd_dr.sbc_ops.sbc_unmap_ops.sbc_unmap) {
			cmd->cmd_dr.sbc_ops.sbc_unmap_ops.orig_sbc_unmap = 
				sbc_execute_unmap;
			cmd->execute_cmd = 
				cmd->cmd_dr.sbc_ops.sbc_unmap_ops.sbc_unmap;
		}
		else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		if (cmd->cmd_dr.sbc_ops.sbc_verify_10_16) {
			cmd->execute_cmd = cmd->cmd_dr.sbc_ops.sbc_verify_10_16;
			break;
		}
#endif
		cmd->execute_cmd = sbc_emulate_noop;
...
#ifdef CONFIG_MACH_QNAPTS
		if (qnap_transport_check_cmd_aborted(cmd, false))
			goto err;
#endif
		ptr += 16;
		size -= 16;
...
err:
	...
	qnap_transport_check_dev_under_thin_threshold(cmd->se_dev);
