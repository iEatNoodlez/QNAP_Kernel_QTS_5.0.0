...
#ifdef CONFIG_MACH_QNAPTS
static void _qnap_transport_check_aborted_on_diff_it_nexus(
	struct se_cmd *cmd,
	bool *tas_run,
	bool *tas_path,
	bool *tas_resp
	)
{
	...
	if (qnap_transport_check_cmd_aborted_by_release_conn(cmd, false)) {
		*tas_run = false;
		return;
	}

	set_bit(QNAP_CMD_T_IN_TMR_ABORT_WQ, &cmd->cmd_dr.cmd_t_state);

	/*
	 * sam4r14.pdf, page 89
	 *
	 * When a device server receives a command or task management function on
	 * an I_T nexus that causes commands on the same I_T nexus to be aborted, 
	 * the device server shall not return any notification that commands have
	 * been aborted other than:
	 *    a) a completion response for the command or task management function
	 *       that caused the command(s) to be aborted; and
	 *    b) notification(s) associated with related effects of the command 
	 *       or task management function (e.g., a reset unit attention condition).
	 *
	 * When a device server receives a command or task management function on
	 * an I_T nexus that causes commands on other I_T nexuses to be aborted, 
	 * the device server shall return any notifications for those commands 
	 * based on the setting of the TAS bit in the Control mode page (see SPC-4):
	 *    a) if the TAS bit is set to zero, the device server:
	 *       A) shall not return status for the commands that were aborted;
	 *          and
	 *       B) shall establish a unit attention condition for the SCSI 
	 *          initiator port associated with each I_T nexus containing 
	 *          commands that were aborted with an additional sense code set
	 *          as defined in table 39 and table 40;
	 * or
	 *    b) if the TAS bit is set to one, the device server:
	 *       A) shall complete each aborted command with a TASK ABORTED status;
	 *          and
	 *       B) shall not establish a unit attention condition for this reason
	 *	 
	 */

	if (qnap_transport_tmr_get_code(cmd)) {
		ret = atomic_read(&cmd->cmd_dr.tmr_diff_it_nexus);
		if (ret >= 0) {
			*tas_path = true;
			*tas_resp = ((ret == 1) ? true : false);
			qnap_transport_show_set_task_aborted_msg(cmd, (ret == 1));
		} else
			pr_warn("WARN: result for it_nexus checking is not correct\n");
	}
...
static void target_complete_nacl(struct kref *kref)
{
	...
	qnap_target_free_qnap_se_nacl(nacl->map_qnap_nacl,
		&se_tpg->se_nacl_dr);
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	/* we need to take care some cases to use callback ... */
	bool to_exit = false;

	cmd->b_se_state |= BS_SE_CMD_COMP_FAILURE_WORK;
	cmd->b_se_state_last = BS_SE_CMD_COMP_FAILURE_WORK;
	cmd->complete_jiffies = jiffies;

	if (cmd->cmd_dr.work_io_err != TCM_NO_SENSE) {
		switch (cmd->cmd_dr.work_io_err) {
...
void transport_copy_sense_to_cmd(struct se_cmd *cmd, unsigned char *sense)
{
	...
	cmd->b_se_state |= BS_SE_CMD_EXEC_COMPLETE;
	cmd->b_se_state_last = BS_SE_CMD_EXEC_COMPLETE;
	...
	cmd->b_se_state |= BS_SE_CMD_Q_TO_COMP_WORK;
	cmd->b_se_state_last = BS_SE_CMD_Q_TO_COMP_WORK;
...
#ifdef CONFIG_MACH_QNAPTS
	bool tas_run = true, tas_path = false, tas_resp = false;
	_qnap_transport_check_aborted_on_diff_it_nexus(cmd, &tas_run, &tas_path, 
				&tas_resp);
	if (!tas_run) {
...
#ifdef CONFIG_MACH_QNAPTS
			/* Actually, we think there is something wrong for
			 * LIO code here ... :((
			 * For more details, please refer the comments in 
			 * _qnap_transport_check_aborted_on_diff_it_nexus()
			 */
			if (!tas_resp)
				cmd->scsi_status &= ~(SAM_STAT_TASK_ABORTED);
#endif
...
static bool target_cmd_interrupted(struct se_cmd *cmd)
{
	...
	if (
#ifdef CONFIG_MACH_QNAPTS
	qnap_transport_check_cmd_aborted_by_tmr(cmd, true) ||
#endif
...
#ifdef CONFIG_MACH_QNAPTS
			/* The cmd->transport_complete_callback will be reset to
			 * NULL after the last callback was called. 
			 * Actually (especially, for file buffer i/o), it is 
			 * better to call queue_work() after to return from 
			 * 1st callback (the work may be fired soon ..)
			 */
#if 0 /* for QTS00000-10589, remove the check, otherwise the aborted command will not go to target_abort_work() and command will stuck */
			/* for aysnc i/o (like bio), callback may not called soon */
			if (cmd->transport_complete_callback)
				return true;		 
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	qnap_transport_check_cmd_aborted_by_release_conn(cmd, true) ||
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_transport_hit_read_deletable(&cmd->se_dev->dev_dr, cmd->t_task_cdb)) 
	{
		scsi_status = SAM_STAT_CHECK_CONDITION;
		success = false;
		cmd->scsi_status = scsi_status;
		/* force set to no-space case */		
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_TARGET_PERF_FEATURE
	if (qnap_target_perf_enabled() && cmd->run_complete_ok_directly) {
		cmd->work.func(&cmd->work);
		return;
	}
#endif
#endif
...
{
	cmd->b_se_state = BS_SE_CMD_INIT;
	cmd->b_se_state_last = BS_SE_CMD_INIT;
	cmd->start_jiffies = jiffies;
	...
	qnap_transport_init_se_cmd_dr(cmd);

	if (qnap_zmc_init_data)
		qnap_zmc_init_data(cmd);

	/* take care this !!
	 * remember the tfo may NOT contain YOUR own interface (i.e. xcopy code)
	 */
	qnap_odx_transport_init(&cmd->odx_dr, tfo->q_tf_ops);
#endif
}
...
target_cmd_init_cdb(struct se_cmd *cmd, unsigned char *cdb)
{
	...
	cmd->b_se_state |= BS_SE_CMD_SET_CMD_CDB;
	cmd->b_se_state_last = BS_SE_CMD_SET_CMD_CDB;
...
void __target_execute_cmd(struct se_cmd *cmd, bool do_checks)
{
	...
	cmd->b_se_state |= BS_SE_CMD_EXEC_CMD_2;
	cmd->b_se_state_last = BS_SE_CMD_EXEC_CMD_2;
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef QNAP_TARGET_PERF_FEATURE
	/* not go to random task handler, we do it in
	 * qnap_transport_perf_enqueue_cmd() */
	if (qnap_target_perf_enabled())
		__ret = -EIO;
	else
#endif
		__ret = qnap_target_exec_random_task(cmd); /* 0: put cmd to wq already */

	if (!__ret)
		return;

	/* read_deletable feature coworks with storage manager and check cond
	 * was hit or not before issue command
	 */
	if (qnap_transport_hit_read_deletable(&cmd->se_dev->dev_dr,
			cmd->t_task_cdb))
	{
	...
	if (zmc_ops && zmc_ops->prep_final_sgl) {
		__ret = zmc_ops->prep_final_sgl(cmd);
		if (__ret && (__ret != -ENOTSUPP) && (__ret != -EPERM)) {
			ret = TCM_LOGICAL_UNIT_COMMUNICATION_FAILURE;
			goto err;
		}
	}
...
void target_execute_cmd(struct se_cmd *cmd)
{
	...
	cmd->b_se_state |= BS_SE_CMD_EXEC_CMD;
	cmd->b_se_state_last = BS_SE_CMD_EXEC_CMD;	 

#ifdef CONFIG_MACH_QNAPTS
	if (qnap_odx_pre_check_odx(cmd) == -EBUSY) {
...
static void target_complete_ok_work(struct work_struct *work)
{
	...
	cmd->b_se_state |= BS_SE_CMD_COMP_OK_WORK;
	cmd->b_se_state_last = BS_SE_CMD_COMP_OK_WORK;
	cmd->complete_jiffies = jiffies;
...
void *transport_kmap_data_sg(struct se_cmd *cmd)
{
	...
	if (zmc_ops && zmc_ops->kmap_data_sg) {
		ret = zmc_ops->kmap_data_sg(cmd, cmd->data_length, &vmap_data);
		if ((ret != -ENOTSUPP) && (ret != -EPERM))
			return vmap_data;	
	}
#endif
	/*
...
void transport_kunmap_data_sg(struct se_cmd *cmd)
{
	...
	if (zmc_ops && zmc_ops->kunmap_data_sg) {
		ret = zmc_ops->kunmap_data_sg(cmd);
		if ((ret != -ENOTSUPP) && (ret != -EPERM))
			return;	
	}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		struct target_zmc_ops *zmc_ops = cmd->cmd_dr.zmc_ops;
		if (zmc_ops && zmc_ops->alloc_sgl) {
			ret = zmc_ops->alloc_sgl(cmd); 
			if (ret && (ret != -ENOTSUPP) && (ret != -EPERM))
				return TCM_LOGICAL_UNIT_COMMUNICATION_FAILURE;
		} else
			ret = -ENOTSUPP;
...
#ifdef QNAP_TARGET_PERF_FEATURE
			if (qnap_target_perf_enabled()) {
				qnap_transport_perf_exec_wt_cmd(cmd);
				return 0;
			}
		}
#endif
...
static void target_wait_free_cmd(struct se_cmd *cmd, bool *aborted, bool *tas)
{
	...
	cmd->b_se_state |= BS_SE_CMD_FREE;
	cmd->b_se_state_last = BS_SE_CMD_FREE;
...
int transport_generic_free_cmd(struct se_cmd *cmd, int wait_for_tasks)
{
	...
	if (wait_for_tasks){
		cmd->b_se_state |= BS_SE_CMD_FREE_1;
		cmd->b_se_state_last = BS_SE_CMD_FREE_1;
...
int target_get_sess_cmd(struct se_cmd *se_cmd, bool ack_kref)
{
	...
	se_cmd->b_se_state |= BS_SE_CMD_GET_SESS;
	se_cmd->b_se_state_last = BS_SE_CMD_GET_SESS;
...
{
	se_cmd->b_se_state |= BS_SE_CMD_PUT_SESS;
	se_cmd->b_se_state_last = BS_SE_CMD_PUT_SESS;
...
void target_show_cmd(const char *pfx, struct se_cmd *cmd)
{
	...
	if (!(cmd->se_cmd_flags & SCF_SCSI_TMR_CDB)) {
		pr_info("%scmd %#02x:%#02x with tag %#llx dir %s i_state %d t_state %s len %d refcnt %d transport_state %s "
			 "bs_state:0x%llx(0x%llx), ptime:%ld(s)\n",
			 ...
			 kref_read(&cmd->cmd_kref), ts_str,
 			 cmd->b_se_state, cmd->b_se_state_last,
 			 ((jiffies - cmd->start_jiffies) / HZ));
	} else {
		pr_info("%stmf %s with tag %#llx ref_task_tag %#llx i_state %d t_state %s refcnt %d transport_state %s "
			 "bs_state:0x%llx(0x%llx), ptime:%ld(s)\n",
...
#ifdef CONFIG_MACH_QNAPTS
	qnap_transport_show_cmd("wait_for_tasks: stopping ", cmd);
#endif
...
#ifdef QNAP_TARGET_PERF_FEATURE
	if (qnap_target_perf_enabled())
		qnap_transport_perf_wakeup_workers();
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	qnap_transport_show_cmd("wait_for_tasks: Stopped ", cmd);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	else if (reason == TCM_OPERATION_IN_PROGRESS) {
		/* please refer spc4r37a, 
		 * for sense code - OPERATION IN PROGRESS, its ASC is 0x0 
		 */
		asc = si->asc;
		ascq = si->ascq;		
	}
...
#ifdef CONFIG_MACH_QNAPTS
	if (si->add_sector_info && !qnap_odx_set_scsi_sense_information(cmd))
		return;
#endif
...
{
#ifdef CONFIG_MACH_QNAPTS
	return ((dev->dev_attrib.dev_attr_dr.emulate_v_sup > 0) && \
