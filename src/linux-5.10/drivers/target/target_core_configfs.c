...
static int __init target_core_init_configfs(void)
{
	...
	if (qnap_odx_alloc_cache_res() != 0) {
		qlib_destroy_iorec_cache();
		goto out;
	}

	if (qnap_target_zmc_register)
		qnap_target_zmc_register(iscsi_target_zmc_ops);
...
static void __exit target_core_exit_configfs(void)
{
	...
	if (qnap_target_zmc_unregister)
		qnap_target_zmc_unregister(iscsi_target_zmc_ops);
