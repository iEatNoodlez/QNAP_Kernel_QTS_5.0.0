...
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_transport_check_cmd_aborted(&cmd->se_cmd, false)) {
		pr_warn("Dataout timeout timer triggered but the cmd was "
			"aborted already, skip to disconnect iscsi conn\n"); 
		spin_unlock_bh(&cmd->dataout_timeout_lock);
		iscsit_dec_conn_usage_count(conn);
		return; 
	}
