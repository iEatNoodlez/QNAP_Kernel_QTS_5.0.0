...
#include "qnap/iscsi_target_qzmc.h"
#include "qnap/iscsi_target_tcp_qzmc.h"
...
#ifdef CONFIG_MACH_QNAPTS
static void __qnap_iscsit_free_write_pending_cmd(
	struct iscsi_cmd *cmd
	)
{
	struct se_cmd *se_cmd = &cmd->se_cmd;
	bool to_free_cmd = false;

	/* se_tfo will be setup for scsi cmd or tmr cmd */
	...
	)
	{
		...
		if (to_free_cmd) {
			/* we do this to put refcnt for write pending cmd case */
			   ...
			   u32 buf_len)
{
	if (dr) {
#ifdef CONFIG_MACH_QNAPTS
		if (conn->zmc_ops && scsi_read_6_10_12_16(cmd->se_cmd.t_task_cdb)) {
			return conn->zmc_ops->xmit_datain_pdu(conn, 
					cmd, buf);
		}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_iscsit_zmc_register) {
		qnap_iscsit_zmc_register(iscsit_tcp_zmc_ops);
	}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_iscsit_zmc_unregister) {
		qnap_iscsit_zmc_unregister(iscsit_tcp_zmc_ops);
	}
...
#ifdef CONFIG_MACH_QNAPTS
		if (cmd->conn->zmc_ops && cmd->conn->zmc_ops->dump_data_payload) {
			rc = cmd->conn->zmc_ops->dump_data_payload(cmd->conn, cmd,
...
#ifdef CONFIG_MACH_QNAPTS
		if (cmd->conn->zmc_ops && cmd->conn->zmc_ops->handle_immediate_data) {
			immed_ret = cmd->conn->zmc_ops->handle_immediate_data(
					cmd->conn, cmd, hdr, cmd->first_burst_len);
		}
		else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_iscsit_zmc_setup_transport_zmc_ops) {
		rc = qnap_iscsit_zmc_setup_transport_zmc_ops(conn, cmd);
		if (rc && (rc != -ENOTSUPP)) {
			   ...
			   bool *success)
{
	...
	if (cmd->cmd_flags & ICF_GOT_LAST_DATAOUT) {
		pr_err("Command ITT: 0x%08x received DataOUT after"
			...
			cmd->init_task_tag);
#ifdef CONFIG_MACH_QNAPTS
		if (conn->zmc_ops && conn->zmc_ops->dump_data_payload) {
			return conn->zmc_ops->dump_data_payload(conn, cmd,
					payload_length, 1);
		}
#endif	
...
#ifdef CONFIG_MACH_QNAPTS
			if (conn->zmc_ops && conn->zmc_ops->dump_data_payload) {
				return conn->zmc_ops->dump_data_payload(conn, 
							cmd, payload_length, 1);
			}
#endif
			return iscsit_dump_data_payload(conn, payload_length, 1);
		}
			     ...
			     bool data_crc_failed)
{
	...
	if ((rc == DATAOUT_NORMAL) || (rc == DATAOUT_WITHIN_COMMAND_RECOVERY))
		...
		if (qnap_target_perf_enabled()) {
			qnap_transport_perf_exec_wt_cmd(&cmd->se_cmd);
			return 0;
		} 
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (conn->zmc_ops && conn->zmc_ops->get_dataout)
		rc = conn->zmc_ops->get_dataout(conn, cmd, hdr);
	else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		if (conn->zmc_ops && conn->zmc_ops->get_rx_data) {
			rx_got = conn->zmc_ops->get_rx_data(conn, cmd, &iov[0], 
					niov, rx_size);
		}
		else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		if (conn->zmc_ops && conn->zmc_ops->get_rx_data) {
			/* don't care the cmd_priv since we will put data 
			 * into iov vector
			 */
			rx_got = conn->zmc_ops->get_rx_data(conn, cmd, &iov[0], 
							niov, rx_size);
	...
	list_for_each_entry(tiqn, &g_tiqn_list, tiqn_list) {
		...
		list_for_each_entry(tpg, &tiqn->tiqn_tpg_list, tpg_list) {
			...
			if(qnap_iscsit_check_allow_np(tpg, conn, network_transport) == 0) {
				pr_info("IP:%pISc is not in the allow potal, do not report the Target:%s for discovery.\n", &conn->local_sockaddr, tiqn->tiqn);
				continue;
			}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (conn->zmc_ops && conn->zmc_ops->get_rx_pdu)
		conn->zmc_ops->get_rx_pdu(conn);
	else
#endif
...
static void iscsit_release_commands_from_conn(struct iscsi_conn *conn)
{
	...
	list_for_each_entry_safe(cmd, cmd_tmp, &tmp_list, i_conn_node) {
		...
		if (se_cmd->se_tfo != NULL) {
			...
			if (!(se_cmd->se_cmd_flags & SCF_SCSI_TMR_CDB)) {
				/* we will set QNAP_CMD_T_RELEASE_FROM_CONN here */
				if (se_cmd->cmd_dr.io_ops.do_drain_io)
					se_cmd->cmd_dr.io_ops.do_drain_io(se_cmd, -1);	
			}
#endif
			spin_lock_irq(&se_cmd->t_state_lock);
			if (se_cmd->transport_state & CMD_T_ABORTED) {
				/* this job transfer to the following code,
				 * (get cmd from tmp_list by list_for_xxx)
				 */
	...
	list_for_each_entry_safe(cmd, cmd_tmp, &tmp_list, i_conn_node) {
		list_del_init(&cmd->i_conn_node);
#ifdef CONFIG_MACH_QNAPTS
		/* iscsit_release_commands_from_conn will be called during the
		 * service stop or iscsi reinstatement is executing so need to
		 * free any write pending cmd here since nobody will send R2T 
		 */
		__qnap_iscsit_free_write_pending_cmd(cmd);
#endif
...
#endif
	qnap_odx_iscsit_it_nexus_discard_all_tokens(conn);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (conn->zmc_ops && conn->zmc_ops->restore_sock_callback)
		conn->zmc_ops->restore_sock_callback(conn);
#endif
	if (conn->sock)
...
/* ===========================================
 * wrapper codes used by iscsit zmc functions 
 * =========================================== 
 */
int qnap_iscsit_zmc_add_reject(
	struct iscsi_conn *conn,
	u8 reason,
	unsigned char *buf
	)
...
int qnap_iscsit_zmc_target_rx_opcode(
	struct iscsi_conn * conn, 
	unsigned char * buf
	)
{
...
void qnap_iscsit_zmc_do_crypto_hash_buf(
	struct ahash_request *hash,
	const void *buf,
	u32 payload_length,
	u32 padding,
...
u32 qnap_iscsit_zmc_do_crypto_hash_sg(
	struct ahash_request * hash, 
	struct iscsi_cmd * cmd, 
	u32 data_offset, 
	u32 data_length, 
...
void qnap_iscsit_zmc_rx_thread_wait_for_tcp(struct iscsi_conn * conn)
{
	iscsit_rx_thread_wait_for_tcp(conn);
}

void qnap_iscsit_zmc_tx_thread_wait_for_tcp(struct iscsi_conn * conn)
{
	iscsit_tx_thread_wait_for_tcp(conn);
}
#endif
