...
#ifdef CONFIG_MACH_QNAPTS
			if (conn->zmc_ops && conn->zmc_ops->dump_data_payload) {
				if (conn->zmc_ops->dump_data_payload(conn, 
						cmd, payload_length, 1) < 0)
...
#ifdef CONFIG_MACH_QNAPTS
			if (conn->zmc_ops && conn->zmc_ops->dump_data_payload) {
				if (conn->zmc_ops->dump_data_payload(conn, cmd,
						payload_length, 1) < 0)
					return DATAOUT_CANNOT_RECOVER;
				return DATAOUT_WITHIN_COMMAND_RECOVERY;
			}
	...
	unsigned char *buf)
{
	...
	case ISCSI_PDU_RECEIVED_OK:
		...
		if (cmd->conn->zmc_ops && cmd->conn->zmc_ops->dump_data_payload) {
			return cmd->conn->zmc_ops->dump_data_payload(cmd->conn, 
