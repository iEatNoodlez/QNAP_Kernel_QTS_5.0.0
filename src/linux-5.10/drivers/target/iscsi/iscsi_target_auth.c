	...
	unsigned int *nr_out_len)
{
	...
	unsigned char decode_pwd[MAX_CHAP_CHALLENGE_LEN + 1];
	size_t compare_len;
	struct iscsi_chap *chap = conn->auth_protocol;
...
#ifdef CONFIG_MACH_QNAPTS
	if (type == BASE64) {
		if (strlen(chap_r) != __qnap_get_base64_encode_len(
							chap->digest_size)) {
			pr_err("[base64] Malformed CHAP_R\n");
			goto out;
		}
	} 
	else
...
#ifdef CONFIG_MACH_QNAPTS
	if (type == BASE64) {
		if (qnap_iscsit_decode_base64_string(chap_r, client_digest, 
						chap->digest_size) < 0) {
			pr_err("[base64] fail to decode CHAP_R\n");
			goto out;
		}
	} 
	else
...
#ifdef CONFIG_MACH_QNAPTS
	if(qnap_iscsit_decode_base64_string(auth->password, decode_pwd, sizeof(decode_pwd)) != 0){
		pr_err("[base64] fail to decode CHAP to buf, buf_len:%d\n", sizeof(decode_pwd));
		goto out;
	}
	ret = crypto_shash_update(desc, (char *)&decode_pwd,
			  strlen(decode_pwd));
#else
...
#ifdef CONFIG_MACH_QNAPTS
		/* initiator not want target authentication ? (rfc3720, p186) */
		pr_warn("initiator doesn't require target authentication \n");
		auth_ret = 0;
#endif
		goto out;
	}
...
#ifdef CONFIG_MACH_QNAPTS
	if (type == BASE64) {
		initiatorchg_len = __qnap_get_base64_decode_len(initiatorchg);
...
#ifdef CONFIG_MACH_QNAPTS
	if (type == BASE64) {
		if (qnap_iscsit_decode_base64_string(initiatorchg, 
				initiatorchg_binhex, CHAP_CHALLENGE_STR_LEN) < 0)
		{
			pr_err("[base64] fail to decode CHAP_C\n");
			goto out;
		}
	}
	else
...
#ifdef CONFIG_MACH_QNAPTS
	if(qnap_iscsit_decode_base64_string(auth->password_mutual, decode_pwd, sizeof(decode_pwd)) < 0){
		pr_err("[base64] fail to decode CHAP Mutual to buf, buf_len:%d\n", sizeof(decode_pwd));
		goto out;
	}
	ret = crypto_shash_update(desc, decode_pwd, strlen(decode_pwd));
#else
...
#ifdef CONFIG_MACH_QNAPTS
	if (type == BASE64) {
		/* we got the type from CHAP_C already,
		 * actually, this is NOT must but we still can do it to reply 
		 * the base64 format of the CHAP_R (we also can reply hex format
		 * for CHAP_R only).
		 */
		unsigned char *b64_resp = NULL;
		int resp_len = __qnap_get_base64_encode_len(chap->digest_size);
		resp_len += 1;
		b64_resp = kzalloc(resp_len, GFP_KERNEL);
		if (!b64_resp) {
			pr_err("fail to alloc base64 resp buffer\n");
			goto out;
		}

		qnap_iscsit_encode_base64_string(digest, chap->digest_size, b64_resp);
		*nr_out_len += sprintf(nr_out_ptr + *nr_out_len, "CHAP_R=0b%s",
				b64_resp);
		*nr_out_len += 1;
		pr_debug("[server] Sending (base64) CHAP_R=0b%s\n", b64_resp);
