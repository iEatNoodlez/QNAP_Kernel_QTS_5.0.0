...
#ifdef CONFIG_MACH_QNAPTS
		if (conn->zmc_ops && conn->zmc_ops->set_sock_callback)
			conn->zmc_ops->set_sock_callback(conn);
#endif
		/*
...
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_iscsit_zmc_create_conn_priv)
		qnap_iscsit_zmc_create_conn_priv(conn);
#endif
	return conn;
...
{
#ifdef CONFIG_MACH_QNAPTS
	if (qnap_iscsit_zmc_release_conn_priv)
		qnap_iscsit_zmc_release_conn_priv(conn);
#endif
