...
static void cmd_rw_aio_complete(struct kiocb *iocb, long ret, long ret2)
{
	...
	bool is_thin_dev = qlib_thin_lun(&se_dev->dev_dr);

	if (is_thin_dev && (se_cmd->data_direction == DMA_TO_DEVICE)) {
		/* this call comes from bio callback so we can check ret code directly */		
		if (ret == -ENOSPC) {
			se_cmd->cmd_dr.work_io_err = TCM_SPACE_ALLOCATION_FAILED_WRITE_PROTECT;
			pr_warn("%s: space was full\n", __func__);
		}
		if ((ret == cmd->len) && (!qnap_transport_check_cmd_hit_thin_threshold(se_cmd))) 
		{
			se_cmd->cmd_dr.work_io_err = 
					TCM_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED;
			/* change ret code due to we want send sense code */
			ret = -EIO;
		}
	}
#endif
	      ...
	      enum dma_data_direction data_direction)
{
	...
	int ret = 0, i;
#ifdef CONFIG_MACH_QNAPTS
	struct fio_data_set data;
	struct target_zmc_ops *zmc_ops = cmd->cmd_dr.zmc_ops;

	if (zmc_ops && zmc_ops->sbc_write && scsi_write_6_10_12_16(cmd->t_task_cdb)) {
		/* take care this code, even if the aio mechanism (set by IOCB_DIRECT) 
		 * will submit bio(s) directly but it will fail on checking 
		 * iov_iter_alignment() in __blkdev_direct_IO() due the the
		 * skb page data(sg off may not aligned by block size) .. 
		 * so we will submit bio(s) by ourself ..
		 */
		data.async_io = true;
		ret = zmc_ops->sbc_write(cmd, &data);
		if (ret != -ENOTSUPP) {
			if (ret)
				WARN_ON(true);	
		    ...
		    u32 sgl_nents, u32 data_length, int is_write)
{
	...
	if (cmd->cmd_dr.io_ops.after_do_drain_io)
		cmd->cmd_dr.io_ops.after_do_drain_io(cmd, NULL);

	if (cmd->cmd_dr.io_ops.check_io_drained) {
		if (cmd->cmd_dr.io_ops.check_io_drained(cmd)) {
			/* actually, we don't care the ret value (done bytes
			 * for read or write) since we drain the commands
			 */
			kfree(bvec);
			return 1;
		}
...
fd_execute_write_same(struct se_cmd *cmd)
{
	...
	if (zmc_ops && zmc_ops->sbc_write_same) {
		ret = zmc_ops->sbc_write_same(cmd, NULL);
		if (!ret || (ret != -ENOTSUPP))
			return cmd->cmd_dr.work_io_err;
	}
#endif
	      ...
	      enum dma_data_direction data_direction)
{
	 ...
	 */
	if (zmc_ops && zmc_ops->sbc_write && scsi_write_6_10_12_16(cmd->t_task_cdb)) {
		/* take care this code, we only replace sgl, sgl_nents since 
		 * generic_perform_write() (called by vfs write) will help us 
		 * do buffer-copy
		 */
		data.async_io = false;
		ret = zmc_ops->sbc_write(cmd, &data);
	...
	if (data_direction == DMA_FROM_DEVICE) {
				...
				(cmd->data_direction == DMA_TO_DEVICE))
		{
			...
			if (tmp_ret != ret) {
				...
				/* after every write operation, we check whether 
				 * data size hits thin threshold or not. If yes, 
				 * it shall response the UA status for 'hit threshold'
				 */
				if (!qnap_transport_check_cmd_hit_thin_threshold(cmd)) {
					cmd->cmd_dr.work_io_err = 
						TCM_THIN_PROVISIONING_SOFT_THRESHOLD_REACHED;
...
#ifdef CONFIG_MACH_QNAPTS
			ret = qnap_transport_vfs_fsync_range(dev, start, end, 1);
			if (!ret)
				ret = 1;

			if (is_thin_dev && (ret == -ENOSPC)) {
				cmd->cmd_dr.work_io_err = 
						TCM_SPACE_ALLOCATION_FAILED_WRITE_PROTECT;
				return TCM_SPACE_ALLOCATION_FAILED_WRITE_PROTECT;
			}
