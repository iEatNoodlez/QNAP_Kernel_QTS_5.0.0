...
#if  defined(X86_SUMMITRIDGE)
static void xhci_reset_work(struct work_struct *work);
#endif
/*
...
#if  defined(X86_SUMMITRIDGE)
    INIT_WORK(&xhci->reset_work, xhci_reset_work);
#endif
...
static void xhci_reset_work(struct work_struct *work)
{
	...
	xhci = container_of(work, struct xhci_hcd, reset_work);
	...
	spin_unlock_irq(&xhci->lock);
	if (retval) {
		xhci_err(xhci, "Unable to issue HCRST\n");
		goto fail;
	}
	...
	retval = xhci_init(hcd->primary_hcd);
	if (retval) {
		xhci_err(xhci, "Re-intialization failed\n");
		goto fail;
	}
	...
	command |= CMD_RUN;
	writel(command, &xhci->op_regs->command);
	xhci_handshake(&xhci->op_regs->status, STS_HALT, 0, 250 * 1000);
	spin_unlock_irq(&xhci->lock);

	if (!retval && xhci_pending_portevent(xhci)) {
		usb_hcd_resume_root_hub(xhci->shared_hcd);
		usb_hcd_resume_root_hub(hcd);
	}
