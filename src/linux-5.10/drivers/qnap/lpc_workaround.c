...
static void apl_wa_disable(void);
static void bsl_wa_enable(void);
static void bsl_wa_disable(void);
...
#else
static void loop_timer(struct timer_list *t)
#endif
{
	unsigned long flags;
	...
	if(!dont_do_wa) {
		...
		if(workaround_data != NULL) workaround_data->enable();
...
#else
static void workaround_disable(struct timer_list *t)
#endif
{
	unsigned long flags;
	spin_lock_irqsave(&wa_lock, flags);
	if(workaround_data != NULL) workaround_data->enable();
	workaround_data->disable();
	wa_disabled_count++;
	spin_unlock_irqrestore(&wa_lock, flags);
}
...
void intel_apl_ir_workaround(void)
{
	unsigned long flags;

	// only do this in apollo lake
	if(!dont_do_wa) {
		if( (workaround_data != NULL) && (workaround_data->base_addr == 0xE00F8000)) {
			spin_lock_irqsave(&wa_lock, flags);
...
EXPORT_SYMBOL(intel_apl_ir_workaround);
...
#else
	timer_setup(&timer_to_disable, workaround_disable, 0);
	timer_setup(&timer_to_loop, loop_timer, 0);
#endif
