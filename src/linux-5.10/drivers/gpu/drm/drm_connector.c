...
static bool drm_force_dp_to_hdmi = 0;    /* Default disabled. */
module_param_named(force_dp_to_hdmi, drm_force_dp_to_hdmi, bool, 0600);
MODULE_PARM_DESC(force_dp_to_hdmi, "Force renaming DP port as HDMI port. 0: Disable, 1:enable (default: 0)");
		       ...
		       int connector_type)
{
	...
	int new_connector_type = connector_type;

	if (drm_force_dp_to_hdmi && (connector_type == DRM_MODE_CONNECTOR_DisplayPort))
		new_connector_type = DRM_MODE_CONNECTOR_HDMIA;

	connector_ida = &drm_connector_enum_list[new_connector_type].ida;
	...
	if (ret < 0) {
		DRM_DEBUG_KMS("Failed to allocate %s connector index: %d\n",
			      drm_connector_enum_list[new_connector_type].name,
			      ret);
		goto out_put;
	}
	connector->index = ret;
...
//INTEL_PATCH_YCBCR_LOCAL+
void drm_mode_update_ycbcr420_flags(struct drm_mode_modeinfo *umode,
    const struct drm_display_mode *kmode, const struct drm_display_info *info)
{
	if (drm_mode_is_420_only(info, kmode)) {
		umode->flags |= DRM_MODE_FLAG_YCBCR420;
		DRM_DEBUG_KMS("Found 420 mode: %s, vic:%d\n",
		    kmode->name, drm_match_cea_mode(kmode));
	}
}
//INTEL_PATCH_YCBCR_LOCAL-
			  ...
			  struct drm_file *file_priv)
{
	...
	if (drm_force_dp_to_hdmi && (out_resp->connector_type == DRM_MODE_CONNECTOR_DisplayPort))
		out_resp->connector_type = DRM_MODE_CONNECTOR_HDMIA;
	...
	if ((out_resp->count_modes >= mode_count) && mode_count) {
		...
		list_for_each_entry(mode, &connector->modes, head) {
			...
			drm_mode_update_ycbcr420_flags(&u_mode, mode, &connector->display_info); //INTEL_YCBCR_PATCH_LOCAL
