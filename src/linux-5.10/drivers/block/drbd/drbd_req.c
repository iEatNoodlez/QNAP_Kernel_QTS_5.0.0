...
blk_qc_t drbd_submit_bio(struct bio *bio)
{
	...
	enum drbd_disk_state disk_state = device->state.disk; /* QNAP_PATCH on 20210107 */
#endif
	...
	if ( disk_state == D_DISKLESS ){ // handle IO error on backing device
		bio_endio(bio);
		return BLK_QC_T_NONE;
	}

	/* for QNAP bypass drbd data path in 20150901, default is bypass */
	if ( device->ldev->disk_conf->disk_bypass )
	{
		bio_set_dev(bio, device->ldev->backing_bdev);
		submit_bio_noacct(bio);
		return BLK_QC_T_NONE;
