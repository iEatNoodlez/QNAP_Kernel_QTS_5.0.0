...
#ifdef CONFIG_MACH_QNAPTS
static void _disable_active_led(struct timer_list *t)
{
	struct mvpp2_port *port = from_timer(port, t, active_led_timer);

	gpiod_set_blink(port->led_active, 0);
}

static inline void disable_active_led(struct mvpp2_port *port)
{
	if(port->led_active) {
...
static void trigger_active_led(struct mvpp2_port *port)
{
	if(port->led_active) {
...
static void link_led_change(struct mvpp2_port *port, int value)
{
	if(port->led_link_10G)
	{
		gpiod_set_value(port->led_link_10G, value);
		if(port->led_link_1G)
			gpiod_set_value(port->led_link_1G, 0);
	}
}
        ...
        struct mvpp2_port *port)
{
	...
	port->led_link_10G = _mv_pp2x_led_gpio_probe(pdev, node, "led-link-10G");
	port->led_link_1G = _mv_pp2x_led_gpio_probe(pdev, node, "led-link-1G");

	if(port->led_active)
		timer_setup(&port->active_led_timer, _disable_active_led, 0);
}
#endif
...
static int mvpp22_gop_init(struct mvpp2_port *port)
{
	...
	case PHY_INTERFACE_MODE_10GKR: // QNAP fix 997
...
static int mvpp2_set_wol(struct net_device *ndev, struct ethtool_wolinfo *wolinfo)
{
	...
	if (ndev->phydev && ndev->phydev->drv && ndev->phydev->drv->set_wol)
	{
		...
		pic_wol->enable = wolinfo->wolopts & WAKE_MAGIC ? 1 : 0;
		send_hal_netlink(&hal_event);
		mcu_wol_enable = pic_wol->enable;
...
static void mvpp2_phylink_mac_link_state(struct phylink_config *config,
                    struct phylink_link_state *state)
{
...
static const struct phylink_mac_ops mvpp2_phylink_ops = {
    ...
    .mac_pcs_get_state = mvpp2_phylink_mac_link_state,  // .mac_link_state
...
#ifdef CONFIG_MACH_QNAPTS
	mv_pp2x_led_gpio_probe(pdev, port_node, port);
#endif
