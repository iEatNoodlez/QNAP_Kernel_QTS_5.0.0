...
static void stmmac_get_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	...
	if (dev && dev->phydev && dev->phydev->drv && dev->phydev->drv->get_wol)
	{
		dev->phydev->drv->get_wol(dev->phydev, wol);
		return;
	}
...
static int stmmac_set_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	...
	if (dev && dev->phydev && dev->phydev->drv && dev->phydev->drv->set_wol)
	{
		return dev->phydev->drv->set_wol(dev->phydev, wol);
	}
