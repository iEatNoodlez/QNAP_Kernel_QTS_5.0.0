...
#ifdef CONFIG_MACH_QNAPTS
static const char tb_net_gstrings_stats[][ETH_GSTRING_LEN] = {
	"tx_packets",
	"rx_packets",
	"tx_bytes",
	"rx_bytes",
...
{
	struct tbnet *port = netdev_priv(net_dev);
	const struct tb_xdomain *xd = port->xd;
	u8 phy_port;
	...
	printk(KERN_ALERT "Thunderbolt(TM) Networking port %u - changing MTU from %u to %d\n",
		   phy_port, net_dev->mtu, new_mtu);
...
#ifdef CONFIG_MACH_QNAPTS
static int tb_net_get_link_ksettings(__maybe_unused struct net_device *net_dev,
...
static u32 tb_net_get_msglevel(struct net_device *net_dev)
{
	struct tbnet *port = netdev_priv(net_dev);
...
static void tb_net_set_msglevel(struct net_device *net_dev, u32 data)
{
	struct tbnet *port = netdev_priv(net_dev);
...
static void tb_net_get_strings(__maybe_unused struct net_device *net_dev,
				u32 stringset, u8 *data)
{
	if (stringset == ETH_SS_STATS)
		memcpy(data, tb_net_gstrings_stats,
		       sizeof(tb_net_gstrings_stats));
}

static void tb_net_get_ethtool_stats(struct net_device *net_dev,
...
static int tb_net_get_sset_count(__maybe_unused struct net_device *net_dev,
				  int sset)
{
	if (sset == ETH_SS_STATS)
		return sizeof(tb_net_gstrings_stats) / ETH_GSTRING_LEN;
	return -EOPNOTSUPP;
}

static void tb_net_get_drvinfo(struct net_device *net_dev,
				struct ethtool_drvinfo *drvinfo)
{
	struct tbnet *port = netdev_priv(net_dev);
    struct tb_xdomain *xd = port->xd;

	strlcpy(drvinfo->bus_info, pci_name(xd->tb->nhi->pdev),
		sizeof(drvinfo->bus_info));
	drvinfo->n_stats = tb_net_get_sset_count(net_dev, ETH_SS_STATS);
}

static const struct ethtool_ops tb_net_ethtool_ops = {
	.get_drvinfo		= tb_net_get_drvinfo,
	.get_link		= ethtool_op_get_link,
	.get_msglevel		= tb_net_get_msglevel,
	.set_msglevel		= tb_net_set_msglevel,
	.get_strings		= tb_net_get_strings,
	.get_ethtool_stats	= tb_net_get_ethtool_stats,
	.get_sset_count		= tb_net_get_sset_count,
	.get_link_ksettings	= tb_net_get_link_ksettings,
};
#endif
...
static void tbnet_generate_mac(struct net_device *dev)
{
	...
	dev->dev_addr[0] = (0x0f << 4) | (phy_port << 1);
#else
...
#ifdef CONFIG_MACH_QNAPTS
static void qnap_rename_tbtnet(struct net_device *dev)
{
	const struct tbnet *net = netdev_priv(dev);
	const struct tb_xdomain *xd = net->xd;
	u8 phy_port;
	...
	if(xd->tb->nhi->pdev->device == PCI_DEVICE_ID_INTEL_FALCON_RIDGE_C_4C_NHI)
		scnprintf(dev->name, sizeof(dev->name), "tbtnet%%dp%hhu", phy_port);
	else
		scnprintf(dev->name, sizeof(dev->name), "tbtnet%%dp%hhu",
					TB_LINKS_PER_PHY_PORT - 1 - phy_port);
}
#endif
...
static int tbnet_probe(struct tb_service *svc, const struct tb_service_id *id)
{
...
#ifdef CONFIG_MACH_QNAPTS
	tbt_net_change_mtu(dev, TBNET_MAX_MTU - ETH_HLEN);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	dev->ethtool_ops = &tb_net_ethtool_ops;
	net->msg_enable = netif_msg_init(debug, DEFAULT_MSG_ENABLE);
	...
	printk(KERN_ALERT "%s, send_hal_netlink to resume\n", __func__);
	// Fix Bug 215015
	//atomic_inc(&tbt_ports);
#endif
...
static void tbnet_remove(struct tb_service *svc)
{
	...
	printk(KERN_ALERT "[%s] %s, send_hal_netlink to suspend.\n",
						net->dev->name, __func__);
