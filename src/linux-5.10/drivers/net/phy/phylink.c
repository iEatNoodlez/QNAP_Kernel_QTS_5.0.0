 ...
 *    1     1       0     1     RX
 */
static void mv_phylink_resolve_flow(struct phylink *pl,
				 struct phylink_link_state *state)
{
	int new_pause = 0;
...
static void phylink_resolve(struct work_struct *w)
{
	...
	if (pl->phylink_disable_state) {
		...
		case MLO_AN_PHY:
			link_state = pl->phy_state;
			mv_phylink_resolve_flow(pl, &link_state);
		...
		case MLO_AN_INBAND:
			...
			if (pl->phydev && pl->phy_state.link) {
				...
				if (link_state.link == 0)
				{
					// workaround: mac not read when phy is ready in booting. so check mac link state again after setting mac.
					phylink_err(pl, "info: old link_state.link = %d, pl->phy_state.link = %d, phydev->link = %d, phydev->state = %d\n", link_state.link, pl->phy_state.link, pl->phydev->link, pl->phydev->state);
					phylink_mac_pcs_get_state(pl, &link_state);
					phylink_err(pl, "info: new link_state.link = %d, pl->phy_state.link = %d, phydev->link = %d, phydev->state = %d\n", link_state.link, pl->phy_state.link, pl->phydev->link, pl->phydev->state);
					link_changed = (link_state.link != netif_carrier_ok(ndev));
					phylink_err(pl, "info: link_changed = %d, link_state.link=%d != netif_carrier_ok(ndev)=%d\n", link_changed, link_state.link, netif_carrier_ok(ndev));
				}
			}
			phylink_apply_manual_flow(pl, &link_state);
			break;
...
#endif // End of #if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_MVEBU)
