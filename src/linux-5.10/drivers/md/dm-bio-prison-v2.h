...
struct dm_cell_key_v2 {
	...
	unsigned bed_begin:PRISON_CELL_BED_BITS;
	unsigned bed_end:PRISON_CELL_BED_BITS;
};
...
struct dm_bio_prison_cell_v2 {
	...
	unsigned entry:4;
	unsigned sweeper:4;
	uint16_t shared_count[MAX_PLOCK];
	struct list_head cont_list[MAX_PLOCK];
 ...
 *
 * Returns lock_id if the lock is granted.
 */
