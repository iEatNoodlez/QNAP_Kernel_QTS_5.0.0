			       ...
			       struct queue_limits *limits)
{
	...
	if (!dm_table_locate_thick(t))
		blk_queue_flag_clear(QUEUE_FLAG_THICK, q);
	else
		blk_queue_flag_set(QUEUE_FLAG_THICK, q);

	if (!dm_table_enable_encryption(t))
		blk_queue_flag_clear(QUEUE_FLAG_ENCRYPT, q);
	else
		blk_queue_flag_set(QUEUE_FLAG_ENCRYPT, q);
