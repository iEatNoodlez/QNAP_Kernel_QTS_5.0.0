...
static int __bitset_alter_unaligned(struct dm_disk_bitset *info,
                                    enum bitset_alter_type type,
                                    uint32_t index, unsigned long *bits,
                                    uint32_t nr_bits, dm_block_t *root)
{
	int r = 0;
	unsigned int p, rs, re;

	bitmap_for_each_set_region(bits, rs, re, 0, nr_bits)
		for (p = rs; !r && p < re; rs++)
			if (type == BITSET_SET)
				r = dm_bitset_set_bit(info, *root,
				                      index + p, root);
			else
				r = dm_bitset_clear_bit(info, *root,
				                        index + p, root);

	return r;
}

static int __bitset_alter_aligned(struct dm_disk_bitset *info,
                                  enum bitset_alter_type type,
                                  uint32_t index, unsigned long *bits,
                                  uint32_t nr_bits, dm_block_t *root)
{
	int r;
	unsigned alter_nr_bits;
	unsigned long *meta_bits;

	while (true) {
		r = get_array_entry(info, *root, index, root);
		if (r)
			return r;

		meta_bits = (unsigned long *)&info->current_bits;
		alter_nr_bits = min((unsigned)nr_bits,
		                    (unsigned)BITS_PER_ARRAY_ENTRY);

		if (type == BITSET_SET) {
			if (bits)
				bitmap_or(meta_bits, meta_bits, bits,
			                  alter_nr_bits);
			else
				bitmap_fill(meta_bits, alter_nr_bits);
		} else {
			if (bits)
				bitmap_andnot(meta_bits, meta_bits, bits,
				              alter_nr_bits);
			else
				bitmap_zero(meta_bits, alter_nr_bits);
		}
		info->dirty = true;

		if (nr_bits <= BITS_PER_ARRAY_ENTRY)
			break;

		index += BITS_PER_ARRAY_ENTRY;
		nr_bits -= BITS_PER_ARRAY_ENTRY;
		if (bits)
			bits += (BITS_PER_ARRAY_ENTRY / BITS_PER_LONG);
	}

	return 0;
}

int dm_bitset_set_bits(struct dm_disk_bitset *info, dm_block_t root,
                       uint32_t index, unsigned long *bits, uint32_t nr_bits,
                       dm_block_t *new_root)
{
	*new_root = root;

	if (index % BITS_PER_ARRAY_ENTRY ||
	    BITS_PER_ARRAY_ENTRY % BITS_PER_LONG)
		return __bitset_alter_unaligned(info, BITSET_SET, index,
		                                bits, nr_bits, new_root);
	else
		return __bitset_alter_aligned(info, BITSET_SET, index, bits,
		                              nr_bits, new_root);
}
EXPORT_SYMBOL_GPL(dm_bitset_set_bits);

int dm_bitset_clear_bits(struct dm_disk_bitset *info, dm_block_t root,
                         uint32_t index, unsigned long *bits, uint32_t nr_bits,
                         dm_block_t *new_root)
{
	*new_root = root;
	...
	if (index % BITS_PER_ARRAY_ENTRY)
		return __bitset_alter_unaligned(info, BITSET_CLEAR,
		                                index, bits, nr_bits, new_root);
	else
		return __bitset_alter_aligned(info, BITSET_CLEAR,
		                              index, bits, nr_bits, new_root);
}
EXPORT_SYMBOL_GPL(dm_bitset_clear_bits);
