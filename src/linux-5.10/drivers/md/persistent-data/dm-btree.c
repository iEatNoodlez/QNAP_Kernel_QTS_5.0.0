...
static void prefetch_children(struct dm_transaction_manager *tm,
			      struct frame *f)
{
	unsigned i;
	struct dm_block_manager *bm = dm_tm_get_bm(tm);
	for (i = 0; i < f->nr_children; i++)
		dm_bm_prefetch(bm, value64(f->n, i));
}

static int push_preread_frame(struct preread_stack *s, dm_block_t b,
			      unsigned level)
{
	int r;
	uint32_t flags;
	...
	if (s->top >= MAX_SPINE_DEPTH - 1) {
		DMERR("push_preread_frame stack out of memory");
		return -ENOMEM;
	}
	...
	if (flags & INTERNAL_NODE || is_internal_level(s->info, f))
		prefetch_children(s->tm, f);
	return 0;
}

static void pop_preread_frame(struct preread_stack *s)
{
	struct frame *f = s->spine + s->top--;
	dm_tm_unlock(s->tm, f->b);
}

static int unprocessed_preread_frames(struct preread_stack *s)
{
	return s->top >= 0;
}

static int top_preread_frame(struct preread_stack *s, struct frame **f)
{
	if (s->top < 0) {
...
static void unlock_all_preread_frames(struct preread_stack *s)
{
	struct frame *f;

	while (unprocessed_preread_frames(s)) {
		f = s->spine + s->top--;
		dm_tm_unlock(s->tm, f->b);
	}
}

int dm_btree_preread(struct dm_btree_info *info, dm_block_t root)
{
	int r;
	struct preread_stack *s;
	...
	while (unprocessed_preread_frames(s)) {
		...
		if (flags & INTERNAL_NODE) {
			...
			f->current_child++;
			r = push_preread_frame(s, b, f->level);
			if (r)
				goto out;
		} else if (is_internal_level(info, f)) {
			b = value64(f->n, f->current_child);
			f->current_child++;
			r = push_preread_frame(s, b, f->level + 1);
			if (r)
				goto out;
...
out:
	if (r) {
		...
		unlock_all_preread_frames(s);
	}
	kfree(s);

	return r;
}
EXPORT_SYMBOL_GPL(dm_btree_preread);
...
static int insert(struct dm_btree_info *info, dm_block_t root,
		  ...
		  int *inserted, uint64_t *highest_key_in_leaf)
		  __dm_written_to_disk(value)
{
	int r;
	...
	if (highest_key_in_leaf) {
		struct btree_node *node = n;

		uint32_t index = le32_to_cpu(node->header.nr_entries) - 1;
		*highest_key_in_leaf = le64_to_cpu(node->keys[index]);
	}
...
int dm_btree_insert(struct dm_btree_info *info, dm_block_t root,
		    ...
		    __dm_written_to_disk(value)
{
	return insert(info, root, keys, value, new_root, NULL, NULL);
}
...
int dm_btree_insert_notify(struct dm_btree_info *info, dm_block_t root,
			   ...
			   __dm_written_to_disk(value)
{
	return insert(info, root, keys, value, new_root, inserted, highest_key_in_leaf);
}
...
static int stack_push(struct stack_node **top, dm_block_t item)
{
	...
	new->next = *top;
	*top = new;
	DMDEBUG("%s:%d, push blocknr %llu into stack", __func__, __LINE__,
		new->blocknr);
...
static struct stack_node *stack_pop(struct stack_node **top)
{
	...
	*top = (*top)->next;
	...
	DMDEBUG("%s:%d, pop blocknr %llu from stack", __func__, __LINE__,
		tmp->blocknr);
	return tmp;
}
		       ...
		       void *context)
{
	...
	while (!isEmpty(stack_top)) {
		...
		r = stack_node_read_lock(info, tmp->blocknr, &nodes);
		if (r) {
			kfree(tmp);
			goto err_out;
		}
