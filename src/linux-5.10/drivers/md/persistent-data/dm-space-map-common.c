			...
			unsigned *result, int *len)
{
	...
	while (begin < end) {
		if (!(begin & (ENTRIES_PER_WORD - 1)) &&
		    dm_bitmap_word_used(addr, begin) && !found) {
			begin += ENTRIES_PER_WORD;
			continue;
		}
...
dm_block_t sm_ll_get_max_blocks(struct ll_disk *ll)
{
	return ll->max_blocks(ll);
}
...
static dm_block_t metadata_ll_max_blocks(struct ll_disk *ll)
{
	if (ll->block_size == 4096)
		return DM_SM_METADATA_CACHE_MAX_BLOCKS;
	else if (ll->block_size == 8192)
		return DM_SM_METADATA_THIN_MAX_BLOCKS;
	else {
		DMWARN("Using default DM_SM_METADATA_MAX_BLOCKS entries of metadata_ll");
		return DM_SM_METADATA_MAX_BLOCKS;
	}
}
...
int sm_ll_new_metadata(struct ll_disk *ll, struct dm_transaction_manager *tm)
{
	...
	ll->max_blocks = metadata_ll_max_blocks;
...
static dm_block_t disk_ll_max_blocks(struct ll_disk *ll)
{
	return -1ULL;
}
...
int sm_ll_new_disk(struct ll_disk *ll, struct dm_transaction_manager *tm)
{
	...
	ll->max_blocks = disk_ll_max_blocks;
