...
static void sm_release_free_map(struct sm_disk *smd)
{
	...
	kvfree(smd->free_map->filter);
	kvfree(smd->free_map->map);
}
...
{
	unsigned long flags, *new_fltr, *new_map, *old_fltr, *old_map;
	...
	new_fltr = vzalloc(array_size(DIV_ROUND_UP(total, (long)1 << FILTER_SHIFT),
				      sizeof(long)));
	if (!new_fltr)
		DMERR("resize filter to %llu failed", smd->ll.nr_blocks);

	new_map = vzalloc(array_size(total, sizeof(unsigned long)));
	if (!new_map)
		DMERR("resize free map to %llu failed", smd->ll.nr_blocks);

	spin_lock_irqsave(&smd->free_map->lock, flags);
	old_fltr = smd->free_map->filter;
	__update_bitmap((void **)&smd->free_map->filter, (void **)&new_fltr,
			DIV_ROUND_UP(org, (long)1 << FILTER_SHIFT) *
				sizeof(unsigned long));

	old_map = smd->free_map->map;
				     ...
				     dm_block_t *count)
{
	...
	*count = 0;
	for (b = begin; !r; b = result + free_blks) {
		r = sm_ll_find_serial_free_blks(&smd->ll, b, end,
						&result, &free_blks);
		if (r) {
			if (r != -ENOSPC) {
				DMWARN("get serial free blk cnt fail");
				r1 = r;
			}
			break;
		}
...
static void sm_allocate_free_map(struct sm_disk *smd, int create)
{
	...
	smd->free_map->filter =
		vzalloc(array_size(DIV_ROUND_UP(total, (long)1 << FILTER_SHIFT),
				   sizeof(long)));

	if (!smd->free_map->filter) {
		DMWARN("alloc free filter failed");
		return;
	}

	smd->free_map->map = vzalloc(array_size(total, sizeof(long)));
	if (!smd->free_map->map) {
		DMWARN("alloc free map failed");
	...
	for (b = 0; !r; b = result + free_blks) {
		r = sm_ll_find_serial_free_blks(&smd->ll, b, smd->ll.nr_blocks,
						&result, &free_blks);
		if (r) {
			if (r != -ENOSPC)
				DMWARN("reconstruct free map fai");
			break;
		}

		bitmap_set(smd->free_map->map, (unsigned int)result, free_blks);
		bitmap_set(smd->free_map->filter,
			   (unsigned int)result >> FILTER_SHIFT, 1);
		bitmap_set(smd->free_map->filter,
