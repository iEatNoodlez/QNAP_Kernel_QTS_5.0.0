...
#define DEFAULT_SUB_JOB_SIZE_KB 1024
#else
...
int __init dm_kcopyd_init(void)
{
	...
	_track_job_cache = kmem_cache_create("kcopyd_track_job",
				sizeof(struct track_job) * (SPLIT_COUNT + 1),
...
//		printk("dm-kcopyd track job out of array\n");
	...
	if (job->source.count <= kc->sub_job_size)
		...
		printk("got io %d from flashcache over %d\n", job->source.count, kc->sub_job_size);
		job->progress = 0;
		split_job(job);
	}
...
struct dm_kcopyd_client *dm_kcopyd_client_create_tracked(struct dm_kcopyd_throttle *throttle)
{
	...
	r = mempool_init_slab_pool(&kc->track_job_pool, MIN_JOBS, _track_job_cache);
	if (r)
		goto bad_slab1;
...
bad_workqueue:
	mempool_exit(&kc->job_pool);
bad_slab:
	mempool_exit(&kc->track_job_pool);
bad_slab1:
	kfree(kc);
