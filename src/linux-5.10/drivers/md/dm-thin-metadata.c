...
#define THIN_METADATA_MAX_SECTORS                                              \
	(511 * (1 << 15) * (THIN_METADATA_BLOCK_SIZE / (1 << SECTOR_SHIFT)))
...
static int sb_backup_check(struct dm_block_validator *v, struct dm_block *b,
			   size_t block_size)
{
	struct thin_disk_superblock *disk_super = dm_block_data(b);
	__le32 csum_le;
...
static int locate_sb_backup(struct dm_pool_metadata *pmd)
{
	...
	for (i = 1; i <= SB_BACKUP_MAX_COUNT; i++) {
		...
		DMDEBUG("%s: block %d, set backup id = %llu off = %u", __func__,
			i, pmd->backup_id, pmd->backup_off);
		dm_bm_unlock(bsblock);
	}
...
static int __write_initial_sb_backups(struct dm_pool_metadata *pmd)
{
	...
	for (i = 1; i <= SB_BACKUP_MAX_COUNT; i++) {
		...
		disk_super->magic = cpu_to_le64(THIN_SUPERBLOCK_MAGIC);
		DMDEBUG("%s: wipe block %llu as zeroed superblock", __func__,
			dm_bm_nr_blocks(pmd->bm) - i);
		dm_bm_unlock(block);
	}
				     ...
				     struct dm_pool_metadata *pmd)
{
	...
	if (le32_to_cpu(disk_super->version) > THIN_VERSION) {
		...
		case 1:
		case 2:
			DMINFO("pool version: %u, mask superblock and fast block clone feature",
			       le32_to_cpu(disk_super->version));
			flags &= ~(THIN_FEATURE_SUPERBLOCK_BACKUP |
				   THIN_FEATURE_FAST_BLOCK_CLONE);
			break;
		case 3:
			...
			break;
		default:
			DMERR("unknown dm-thin version %u",
			      le32_to_cpu(disk_super->version));
			return -EVERSION;
		}
...
static void check_clone_support(struct dm_pool_metadata *pmd)
{
	if (old_meta(pmd->version))
		pmd->details_info.value_type.size =
			sizeof(struct old_disk_device_details);
}
...
static int device_details_walker(void *context, uint64_t *keys, void *leaf)
{
	...
	if (old_meta(pmd->version)) {
		...
		if (le64_to_cpu(old_details_le->snap_origin) == ULLONG_MAX)
			pmd->mapped_blocks +=
				le64_to_cpu(old_details_le->mapped_blocks);
	} else {
...
{
	return dm_btree_walk_iter(&pmd->details_info, pmd->details_root,
				  device_details_walker, pmd);
}

static int __init_new_features(struct dm_pool_metadata *pmd,
			       struct thin_disk_superblock *disk_super)
{
	int r;
 ...
 static int __open_metadata(struct dm_pool_metadata *pmd)
 {
	...
	r = dm_bm_write_lock(pmd->bm, THIN_SUPERBLOCK_LOCATION, &sb_validator,
			     &sblock);
	if (r < 0) {
	...
	pmd->data_sm =
		dm_sm_disk_open(pmd->tm, disk_super->data_space_map_root,
...
static struct dm_block_manager *
__get_correct_block_manager(struct dm_pool_metadata *pmd, int *unformatted)
{
	int version, r = 0;
	struct dm_block *sblock;
...
static int __commit_transaction(struct dm_pool_metadata *pmd)
{
	...
	if (unlikely(!pmd->in_service) && !pmd->need_commit)
		return 0;

	if (pmd->pre_commit_fn) {
		r = pmd->pre_commit_fn(pmd->pre_commit_context);
		if (r < 0) {
			DMERR("pre-commit callback failed: %d", r);
			return -EDATAFLUSH;
		}
	}
	...
	if (old_meta_block_size != pmd->metadata_block_size)
		DMERR("%s: fix metadata block size from %llu to %llu", __func__,
		      old_meta_block_size, pmd->metadata_block_size);
...
static int __reserve_pool_block(struct dm_pool_metadata *pmd,
				dm_block_t reserve)
{
...
static int __alloc_new_device(struct dm_pool_metadata *pmd, dm_thin_id dev,
			      struct disk_device_details *details_le,
			      struct dm_thin_device **td, bool changed)
{
 ...
 */
static int __open_new_device(struct dm_pool_metadata *pmd, dm_thin_id dev,
			     int create, struct dm_thin_device **td)
{
	int r;
	bool changed = 0;
...
static int __open_old_device(struct dm_pool_metadata *pmd, dm_thin_id dev,
			     int create, struct dm_thin_device **td)
{
	int r;
	bool changed = false;
...
static int __open_device(struct dm_pool_metadata *pmd, dm_thin_id dev,
			 int create, struct dm_thin_device **td)
{
	if (old_meta(pmd->version))
...
static int __set_clone_details(struct dm_pool_metadata *pmd,
			       struct dm_thin_device *clone, dm_thin_id origin,
			       uint32_t time)
{
	int r;
	struct dm_thin_device *td;
...
static int __create_snap(struct dm_pool_metadata *pmd, dm_thin_id dev,
			 dm_thin_id origin, bool is_clone)
{
	int r;
...
static int __get_origin_mapped_blocks(struct dm_pool_metadata *pmd,
				      dm_block_t *b)
{
	struct dm_thin_device *td;
...
int dm_pool_get_origin_mapped_blocks(struct dm_pool_metadata *pmd,
				     dm_block_t *b)
{
	down_read(&pmd->root_lock);
	if (pmd->space_reserve)
				 ...
				 dm_block_t new_threshold)
{
	...
	if (pmd->space_reserve && new_threshold < pmd->mapped_blocks)
		r = -EINVAL;
	else
...
static int __delete_device(struct dm_pool_metadata *pmd, dm_thin_id dev)
{
	...
	mapped_blocks =
		__is_snapshot(td) ? 0 : max(td->mapped_blocks, td->preallocate);
	__free_thin_device(td);

	r = dm_btree_remove(&pmd->details_info, pmd->details_root, &key,
			    &pmd->details_root);
	if (r)
		return r;
	/*
...
static int __clear_device(struct dm_pool_metadata *pmd, dm_thin_id dev)
{
	...
	root_le = cpu_to_le64(empty_root) __dm_bless_for_disk(&root_le);
	r = dm_btree_insert(&pmd->tl_info, pmd->root, &key, &root_le,
			    &pmd->root);
	if (r)
		return r;
...
int dm_pool_clear_thin_device(struct dm_pool_metadata *pmd, dm_thin_id dev)
{
	int r = -EINVAL;
...
static int __get_thin_mapped_blocks(struct dm_pool_metadata *pmd, dm_thin_id id,
				    dm_block_t *mapped)
{
	int r;
	struct dm_thin_device *td;
...
static int __snapshot_revert(struct dm_pool_metadata *pmd, dm_thin_id origin_id,
			     dm_thin_id snap_id, dm_thin_id clone_id)
{
	int r;
	...
	if (pmd->space_reserve && pmd->max_blocks < pmd->mapped_blocks -
							    mapped_oblocks +
							    mapped_sblocks) {
		...
		DMERR("%s: max_blocks: %llu mapped_blocks: %llu origin blocks: %llu clone blocks: %llu",
		      __func__, pmd->max_blocks, pmd->mapped_blocks,
		      mapped_oblocks, mapped_sblocks);
		return r;
	}
...
int dm_pool_snapshot_revert(struct dm_pool_metadata *pmd, dm_thin_id origin_id,
			    dm_thin_id snap_id, dm_thin_id clone_id)
{
	int r = -EINVAL;
...
static bool __snapshotted_since(struct dm_thin_device *td, uint32_t time)
{
				  ...
				  struct dm_thin_lookup_result *result)
 {
	 ...
	 result->shared =
		 __snapshotted_since(td, exception_time) | result->cloned;
		    ...
		    dm_block_t data_block, unsigned zeroed, uint32_t *time)
{
	...
	value = cpu_to_le64(
		pack_block_time(data_block, time ? *time : pmd->time, zeroed));

	__dm_bless_for_disk(&value);

	r = dm_btree_insert_notify(&pmd->info, pmd->root, keys, &value,
				   &pmd->root, &inserted, NULL);
	if (r)
		return r;
...
static int __prep(struct dm_thin_device *td, dm_block_t block, uint64_t *next_key)
{
	int r, inserted;
	__le64 value;
	struct dm_pool_metadata *pmd = td->pmd;
	...
	r = __find_block(td, block, 1, &result);
	if (r)
		return r;

	value = cpu_to_le64(pack_block_time(result.block, result.time, result.zeroed));

	__dm_bless_for_disk(&value);

	r = dm_btree_insert_notify(&pmd->info, pmd->root, keys, &value,
	                           &pmd->root, &inserted, next_key);
	if (r)
		return r;

	td->changed = true;
	if (inserted) {
		DMERR("prep block %llu should never lead to mapping insert", block);
		return -EINVAL;
	}
...
int dm_thin_prep_discard_range(struct dm_thin_device *td, dm_block_t block, dm_block_t end)
{
	int r = -EINVAL;
	uint64_t next_key;
	dm_block_t head = block;
	...
	while (block < end) {
		r = __prep(td, block, &next_key);
		if (r)
			goto out;

		block = min((dm_block_t)next_key + 1, end);
	}
out:
	pmd_write_unlock(td->pmd);
...
static int __remove_range(struct dm_thin_device *td, dm_block_t begin, dm_block_t end)
{
	...
	if (td->preallocate < td->mapped_blocks + td->on_the_fly) {
		dm_block_t exceeds =
			td->mapped_blocks + td->on_the_fly - td->preallocate;
		__free_pool_block(pmd, min((dm_block_t)total_count, exceeds));
	}
...
int dm_pool_block_ref(struct dm_pool_metadata *pmd, dm_block_t b, uint32_t *result)
{
	int r;

	down_read(&pmd->root_lock);
	r = dm_sm_get_count(pmd->data_sm, b, result);
	up_read(&pmd->root_lock);
...
{
	int r = -EINVAL;
	struct dm_pool_metadata *pmd = metadata;
...
{
	if (pmd->metadata_block_size == 8) {
		return 255 * ((1 << 14) - 64) * pmd->metadata_block_size;
	} else if (pmd->metadata_block_size == 16) {
		return 511 * ((1 << 15) - 64) * pmd->metadata_block_size;
	} else
		DMERR("unsupported block size");
...
static sector_t get_metadata_dev_size(struct dm_pool_metadata *pmd,
				      struct block_device *bdev)
{
...
dm_block_t get_metadata_dev_size_in_blocks(struct dm_pool_metadata *pmd,
					   struct block_device *bdev)
{
...
void warn_if_metadata_device_too_big(struct dm_pool_metadata *pmd,
				     struct block_device *bdev)
{
	sector_t metadata_dev_size =
		i_size_read(bdev->bd_inode) >> SECTOR_SHIFT;
	...
	if (metadata_dev_size > warning_metadata_max_sector(pmd))
		DMWARN("Metadata device %s is larger than %llu sectors: excess space will not be used.",
			...
			struct dm_thin_device *dst_td, dm_block_t dst_blk)
{
	...
	if (!src->cloned) {
		...
		r = __insert(src_td, src_blk, src->block, src->zeroed,
			     &src->time);
		if (r)
			goto fail;
	}
...
int dm_thin_set_preallocate(struct dm_thin_device *td, dm_block_t threshold)
{
	...
	if (td->preallocate > max(threshold, mapped))
		__free_pool_block(pmd,
				  td->preallocate - max(threshold, mapped));
	else if (threshold > max(td->preallocate, mapped))
		r = __reserve_pool_block(pmd, threshold - max(td->preallocate,
							      mapped));
	if (!r)
...
#define OFF false
static int
turn_feature(struct dm_pool_metadata *pmd, unsigned long feature, bool onoff)
{
	int r = -EINVAL;
...
{
	return turn_feature(pmd, THIN_FEATURE_SUPERBLOCK_BACKUP, ON);
}

int dm_pool_stop_backup_sb(struct dm_pool_metadata *pmd)
{
	return turn_feature(pmd, THIN_FEATURE_SUPERBLOCK_BACKUP, OFF);
}

int dm_pool_enable_block_clone(struct dm_pool_metadata *pmd)
{
	return turn_feature(pmd, THIN_FEATURE_FAST_BLOCK_CLONE, ON);
}

int dm_pool_disable_block_clone(struct dm_pool_metadata *pmd)
{
	return turn_feature(pmd, THIN_FEATURE_FAST_BLOCK_CLONE, OFF);
}
