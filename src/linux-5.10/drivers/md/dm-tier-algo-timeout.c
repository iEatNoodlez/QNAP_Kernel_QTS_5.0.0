...
{
	int cool_down, curseconds = get_seconds();;

	if (*tierid >= data->tier_num - 1)
		return false;

	cool_down = atomic_read(&timeout->cool_down);
	if (curseconds - atomic_read(&timeout->block_stats[block].lastused) >= cool_down)
...
{
	return (int)get_seconds() - atomic_read(&bstats->lastused);
}
