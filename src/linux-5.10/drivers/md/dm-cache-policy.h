...
struct policy_stat {
	...
	atomic_t reclaim;
	atomic_t invalidate;
	atomic_t periodic_cleaning;
	atomic_t force_cleaning;
	atomic_t callocated;
	atomic_t requeue;
};
...
struct dm_cache_policy {
	 ...
	 * lookup).
	 */
	int (*lookup_with_work)(struct dm_cache_policy *p, dm_oblock_t oblock,
				dm_cblock_t *cblock, int data_dir, bool fast_copy,
				struct policy_work **work);

	int (*lookup_simple)(struct dm_cache_policy *p, dm_oblock_t oblock,
			    dm_cblock_t *cblock);
		/*
	...
	void (*sort_mapping)(struct dm_cache_policy *p);
	...
	int (*show_stats)(struct dm_cache_policy *p, char *result,
			  unsigned maxlen, ssize_t *sz_ptr);

	int (*cache_resize) (struct dm_cache_policy *p, sector_t origin_size);

	void (*suspend) (struct dm_cache_policy *p);
	int (*resume) (struct dm_cache_policy *p);

	void (*shuffle)(struct dm_cache_policy *p);

	struct policy_stat stat;
...
struct dm_cache_policy_type {
	...
	struct module *owner;
	struct dm_cache_policy *(*create)(sector_t origin_size,
