...
struct background_tracker *btracker_create(unsigned max_work)
{
	...
	r = mempool_init_slab_pool(&b->work_pool, b->max_work,
				   b->work_cache);
	if (r) {
		DMERR("couldn't create mempool for background work");
...
static struct bt_work *alloc_work(struct background_tracker *b)
{
	...
	return mempool_alloc(&b->work_pool, GFP_NOWAIT);
}
...
struct policy_work *btracker_search(struct background_tracker *b,
		                    dm_oblock_t oblock)
{
	struct bt_work *w = __find_pending(b, oblock);

	return w ? &w->work : NULL;
}
EXPORT_SYMBOL_GPL(btracker_search);
