...
static int crypt_map(struct dm_target *ti, struct bio *bio)
{
	...
	if (unlikely(bio_op(bio) == REQ_OP_QNAP_MAP_ZERO)) {
		DMERR_LIMIT("crypt target does not support REQ_QNAP_MAP_ZERO bio");
		bio_io_error(bio);
...
static int crypt_locate_thin(struct dm_target *ti, locate_thin_callout_fn fn,
			     ...
			     void **thin)
{
	...
	return fn(cc->dev->bdev, cc->start + dm_target_offset(ti, start), len,
		  remap_desc, thin);
}

static int crypt_invalidate(struct dm_target *ti, sector_t start, sector_t len,
			    invalidate_callback_fn fn)
{
	struct crypt_config *cc = ti->private;
