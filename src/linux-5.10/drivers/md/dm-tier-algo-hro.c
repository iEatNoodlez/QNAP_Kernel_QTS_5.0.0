...
static uint64_t hro_score(struct per_block_stats *stats, int weight)
{
	uint64_t score, freq;
...
static void rawdata_analysis_callback(void *ctx1, 
	void *ctx2, struct per_block_info *info)
{
	struct analyze_data *data = (struct analyze_data *)ctx1;
	dm_block_t *block_num = (dm_block_t *)ctx2;
	uint32_t tid = 0, bottom_tid;
...
static void rawdata_analysis(struct dm_tier_algo *a, struct analyze_data *data)
{
	...
	arrange_stats(hro, data);
 	hro_build_cluster_set(data);
	hro_cluster_set_walk(&data->set,
		rawdata_analysis_callback, data, block_num);
	return;
}	

static void rectify_callback(void *ctx1, void *ctx2, struct per_block_info *info)
{
...
static void stats_rectify(struct dm_tier_algo *a, struct analyze_data *data)
{
	hro_cluster_set_walk(&data->set, rectify_callback, a, data);
	return;
}
...
static int hro_resize(struct dm_tier_algo *a, dm_block_t new_block_num)
{
	...
	new_stats = vzalloc(array_size(new_block_num,
				       sizeof(struct per_block_stats)));
