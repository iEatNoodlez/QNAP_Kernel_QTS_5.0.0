 ...
 */
struct page_list *alloc_pl(gfp_t gfp)
{
	struct page_list *pl;
...
EXPORT_SYMBOL_GPL(alloc_pl);

void free_pl(struct page_list *pl)
{
	__free_page(pl->page);
	kfree(pl);
}
EXPORT_SYMBOL_GPL(free_pl);
...
struct io {
	...
	struct bio *bio; //? not sure we still need this
		      ...
		      struct io *io)
{
	...
	else if (op == REQ_OP_QNAP_MAP || op == REQ_OP_QNAP_MAP_ZERO)
		special_cmd_max_sectors = queue_max_hw_sectors(q);
