...
static int zhaoxin_rng_init(struct hwrng *rng)
{
	...
	/* Some CPUs don't have the MSR_ZHAOXIN_RNG anymore.  The RNG
	 ...
	 */
	rdmsr(MSR_ZHAOXIN_RNG, lo, hi);

	old_lo = lo;
	lo &= ~(0x7f << ZHAOXIN_STRFILT_CNT_SHIFT);
	lo &= ~ZHAOXIN_XSTORE_CNT_MASK;
	lo &= ~(ZHAOXIN_STRFILT_ENABLE | ZHAOXIN_STRFILT_FAIL | ZHAOXIN_RAWBITS_ENABLE);
	lo |= ZHAOXIN_RNG_ENABLE;
	lo |= ZHAOXIN_NOISESRC1;

	/* Enable secondary noise source on CPUs where it is present. */
    lo |= ZHAOXIN_NOISESRC2;

	if (lo != old_lo)
		wrmsr(MSR_ZHAOXIN_RNG, lo, hi);

	/* perhaps-unnecessary sanity check; remove after testing if
	   unneeded */
	rdmsr(MSR_ZHAOXIN_RNG, lo, hi);
	if ((lo & ZHAOXIN_RNG_ENABLE) == 0) {
...
static const struct x86_cpu_id zhaoxin_rng_ids[] __initconst = {
...
static int __init mod_init(void)
{
	...
	if (!boot_cpu_has(X86_FEATURE_XSTORE) || !x86_match_cpu(zhaoxin_rng_ids))
		return -ENODEV;
