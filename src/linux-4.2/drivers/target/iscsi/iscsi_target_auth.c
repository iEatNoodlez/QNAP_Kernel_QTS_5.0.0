...
#ifdef CONFIG_MACH_QNAPTS
	if (type != HEX)
		qnap_iscsit_chap_base64_to_hex(client_digest, chap_r, sizeof(client_digest));
	else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if(qnap_iscsit_chap_base64_to_hex(decode_pwd, auth->password, sizeof(decode_pwd)) < 0){
...
#ifdef CONFIG_MACH_QNAPTS
	if(qnap_iscsit_chap_base64_to_hex(decode_pwd, auth->password_mutual, sizeof(decode_pwd)) < 0){
