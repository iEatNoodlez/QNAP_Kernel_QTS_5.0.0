	...
	if (np->np_sockaddr.ss_family == AF_INET6) {
		...
		if (!rc) {
			if (!ipv6_addr_v4mapped(&sock_in6.sin6_addr)) {
				snprintf(conn->local_ip, sizeof(conn->local_ip), "[%pI6c]",
					&sock_in6.sin6_addr.in6_u);
				snprintf(conn->local_ip_address, sizeof(conn->local_ip_address), "%pI6c",
					&sock_in6.sin6_addr.in6_u);
			} else {
				snprintf(conn->local_ip, sizeof(conn->local_ip), "%pI4",
					&sock_in6.sin6_addr.s6_addr32[3]);
				snprintf(conn->local_ip_address, sizeof(conn->local_ip_address), "%pI4",
					&sock_in6.sin6_addr.s6_addr32[3]);
			}
		...
		if (!rc) {
			sprintf(conn->local_ip, "%pI4",
					&sock_in.sin_addr.s_addr);
			sprintf(conn->local_ip_address, "%pI4",
					&sock_in.sin_addr.s_addr);
