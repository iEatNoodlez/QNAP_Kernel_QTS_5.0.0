			...
			list_for_each_entry_safe(cmd, cmd_p, &conn->conn_cmd_list, i_conn_node) {
				...
				if(se_cmd) {
					...
					data_size += sprintf(data_buf + data_size, 
						...
						"op:0x%x, tmf:0x%x, b_se_state:0x%llx,0x%llx, time:%ld (%ld-%ld)\n", 
						se_cmd, be32_to_cpu(se_cmd->se_tfo->get_task_tag(se_cmd)), se_cmd->se_tfo->get_cmd_state(se_cmd), 
						se_cmd->t_state, se_cmd->transport_state, se_cmd->t_task_cdb[0], se_cmd->tmf_code,
...
static ssize_t lio_target_tpg_show_cmd_dump(
	struct se_portal_group *se_tpg,
	char *page)
{
	return lio_dump_iscsi_target_cmd(se_tpg, page);
}

TF_TPG_BASE_ATTR_RO(lio_target, cmd_dump);
